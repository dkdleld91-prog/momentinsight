...
```json
{
  ...
  "placeName": "상호명",
  "maxRank": 300,
  "providerDeadlineAt": 1784520000000
}
```
...
Actor 결과 배열의 순서는 실제 네이버 PC 목록과 다를 수 있으므로 운영 순위의 근거로 사용하지 않습니다. 운영 수집기는 네이버 PC 목록에서 직접 검증한 오가닉 행만 순위로 반환하며, 확인 범위에서 대상을 찾지 못한 경우 Actor 순위로 대체하지 않고 `rank:null`과 실제 `checkedCount`를 반환합니다. Apify 관련 함수와 설정은 공급자 출력 진단 및 향후 별도 보조 필드 연동을 위해서만 남아 있습니다.

Vercel은 요청마다 절대 마감 시각 `providerDeadlineAt`을 전달합니다. 이 값에는 Render cold start가 포함되며, 수집기는 마감 전 응답·브라우저 종료 여유를 남긴 채 실제 확인한 부분 결과를 반환합니다. 직접 호출에서 이 값을 생략하면 수집기 기본 예산을 사용합니다.
...
- 운영 순위는 `source=naver_map_pc_list_collector`, `rankEvidence=naver_pc_organic_list`인 실제 PC 오가닉 목록만 신뢰합니다.
