...
- 키워드 시장 3지표 요약 개발·배포 검증 완료: 운영팀과 광고주 `키워드 조회`에 기존 연관 키워드 미리보기와 조화를 이루는 컴팩트 카드로 `검색 수요`·`경쟁 강도`·`판매 기회율`을 추가했다. 검색 수요는 정확 월 검색량의 로그 점수, 경쟁 강도는 검색광고 경쟁도 60%와 쇼핑 상품수/검색량 포화도 40%, 판매 기회율은 수요 65%와 역경쟁 35%의 참고 지표다. 판매율을 실제 주문·매출 전환율로 오인하지 않도록 명칭을 `판매 기회율`로 확정하고 고지를 상시 표시한다. 정확 검색량이 없는 범위값에서는 수요·판매 기회 점수를 만들지 않는다. 신규 3개 회귀를 포함한 API·서버 165/165, 수집기 51/51, Production 인증 18/18, 역할 parity·CSP·전체 `check:release`가 통과했으며 N 상품·플레이스 30일 보호 잠금 4함수·20파일·11마이그레이션도 그대로다.
