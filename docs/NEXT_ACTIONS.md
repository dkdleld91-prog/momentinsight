...
- 호칭을 실장으로 전환하고(호출어 포함) "○○ 완료로 해줘" 음성·텍스트 완료 명령을 추가했습니다(owner-tool 11/11·work-items 15/15·전체 release gate 통과, 보호 잠금 2건 대표 지시로 갱신). 다음은 로그인된 owner 화면에서 호출어 "실장" 인식과 완료 명령 1건 실사용 확인입니다.
