...
- 상품 SEO 공개 화면 자동 점검 로컬 완료·배포 대기: 초보 사업자가 직접 선택하던 상세페이지·상품정보고시·할인율·리뷰 포인트 입력을 제거했다. 네이버 상품 URL 하나로 상품명·카테고리·리뷰·할인·리뷰 포인트·상세콘텐츠 등록 근거를 서버에서 자동 확인하고 운영팀·광고주 화면에 동일하게 반영한다. 공개 화면만으로 확정할 수 없는 상품정보고시 작성 상태와 상세페이지 80% 충족 여부는 점수를 발명하지 않고 `자동 확인 불가`로 안내한다.
- 자동 점검 검증 상태: 실제 상품 `12149720593`에서 리뷰 457개, 판매가 86,100원·할인가 68,900원·할인율 19%, 리뷰 포인트 최대 1,000원, 상세콘텐츠 등록 근거를 확인했다. 전용 자동 수집·평가 15/15, API·서버 194/194, 플레이스 수집기 51/51, 서버 계약 24/24, 운영팀·광고주 parity, 기준선, 4대 기능 잠금 13함수·21파일·11마이그레이션, Vercel 빌드·공개 CSP 검사를 통과했다. 인앱 브라우저의 localhost 접근은 `ERR_BLOCKED_BY_CLIENT`로 차단되어 배포 전 운영 미리보기에서 최종 클릭 검수를 남긴다.
