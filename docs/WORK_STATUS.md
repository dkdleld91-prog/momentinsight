...
- 플레이스 정확성 수정: 플레이스ID `2019299673`은 이름 유사도보다 정확 ID를 강제하고, 입력 URL의 `lng/lat` 좌표를 검색에 사용하며 공식 모바일 플레이스에서 `팽오리농장 부평점` 상호명을 보강한다. 네이버 공개 목록이 54개에서 끝난 실조회는 300위 미노출로 확정하지 않고 부분 조회로 저장해 기존 확정값을 보존한다.
