...
check(
  "N Shopping runtime 1.1.19 adds bounded rendered-order recovery and keeps exact-parent identity",
  hasAll(shoppingStableFiniteWindowMigration, [
    ...
    && shoppingCandidatePerformanceAudit.includes(
      `export const N30_TARGET_RUNTIME_VERSION = "1.1.19";`,
    )
...
check(
  ...
  shoppingChromeManifest.version === "1.1.19"
    ...
    && /const EXPECTED_RUNTIME_VERSION = "1\.1\.19";/.test(shoppingLocalWorker)
    && /const EXPECTED_WORKER_RUNTIME_VERSION = "1\.1\.19";/.test(shoppingLocalWorkerHandler)
    && /const SHOPPING_WORKER_EXPECTED_RUNTIME_VERSION = "1\.1\.19";/.test(productTrackers)
