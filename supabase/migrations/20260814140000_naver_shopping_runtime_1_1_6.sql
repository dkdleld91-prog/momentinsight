...
-- Runtime 1.1.6 additionally hardens row isolation, signed clock skew,
-- native request framing, and bounded submit payloads without relaxing atomic 300.
-- Keep progress and cadence fail-closed
-- so an older installed runtime cannot claim to be release-ready.
...
begin
  if p_run_id is null
    ...
    or trim(coalesce(p_runtime_version, '')) <> '1.1.6'
...
begin
  ...
  return jsonb_build_object(
    ...
    'candidate_eligible', coalesce((current_row.circuit_state = 'closed'
      ...
      and current_row.runtime_version = '1.1.6'
