-- Runtime 1.1.24 (2026-09-11): failure evidence. The local worker now records
-- rendered-order failure details that fall outside its known list (sanitized
-- to the same safe charset) instead of erasing them to a bare code; the first
-- 1.1.23 evening logged three bare `provider_stable_rendered_order_unproven`
-- rows that could not be investigated without a console run. Scope
-- classification reads only the base code, so nothing else changes.
-- Collector page rules are unchanged from 1.1.23.
...
-- Apply right after the 1.1.24 server release is live and the coordination row
-- is idle on 1.1.23 (the release itself stops the 1.1.23 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_24_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.23'
    ...
    raise exception 'naver_shopping_runtime_1_1_24_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.23'
       or runtime_fingerprint is distinct from
         '26231beb6eb5cdd127afd3e216e96564d8923ee5195331723c5bc7f7443bc870'
  ) then
    raise exception 'naver_shopping_runtime_1_1_24_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.24',
      runtime_fingerprint =
        '56d274e272155d42771963fc003700b4478f4e94965305ff9ac88b8814c3fa33'
  where runtime_version = '1.1.23'
    and runtime_fingerprint = '26231beb6eb5cdd127afd3e216e96564d8923ee5195331723c5bc7f7443bc870';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_24_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.23'
    and runtime_fingerprint = '26231beb6eb5cdd127afd3e216e96564d8923ee5195331723c5bc7f7443bc870';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_24_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.24';
