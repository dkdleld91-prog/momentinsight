...
- 상품 SEO 기준 재설계 완료·배포 전: 키워드 시장 분석에 속하는 검색 수요·상품 수·경쟁강도를 SEO 점수에서 제거했다. 새 점수는 `상품명 키워드 15`, `40자 이내 10`, `동종 상위 상품 카테고리 20`, `리뷰 20`, `상세페이지 80% 이상 10`, `상품정보고시 직접 작성 10`, `할인율 8`, `리뷰 포인트 7`의 100점 구조다. 공개 쇼핑 API로 확정 가능한 상품명·카테고리·가격 근거는 자동 확인하고 상세페이지·상품정보고시·할인·리뷰 포인트는 판매자 확인 전 점수를 만들지 않는다.
- 진단·검증·배포 상태: 모든 기본 항목이 양호한데 공식 오가닉 확인 범위가 100위 밖이거나 상위 300개에서 미확인일 때만 `SEO 기본 양호 · 트래픽 점검`을 표시하며 실제 트래픽 수치를 발명하지 않는다. 전용 회귀 8/8, API·서버 187/187, 플레이스 수집기 51/51, 역할 parity·기준선, 4대 기능 잠금 13함수·21파일·11마이그레이션, Vercel 빌드·공개 CSP 검사를 통과했다. 인앱 브라우저와 Chrome은 로컬 주소를 보안 정책으로 차단해 시각 캡처는 수행하지 못했으며 현재 배포 지시가 없어 Production에는 반영하지 않았다.
