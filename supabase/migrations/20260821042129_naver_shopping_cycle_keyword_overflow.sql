...
-- The worker envelope remains bounded to 100 tracker claims.  Trackers beyond
-- that bound are explicitly rostered as deferred for this cycle, without a
-- lease, rank mutation, history row, retry mutation, or fabricated success.
alter table public.naver_rank_trackers
  add column if not exists worker_last_cycle_deferred_at timestamptz;

comment on column public.naver_rank_trackers.worker_last_cycle_deferred_at is
  'Latest durable N Shopping cycle deferral caused only by the bounded 100-claim keyword group envelope.';
...
alter table public.naver_shopping_scheduler_events
  ...
  drop constraint if exists naver_shopping_scheduler_events_deferred_tracker_check,
  ...
  add constraint naver_shopping_scheduler_events_deferred_tracker_check check (
    event_type <> 'tracker_deferred'
    or (cycle_id is not null and tracker_id is not null and group_fingerprint is not null)
  );

create unique index if not exists idx_naver_shopping_scheduler_events_cycle_deferred_once
on public.naver_shopping_scheduler_events(cycle_id, tracker_id)
where event_type = 'tracker_deferred';

create or replace function mi_internal.mi_audit_naver_shopping_tracker_deferred()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if not found
    or current_row.scheduler_cycle_status <> 'active'
    or current_row.scheduler_cycle_id is null then
    return null;
  end if;

  -- Cycle-start rows already exist in the ledger. A tracker first observed
  -- after that snapshot must still join the authoritative denominator before
  -- its bounded deferral is recorded.
  ...
  select
    new_row.worker_last_cycle_deferred_at,
    'cycle_rostered',
    new_row.worker_last_cycle_id,
  ...
  where new_row.worker_last_cycle_id = current_row.scheduler_cycle_id
    and new_row.worker_last_cycle_deferred_at is not null
    and new_row.worker_last_cycle_deferred_at
      is distinct from old_row.worker_last_cycle_deferred_at
    and new_row.processing_started_at is not distinct from old_row.processing_started_at
    and new_row.processing_until is not distinct from old_row.processing_until
    and new_row.last_checked_at is not distinct from old_row.last_checked_at
  on conflict (cycle_id, tracker_id)
  ...
  select
    ...
    jsonb_build_object(
      'reason', 'group_claim_limit',
      'claimLimit', 100,
      'lastCheckedAt', new_row.last_checked_at
    )
  from new_rows as new_row
  join old_rows as old_row on old_row.id = new_row.id
  where new_row.worker_last_cycle_id = current_row.scheduler_cycle_id
...
revoke all on function mi_internal.mi_audit_naver_shopping_tracker_deferred()
from public, anon, authenticated, service_role;

drop trigger if exists trg_mi_audit_naver_shopping_tracker_deferred
on public.naver_rank_trackers;
create trigger trg_mi_audit_naver_shopping_tracker_deferred
after update on public.naver_rank_trackers
referencing old table as old_rows new table as new_rows
for each statement execute function mi_internal.mi_audit_naver_shopping_tracker_deferred();
...
begin
  ...
  if p_probe_tracker_id is null then
    ...
    -- normalized keyword still exists.  The global cycle retries after that
