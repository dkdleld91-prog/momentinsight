...
- 자동 메모 정리(2026-09-08 점검): 총관리자가 연장·만료일 지정(`set-plan` extend/set)·무기한(`clear-plan`)을 하면 크론의 자동 메모(`isAutomaticPlanNote`: "구글 미연동 · …", "구글 연동 · …")를 null 로 지운다 — 남겨 두면 새 기간의 유예가 3일로 계산되고 카드·팝업에 옛 문구가 남는다. 이름·일수만 바꾸는 meta 는 메모를 건드리지 않는다. 넛지 팝업은 기한이 지난 뒤 "10월 7일 연동 기한이 지났습니다 — 지금 구글 계정을 연결하고 카카오톡 채널로 연장을 요청해 주세요…"로 바뀐다. 크론의 총관리자 코드 보호는 환경변수와 무관하게 상수(`PRIMARY_AGENCY_CODE`)로도 막는다(clients 표에 총관리자 코드 행 "브랜드 A" 가 실제로 있음).
