...
- 배포 상태: 코드 `0e8a4b0`을 GitHub `main`과 Vercel Production `dpl_61KjLZdC4zrTK8PYkmhFSEQk2SkW`에 반영했다. 운영 `/health`·`/ready`는 릴리스 `0e8a4b07c116`·서울 `icn1`·Supabase ready, 관리자·광고주 화면 200, 비인증 업무 API 401이다. 운영 브라우저에서도 `대행사 연결`·`키워드 조회`·`업무 운영`의 단독 노출과 코드 `datalist`·`list` 0건을 재확인했다.
