-- Runtime 1.1.28 (2026-09-13): same-page seller twins without a per-window bound.
-- 1.1.27 kept at most two same-page twins in the rendered-order candidate and
-- proof; its first production run (2026-09-13 00:24 KST, keyword 콘트로이친,
-- ofyou) still failed with `provider_duplicate_identity:5:48:duplicate_row:5`,
-- so Naver lists that seller product three or more times on page 5. The strict
-- path has never bounded same-page twins; the rendered-order path now matches
-- it exactly. Cross-page repeats stay rejected.
...
-- Apply right after the 1.1.28 server release is live and the coordination row
-- is idle on 1.1.27 (the release itself stops the 1.1.27 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_28_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.27'
    ...
    raise exception 'naver_shopping_runtime_1_1_28_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.27'
       or runtime_fingerprint is distinct from
         'f153198fd05fe6d79efffa0ca39da7a4ff5e65a84d96a2c3de89526e988cdbc8'
  ) then
    raise exception 'naver_shopping_runtime_1_1_28_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.28',
      runtime_fingerprint =
        '6bf207914784d28c8cff9ebb857614d3244684e06b4749453f3f7ee845c2fe88'
  where runtime_version = '1.1.27'
    and runtime_fingerprint = 'f153198fd05fe6d79efffa0ca39da7a4ff5e65a84d96a2c3de89526e988cdbc8';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_28_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.27'
    and runtime_fingerprint = 'f153198fd05fe6d79efffa0ca39da7a4ff5e65a84d96a2c3de89526e988cdbc8';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_28_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.28';
