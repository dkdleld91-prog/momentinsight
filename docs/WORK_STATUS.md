...
### 2026-08-08 N상품 보안확인 반복 차단 안정화 로컬 완료

- 보안확인은 4건 처리 전에 이미 `max_jobs=2`에서 발생했으며, 4건 설정 실행도 첫 작업에서 기존 확인 화면을 감지하고 중단했습니다.
- 안전 상한을 2건으로 복귀하고 기존 보안확인 탭이 해결되기 전에는 서버 작업을 선점하지 않도록 확장 1.0.6을 보완합니다.
- 확인 완료 뒤 정상 검색 데이터가 나타나면 다음 실행에서 자동으로 차단 상태를 해제하며, CAPTCHA 자동 해결·우회는 하지 않습니다.
- 대상 28/28과 전체 릴리스 검사, 보호 잠금 self-test를 통과했고 중앙 Mac 설치본을 `max_jobs=2`로 교체했습니다. 확장 1.0.6 재로드와 현재 보안확인 완료 후 첫 원자 300개 수집은 아직 확인 전입니다.
