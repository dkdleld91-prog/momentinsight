-- Runtime 1.1.26 (2026-09-12): rendered-order proof identity, second pass.
-- 1.1.25 fell back to the canonical product URL, but the failing cards (2026-09-12
-- 12:53–13:33 KST, keywords 찜질기·탄소매트, six windows in a row) carry neither
-- a numeric seller id nor a product link — only Naver's product id. The proof
-- now identifies a card exactly like the strict path (seller id → catalog id →
-- URL → product id). Page-seam tolerance widened to the leading 3 / trailing 5
-- organic rows (`2:9:page_overlap:1`, 13:23·13:43).
...
-- Apply right after the 1.1.26 server release is live and the coordination row
-- is idle on 1.1.25 (the release itself stops the 1.1.25 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_26_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.25'
    ...
    raise exception 'naver_shopping_runtime_1_1_26_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.25'
       or runtime_fingerprint is distinct from
         '424d352b4667427cde8aa272d847cdf29c2c03c5b7760df6e38e5c8c18f2e05e'
  ) then
    raise exception 'naver_shopping_runtime_1_1_26_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.26',
      runtime_fingerprint =
        '6033788f59076da8d625a078a5066385347e9d7fa9d679f48189605596857013'
  where runtime_version = '1.1.25'
    and runtime_fingerprint = '424d352b4667427cde8aa272d847cdf29c2c03c5b7760df6e38e5c8c18f2e05e';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_26_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.25'
    and runtime_fingerprint = '424d352b4667427cde8aa272d847cdf29c2c03c5b7760df6e38e5c8c18f2e05e';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_26_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.26';
