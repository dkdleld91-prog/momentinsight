...
- 30일 순위 복구: 보안 세션 전환 후 총관리자 기본 조회 대상이 실제 `mml93-a01`이 아니라 `session`으로 남아 상품·플레이스 목록 API가 403을 반환했고, UI가 이를 0개 목록처럼 표시한 회귀를 확정했다. DB에는 상품 추적 88개·플레이스 추적 13개와 스냅샷 3,593개가 그대로 보존되어 있다.
...
- 복구 검증: 역할별 조회 계약과 CSP 회귀 기준을 추가했고 서버 계약 22/22, API·서버 122/122, 플레이스 26/26, 전체 `check:quality`와 공개 빌드 무결성을 통과했다. 운영 반영과 Production 실화면 재검증 전 단계다.
