...
### 2026-08-09 N상품 09·15 안내 분리·전체 순환 수집 v1.0.14

- 오전 9시·오후 3시는 광고주에게 보여주는 갱신 기준으로 유지하고, 실제 개발 Chrome 수집은 30분마다 전체 활성 목록을 다시 확인하는 연속 순환 큐로 분리했습니다.
...
- 운영 읽기 전용 확인은 `status=active` 59건·고유 키워드 47개·현재 due 키워드 47개·활성 lease 0건입니다. 오류가 없을 때 약 23.5시간에 한 바퀴이며 병렬 브라우저 수집은 사용하지 않습니다.
- 네이버 제한·418·429는 2시간·6시간·12시간·24시간 보호 대기 후 1건만 재개합니다. CAPTCHA·보안확인은 우회하지 않고, 실패·부분 수집은 마지막 정상 순위와 30일 이력을 유지한 채 남은 oldest-first 큐를 이어갑니다.
- 대상 테스트 50/50, API·서버 397/397, 플레이스 51/51, 쇼핑 51/51, 서버 계약 38/38, Production 인증 18/18, 보호 잠금·공개 빌드·CSP와 전체 `npm run check:release`를 통과했습니다.
- 코드 `49bcd6e`를 GitHub `main`과 Vercel Production `https://momentinsight-jx8uvfc3k-momentlabs.vercel.app`·운영 별칭에 반영했습니다. `/health`·`/ready`는 릴리스 `49bcd6e0d896`, 서울 `icn1`, Supabase ready이고 관리자·광고주 화면은 200입니다.
- Mac 브리지 14개 파일은 저장소와 byte-for-byte 일치하고 scheduler는 `Profile 5`, 10분·08:50·14:50, 최근 exit 0입니다. 고정 확장 ID allowlist와 소스 1.0.14도 확인했습니다.
- Chrome 보안 정책상 `chrome://extensions` 재로드 버튼의 원격 조작은 차단됐습니다. 강제 종료나 우회는 하지 않았으며, 사용자가 `동빈(개발)`에서 확장 재로드를 한 번 누른 뒤 로드 버전 1.0.14와 제한 해제 후 신규 원자 300개를 확인해야 합니다.
