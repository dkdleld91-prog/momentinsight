...
- N 상품 30일 전체 갱신 안전 가속: `mml93-a01`의 운영 추적 25개를 직렬 처리해 약 186초가 걸리던 UI 전체 갱신을 동시 2개로 제한했다. 페이지 진입 시 숨은 `sync-due`도 50개에서 2개로 줄이고, 진행 중인 자동 갱신이 있으면 수동 전체 갱신이 먼저 기다려 같은 추적을 겹쳐 호출하지 않는다. 개별 실패는 나머지 항목을 중단하지 않고 재시도 예정으로 분리한다.
- N 플레이스 30일 전체 갱신 안전 가속: 수집기는 계속 단일 브라우저·직렬 처리한다. 동일 키워드는 네이버 원본 목록이 정상적으로 끝까지 소진된 결과 또는 요청 범위 완주 결과만 기존 10분 캐시에 저장해 재사용하며, 마감·스크롤 한도·선택자 실패·폴백 결과는 캐시하지 않는다. 수집기 `429 collector_busy`는 절대 마감 안에서 `Retry-After`만큼 대기 후 재시도한다.
- 정확성 보존: 상품의 정확 상품ID·검증 원부ID 비교, 광고 제외, 오가닉 300개 확인과 플레이스의 정확 Place ID·네이티브 PC 오가닉 근거·부분조회 진실성은 변경하지 않았다. DB 스키마와 기존 이력도 수정하지 않았다. 배포 전 운영 DB 기준 `mml93-a01` 상품 추적 25개·스냅샷 1,913개, 플레이스 추적 10개·스냅샷 353개를 보존 확인했다.
- 검증 상태: 신규 수집기 2개·서버 busy 재시도 2개 회귀를 포함해 API·서버 160/160, 플레이스 수집기 51/51, 서버 계약 22/22, Production 인증 18/18, 운영팀·광고주 parity·CSP 공개 빌드·전체 `check:release`·`git diff --check`를 통과했다. 빌드된 양 역할 화면은 스크립트 오류·가로 넘침 0건이며 안전 갱신 마커와 기존 50개 자동 일괄 호출 제거를 재확인했다.
