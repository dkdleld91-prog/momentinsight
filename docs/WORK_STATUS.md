...
- 상품 SEO 공개 화면 자동 점검·429 안전 처리 재배포 진행: 초보 사업자가 직접 선택하던 상세페이지·상품정보고시·할인율·리뷰 포인트 입력을 제거했다. 네이버 상품 URL 하나로 상품명·카테고리·리뷰·할인·리뷰 포인트·상세콘텐츠 등록 근거를 서버에서 자동 확인하고 운영팀·광고주 화면에 동일하게 반영한다. 공개 화면만으로 확정할 수 없는 상품정보고시 작성 상태와 상세페이지 80% 충족 여부는 점수를 발명하지 않고 `자동 확인 불가`로 안내한다.
- 자동 점검 운영 검수 상태: 첫 Production `7d2959b895da`는 배포·health·ready가 정상이었으나 실제 상품 조회에서 네이버 공개 페이지 429가 일반 서버 500으로 표시되는 결함을 발견했다. 네이버 429·연결 제한을 재시도 가능한 공개 소스 제한으로 분리하고, 상품명·카테고리·가격의 공식 조회 결과는 계속 표시하도록 보완했다. 실제 상품 `12149720593`의 공개 근거와 429 실응답을 각각 확인했고 전용 17/17, API·서버 196/196, 플레이스 수집기 51/51, 서버 계약 24/24, Production 인증 18/18, 역할 parity·기준선·4대 기능 잠금·Vercel 공개 빌드/CSP 검사를 통과했다.
