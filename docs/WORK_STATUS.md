...
- SEO 수동 유입·구매 입력 제거·로컬 검증 완료: 공개 상품 URL만으로는 판매자 내부의 최근 30일 유입수·구매수를 신뢰성 있게 자동 수집할 수 없으므로 운영자 수동 입력란과 두 수치의 점수 반영을 운영팀·광고주 모두에서 제거했다. SEO 점수는 상품명 30점·카테고리 20점·검색 시장 25점·리뷰 신뢰 25점의 확인 가능한 네 근거만 사용한다. 리뷰 수 상승은 독립적인 긍정 신호로 유지하고 공식 API 결과 순번은 점수에 포함하지 않는 참고값으로 남긴다.
- 자동 연동 경계·검증 상태: 유입·결제 통계는 판매자 인증과 권한 동의를 거친 네이버 커머스 통계 연동이 실제 구축된 경우에만 자동 신호로 재도입한다. 공개 URL 크롤링이나 추정값으로 대체하지 않는다. 리뷰 상승·수동 유입/구매 무시·4항목 고정 회귀 6/6, API·서버 185/185, 플레이스 수집기 51/51, 서버 계약 23/23, Production 인증 18/18, 역할 parity·수동 입력 부재, 4대 기능 잠금 13함수·21파일·11마이그레이션, 공개 빌드·CSP·전체 `check:release`를 통과했다. 운영팀·광고주 데스크톱과 390px에서 입력란 0건·가로 넘침 0건을 확인했으며 Production 배포는 별도 승인 대기다.
