-- 2026-09-19: standby recovery of a navigation-failure circuit while the primary is down.
-- Incident (KST): 09-18 13:34 windows-desktop-primary went silent (desktop failure) ->
-- 14:28 macbook-standby took the lane over (first live proof of failover) -> its first two
-- jobs failed within a second with naver_page_navigation_failed (the Mac had just woken) ->
-- 14:29 circuit open `navigating:naver_page_navigation_failed`. The automatic navigation
-- probe was primary-only and navigation failure is not part of the transient standby
-- handoff class, so every standby claim was refused `circuit_open` until the owner ran a
-- conditional reset at 09-19 00:27 (eleven hours without collection).
-- This re-declares mi_claim_naver_shopping_worker_lane with two changes only:
--   1. the navigation probe may be opened by a standby whose primary is stale
--      (primary_seen_at older than p_primary_stale_seconds, default 180 s);
--   2. the half-open `auto_navigation_probe` lane admits that standby instead of
--      answering `primary_required`.
-- The 10 minute quiet period, the single lease, the failure-code match and the
-- primary-only transient-system probe are unchanged. No runtime literal is carried.
begin;
