...
- 로그인 없는 N 상품 30일 자동 추적 대기열 보완·배포 대기: GitHub Actions 크론과 `MI_RANK_CRON_SECRET`은 정상 실행 중이었으나 상품 워크플로가 1회에 최대 10개만 처리해 활성 71개 중 35개가 밀렸고, 로그인 화면의 1건 보정이 실행 원인처럼 보였다. 플레이스는 활성 13개·대기 0개로 로그인 없이 정상 처리되고 있어 별도 수집기·순위 로직은 변경하지 않았다.
- 수정·보호·검증 상태: 상품 크론 요청은 최대 5건을 기존처럼 순차 처리하고 한 작업 창은 최대 20회·100건까지만 처리한다. 기본 Vercel 백업 요청은 1건을 유지하고 DB 임대 잠금·광고 제외·300위·대표 순위·실패 재시도 계약은 그대로다. 새 경계 단위검사 포함 API·서버 224/224, 플레이스 수집기 51/51, 서버 계약 28/28, 역할 5상태·핵심 5기능, 운영팀·광고주 parity, 공개 빌드/CSP, Production 인증 18/18과 전체 `check:release`가 통과했다. `admin.html`·`client.html`·Supabase 스키마·운영 데이터는 변경하지 않았고 승인된 상품 크론 서버·워크플로 두 파일만 보호 잠금 새 기준으로 다시 고정했다. 배포 후 로그인 없이 대기열이 0으로 소진되는 실제 실행과 `/health`·`/ready`를 최종 확인한다.
