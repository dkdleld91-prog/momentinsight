...
### 2026-08-25 N쇼핑 runtime1.1.13 candidate6·run provenance (로컬 검증 중, 미배포)
...
- migration 감사에서 coordination row만 먼저 잠근 뒤 constraint DDL이 더 강한 table lock을 기다리는 lock-order P1을 RED로 확인했습니다. `lock_timeout=5s`와 transaction 첫 단계의 `ACCESS EXCLUSIVE` table lock으로 순서를 고정해 GREEN 및 독립 재감사를 통과했습니다. 5초 안에 잠금을 못 얻으면 migration 전체가 rollback되는 fail-closed 계약입니다. 로컬에는 Docker/Podman이 없어 신규 SQL을 실제 PostgreSQL에 적용·compile하지 못했으므로, 현재 상태는 미커밋·미배포이고 Production 완전 idle transaction 적용과 실제 함수/ACL/advisor 검증 전에는 DB 정상화로 기록하지 않습니다. candidate setter 호출도 0회입니다.
