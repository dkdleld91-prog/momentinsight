...
const checks = {
  ...
  seoScoringBasisVisible: [adminSource, clientSource].every((source) => source.includes("SEO 점수 기준")
    && source.includes("상품명 적합도 25점")
    && source.includes("키워드 포함·50자 이내·중복 및 홍보 문구")
    && source.includes("상품정보 적합도 20점")
    && source.includes("상위 상품 세부 카테고리·브랜드·제조사")
    && source.includes("리뷰 경쟁력 20점")
    && source.includes("상위 오가닉 상품 최대 5개의 평균 리뷰 대비")
    && source.includes("등록정보 완성도 20점")
    && source.includes("태그 10개·항목별 고시·상세 이미지 8컷")
    && source.includes("순위·트래픽 15점")
    && source.includes("광고 제외 오가닉 5위 이내에서만 만점")
    && source.includes("/seo-evaluation.js?v=seo-v9-20260726")
    && source.includes("검색 수요·경쟁과 가격·할인·배송·리뷰 포인트는 SEO 점수에서 제외합니다.")
    && source.includes("공개 화면에서 확정된 항목만 표시합니다.")
    ...
    && source.includes('<script src="/seo-evaluation.js?v=seo-v9-20260726"></script>')
