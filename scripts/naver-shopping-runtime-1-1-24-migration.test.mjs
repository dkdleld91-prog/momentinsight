...
// Runtime 1.1.24 (2026-09-11): failure-detail evidence (unknown rendered-order
// failure details are recorded sanitized instead of erased). The migration
// only moves the runtime identity pins; every other RPC stays runtime-neutral.

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const migrationDirectory = path.join(root, "supabase", "migrations");
const migrationName = "20260911120000_naver_shopping_runtime_1_1_24_failure_evidence.sql";
const priorMigrationName = "20260911090000_naver_shopping_runtime_1_1_23_rendered_page_tolerance.sql";
...
test("migration moves only the runtime identity pins from 1.1.23 to 1.1.24", () => {
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_24_requires_completed_account_priority'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_24_requires_idle_control_plane'/u);
  ...
  assert.match(migration, new RegExp(`set runtime_version = '1\\.1\\.24',\\s+runtime_fingerprint =\\s+'${NEW_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, new RegExp(`where runtime_version = '1\\.1\\.23'\\s+and runtime_fingerprint = '${OLD_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, /check \(runtime_version = '1\.1\.24'\)/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_24_finite_target_identity_mismatch'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_24_target_mismatch'/u);
  assert.match(migration, /runtime_version = null,\s+runtime_fingerprint = null/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_24_coordination_mismatch'/u);
  ...
  assert.match(progress, /expected_runtime_version constant text := '1\.1\.24';/u);
  ...
  assert.doesNotMatch(progress, /1\.1\.23|26231beb/u);
...
test("the runtime literal audit still passes with the 1.1.24 progress gate as the only carrier", () => {
