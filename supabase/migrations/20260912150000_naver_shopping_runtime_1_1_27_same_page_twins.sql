-- Runtime 1.1.27 (2026-09-12): same-page seller twins in the rendered-order proof.
-- Production 2026-09-11 02:19 → 2026-09-12 18:43 KST, keyword 콘트로이친 (ofyou):
-- eight cycles in a row failed with `provider_duplicate_identity:5:4x:duplicate_row:5`.
-- Naver renders one seller product twice on one SSR page under two product ids.
-- The strict window already keeps both rank slots of such a twin; the
-- rendered-order candidate pass and its two-capture proof now do the same,
-- bounded to two twins per window. Cross-page repeats stay rejected.
...
-- Apply right after the 1.1.27 server release is live and the coordination row
-- is idle on 1.1.26 (the release itself stops the 1.1.26 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_27_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.26'
    ...
    raise exception 'naver_shopping_runtime_1_1_27_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.26'
       or runtime_fingerprint is distinct from
         '6033788f59076da8d625a078a5066385347e9d7fa9d679f48189605596857013'
  ) then
    raise exception 'naver_shopping_runtime_1_1_27_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.27',
      runtime_fingerprint =
        'f153198fd05fe6d79efffa0ca39da7a4ff5e65a84d96a2c3de89526e988cdbc8'
  where runtime_version = '1.1.26'
    and runtime_fingerprint = '6033788f59076da8d625a078a5066385347e9d7fa9d679f48189605596857013';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_27_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.26'
    and runtime_fingerprint = '6033788f59076da8d625a078a5066385347e9d7fa9d679f48189605596857013';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_27_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.27';
