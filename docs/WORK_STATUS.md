...
- 총관리자 전용 대행사 연결 UI·UX 보완 완료·배포 대기: `mml93-a01` Owner 모드에만 `계정 발급 → 운영팀 생성(선택) → 광고주 생성·연결(필수) → 연결 현황`의 정보 계층을 적용했다. 불필요하게 노출되던 비밀값 입력 형태는 서버 재검증을 알리는 보안 세션 상태로 정리하고, 입력 라벨·직속 발급 안내·처리 결과 위치를 명확히 했다. 운영팀 모드는 기존 `광고주 코드 생성` 단일 구조·문구·placeholder를 그대로 유지하며 `client.html`, DB, 서버, 계정 생성 API와 보호된 5대 조회·추적 기능은 변경하지 않았다.
- 검증 상태: 1280px 브라우저에서 총관리자 두 생성 카드 290.5px 균등, CTA 254.5px 전폭, 운영팀 단일 카드 471px, 양 화면 가로 넘침 0을 확인했다. 총관리자 전용 요소는 Owner 10개 노출·운영팀 0개 노출로 분리됐다. 역할 5상태, 운영팀·광고주 parity, 보호 잠금 21함수·23파일·11마이그레이션, API·서버 226/226, 플레이스 수집기 51/51, 서버 계약 28/28, Production 인증 18/18, 공개 빌드/CSP와 전체 `check:release`가 통과했다. 배포 승인은 없으므로 로컬 커밋까지만 진행하고 Production은 변경하지 않는다.
