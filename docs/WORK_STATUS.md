...
- 운영 입력 프리미엄 정보 계층 재설계 완료·배포 대기: 기존의 여섯 카드가 같은 중요도로 펼쳐지던 화면을 `운영 원본 → 진행 상태 → 핵심 결과 → 공개 전 확인` 순서로 재구성했다. 원본 파일·기본 양식·검수·공개 버튼만 우선 노출하고 연결 광고주·승인 기준은 `공개 설정`, CSV 붙여넣기·간단 계산은 `보조 입력` 접이식 영역으로 낮췄다. 원본 업로드·다운로드·삭제, 검수 기준 6개, 월간 자동 계산, 공개 반영과 CSV 계산 데이터 훅은 그대로 유지했으며 `client.html`, DB, 서버와 보호된 5대 조회·추적 기능은 변경하지 않았다.
- 검증·배포 상태: 로컬 운영팀 단독 세션에서 입력값 검수 5/6 정상·원본 미선택 경고, 공개 설정 펼침, CSV `3,280만원·456%·1,080건` 계산, 공개 반영 완료를 확인했다. 1440×1000 데스크톱 육안검수와 390×844 모바일 단일 열·가로 넘침 0, 브라우저 경고/오류 0건을 확인했고 역할 5상태, 운영팀·광고주 parity, 보호 잠금 21함수·23파일·11마이그레이션, API·서버 224/224, 플레이스 51/51, 서버 계약 28/28, Production 인증 18/18, 전체 `check:release`를 통과했다. 사용자의 이번 요청에는 배포 승인이 없으므로 Production 배포는 하지 않는다.
