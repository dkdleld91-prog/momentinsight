...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | 운영팀 선생성 안내 및 광고주 코드 사전차단 | 활성 운영팀이 없을 때 광고주 발급용 운영팀 코드를 비우고, 먼저 운영팀 코드를 만들도록 안내. 임의 운영팀 코드 입력 시 서버 오류 전에 UI에서 차단 | 라이브 API activeTeams=0 확인, `build:vercel`, 원본/복붙 파일 동기화 검색, 로컬 브라우저 로그인/목록/사전차단 검증 | 배포 예정 |
