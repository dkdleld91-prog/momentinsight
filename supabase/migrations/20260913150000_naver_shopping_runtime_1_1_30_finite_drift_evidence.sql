-- Runtime 1.1.30 (2026-09-13): finite markets that also drift, and failure evidence.
-- 일신한일의료기 탄소매트 (fke4ha8gcav4, 215 products) failed four cycles in a row
-- (`provider_stable_finite_window_unproven`, `provider_partial_window:215_300`): its
-- pages drift (paid slots consume raw numbers) and the recovery branch was chosen from
-- the first pass alone, so a finite market with drift could never reach the finite
-- arbitration. Now a partial window on either pass enters finite arbitration, whose
-- candidates fall back to the positional rendered-order parse on drift; pages past the
-- end of a finite market are tolerated. The native host also summarises every capture
-- of a failed collection onto the failure report (bounded, identities and raw numbers
-- only) and the server keeps it 30 days in naver_shopping_failure_evidence
...
-- Apply right after the 1.1.30 server release is live and the coordination row
-- is idle on 1.1.29 (the release itself stops the 1.1.29 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_30_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.29'
    ...
    raise exception 'naver_shopping_runtime_1_1_30_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.29'
       or runtime_fingerprint is distinct from
         '3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77'
  ) then
    raise exception 'naver_shopping_runtime_1_1_30_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.30',
      runtime_fingerprint =
        '62e09cb15720dda97fdc652cfa9b3fba40bdbbc4c98372269867c21ee472d10c'
  where runtime_version = '1.1.29'
    and runtime_fingerprint = '3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_30_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.29'
    and runtime_fingerprint = '3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_30_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.30';
