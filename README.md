...
- 일정 및 공개 코멘트 영역
- 공개 코멘트와 내부 메모 분리
- 키워드 조회, 네이버 상품 순위 추적, 네이버 플레이스 순위 추적, 네이버 SEO 확인, Meta 광고 조사
...
- 자동 순위추적은 GitHub Actions repository secret `MI_RANK_CRON_SECRET`과 Vercel Production 환경변수 `CRON_SECRET`이 같은 값이어야 안정적으로 동작합니다. Vercel 백업 Cron은 `CRON_SECRET` 이름일 때만 Authorization 헤더를 자동 전송합니다.
