...
### 2026-08-09 N상품 네이버플러스 검색 경로 전환 로컬 검증·보호 잠금 승인 대기

- 네이버 통합검색의 `쇼핑` 탭이 기존 가격비교 `/search/all`이 아니라 네이버플러스 `/ns/search`로 이동하는 것을 실제 Chrome `동빈` 프로필에서 확인했습니다. 기존 가격비교 경로는 같은 프로필에서도 네트워크 임시 제한 화면을 반환해 현재 수집 경로로 사용할 수 없습니다.
- 확장프로그램 수집 경로를 `/ns/search`로 전환하고, 공식 DOM 메타데이터 `data-shp-contents-rank`·`data-shp-contents-dtl`에서 판매자 상품 ID·상품 ID·카탈로그 ID를 분리합니다. `data-shp-contents-grp=ad`·광고 링크·광고 배지는 제외하며 300개가 완성되지 않으면 저장하지 않습니다.
- 네이버플러스의 가상 스크롤 구조를 순차 이동하며 중복 없이 누적한 실제 `물티슈` 검색 검증에서 원시 388개·광고 제외 88개·오가닉 300개를 완전 수집했습니다. 첫 항목부터 300번째 항목까지 상품명·판매자 상품 ID·카탈로그 ID·URL을 파싱했고 기존 순위 판정·30일 기록·화면·DB 구조는 변경하지 않았습니다.
...
- 순위 보호 잠금이 변경된 수집기 4개 파일을 의도대로 차단하고 있습니다. 테스트 통과 사실만으로 잠금을 임의 갱신하지 않으며, 사용자 명시 승인 후 잠금 해시 갱신 → self-test → 전체 `npm run check:release` → 중앙 Mac 브리지·확장 설치 검증 순서로 진행합니다. 현재는 커밋·배포하지 않았습니다.
