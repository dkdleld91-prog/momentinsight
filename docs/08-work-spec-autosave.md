...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | N 상품·플레이스 30일 전체 갱신 안전 가속 | 상품 전체 갱신은 제한 동시성 2개로만 처리하고 숨은 자동 갱신과의 중첩을 차단. 플레이스는 네이버 원본 목록의 정상 소진 결과만 같은 키워드에 재사용하고 수집기 사용 중 응답은 제한 대기 후 재시도하여 속도와 정확성을 함께 보존 | API·서버 160/160, 수집기 51/51, 서버 계약 22/22, Production 인증 18/18, 양 역할 parity·CSP·공개 빌드·2차 화면·diff 통과. DB 상품 25/스냅샷 1,913·플레이스 10/스냅샷 353 보존 | 사용자 배포 승인, 운영 반영·실측 진행 |
