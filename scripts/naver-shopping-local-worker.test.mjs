...
function workerEnv() {
  return {
    ...
    MI_NAVER_SHOPPING_RUNTIME_VERSION: "1.1.28",
...
test("derives a content fingerprint for the direct Mac standby fallback", async () => {
  ...
  assert.equal(lane.runtimeVersion, "1.1.28");
...
test("claims one canonical keyword, submits one strict 300 window and drains catch-up", async () => {
  ...
  assert.equal(coordination[0].runtimeVersion, "1.1.28");
...
test("submits one stable finite canary without recording an atomic300 success", async () => {
  ...
  assert.equal(calls.coordination[0].runtimeVersion, "1.1.28");
...
test("isolates every rendered-order proof failure and continues the next scheduled keyword", async () => {
  const cases = [
    ...
    {
      code: "provider_stable_rendered_order_unproven",
      detail: "page_boundary:2:g0:l13",
      expected: "provider_stable_rendered_order_unproven:page_boundary:2:g0:l13",
    },
    {
    ...
    // 1.1.28: details outside the known list are recorded (sanitized), not erased.
    {
      code: "provider_stable_rendered_order_unproven",
      detail: "renderedOrderProof.identity",
      expected: "provider_stable_rendered_order_unproven:renderedorderproof_identity",
    },
    {
      code: "provider_rendered_order_candidate_invalid",
      detail: "2:Helper Slot!",
      expected: "provider_rendered_order_candidate_invalid:2:helper_slot",
    },
  ];
...
test("isolates an exact rank-drift group and continues the next scheduled keyword", async () => {
  ...
  const provider = {
    async collect(request) {
      ...
      if (collectCount === 1) {
        ...
        error.detail = "p1:iy:rl:ek:o0:mk:fx:zx:uf:d0:vj:n1:af:qf:h0:s1";
        throw error;
      }
      return { ...completeWindow(), keyword: request.keyword };
    },
  ...
  assert.equal(
    calls[1].errorCode,
    "naver_next_data_rank_drift:p1:iy:rl:ek:o0:mk:fx:zx:uf:d0:vj:n1:af:qf:h0:s1",
  );
  assert.ok(calls[1].errorCode.length <= 80);
  assert.match(calls[1].errorCode, /^[a-z0-9_:-]{3,80}$/u);
  const failure = calls.coordination.find((call) => call.action === "record-failure");
  assert.equal(failure.scope, "tracker");
  assert.equal(calls.coordination.some((call) => call.action === "block-lane"), false);
  assert.equal(calls.coordination.filter((call) => call.action === "record-success").length, 1);
