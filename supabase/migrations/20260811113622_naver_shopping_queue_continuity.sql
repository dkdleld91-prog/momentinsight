...
-- v1.1.1 keeps one browser job at a time, but makes the queue handoff durable:
-- an unverified tracker always receives its first slot before lookup/aged work,
-- while the existing row lock, due aging and advertiser round-robin stay intact.
...
begin
  if p_run_id is null
    ...
    or trim(coalesce(p_runtime_version, '')) <> '1.1.1'
...
begin
  ...
  if coalesce(p_has_new, false) then
    work_class := 'new';
  elsif coalesce(p_has_due, false)
...
begin
  ...
  return jsonb_build_object(
    ...
    'candidate_eligible', coalesce((current_row.circuit_state = 'closed'
      ...
      and current_row.runtime_version = '1.1.1'
