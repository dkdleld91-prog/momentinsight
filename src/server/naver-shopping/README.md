...
- 오전 9시·오후 3시는 광고주 화면의 갱신 안내 기준입니다. 실제 개발 Chrome은 20분마다 사이트 전체 활성 대기열을 멱등 확인하고 oldest-first로 키워드 1개씩 계속 순환합니다.
- 네트워크 제한·418·429는 30/60/120분 보호 대기 후 1건만 재개하고 이후에도 120분을 상한으로 유지합니다. CAPTCHA·보안확인은 자동 우회하지 않습니다. 실패한 작업과 남은 작업은 마지막 정상값을 보존한 채 기존 대기 순서로 이어집니다.
- 인증된 `프로그램 개발` Chrome 프로필을 가진 승인 작업용 Mac 또는 Windows 데스크탑이 켜져 있고 사용자 세션이 유지돼야 새 300위 수집이 가능합니다. 작업용 장비가 꺼져 있어도 서버 상위 50위 즉시 경로와 기존값 보존 안전망은 계속 작동하지만, 새로운 51~300위 증거는 만들 수 없습니다.
- Windows 설치기는 Chrome 공식 HKCU Native Messaging 등록, 현재 사용자 범위 DPAPI 비밀키, 로그인 사용자 전용 10분 작업 스케줄러를 사용합니다. `chrome://extensions`의 압축해제 확장 로드는 Chrome 보안 경계상 사용자가 한 번 직접 수행합니다.
