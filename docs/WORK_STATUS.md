...
- 상태: 네이버 상품 단건·30일 대표값을 정확 상품과 관련 원부 중 더 높은 오가닉 순위로 통일하고, 40개 보기 기준 페이지·페이지 내 순위를 복구. 실조회·양 역할 UI·전체 품질검사 2회 완료, 로컬 커밋 준비
- 차단 사항: 독립 서버의 네이버 쇼핑 화면 자동 접근은 HTTP 418로 제한된다. 운영 순위는 공식 검색 API의 광고 제외 오가닉 순서를 사용하며, 로그인 네이버 가격비교 화면과 대상 사례를 직접 교차검증했다. 기존 쇼핑 검색 API는 2026-07-31 종료 예정
- 사용자 배포 승인: 정상 실행·2차 테스트·육안 확인 완료 시 배포 승인 범위. 로컬 커밋 후 Production 재검증 예정
