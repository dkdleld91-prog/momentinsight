...
- 실조회: 가상 목록을 끝으로 점프해 중간 행을 놓치고, 장소 카드 내부 프로모션 `li`를 업체로 세던 추가 원인도 제거했다. 최종 독립 조회에서 `홍대 맛집`/`1907427831`은 실제 목록과 수집기가 같은 오가닉 7위, `부평 맛집`/`2019299673`은 양쪽 모두 상위 100개 미발견이며 두 키워드 top10 ID·순위가 전부 일치했다. 네이버 공개 목록은 현재 100개에서 종료되므로 101~300위는 임의 확정하지 않는다.
- 회귀 범위: 상품 N 30일 순위의 조회·광고 제외·원부/정확 상품 대표 판정은 변경하지 않았다. 전체 `check:release`는 API·서버 153/153, 플레이스 tracker 계약 42/42, 플레이스 수집기 40/40, Production 인증 18/18, 서버 계약 22/22, 역할 parity·CSP 공개 빌드를 통과했다. 최초 2차 검수에서 발견한 drain 응답 P0도 서버·workflow 계약을 함께 수정해 재검증한다.
