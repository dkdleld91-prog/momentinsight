-- Runtime 1.1.20 adds one bounded third-capture consensus only when exactly
-- one of the first two rendered-order captures has a numeric page-boundary
-- violation and the other capture is valid. It preserves direct-ID ordering, account-priority
-- evidence, finite-window proof, last-good ranks, scheduler order and cadence.
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_20_requires_completed_account_priority';
  end if;
  ...
  if coordination_found is not true
    ...
    raise exception 'naver_shopping_runtime_1_1_20_requires_idle_control_plane';
  end if;
end
$migration_guard$;

-- A completed request is immutable evidence for the runtime that processed it.
-- Scope cohort uniqueness by that immutable runtime identity so the same
-- account cohort can receive exactly one bounded request after a runtime
-- upgrade without rewriting or deleting the prior request.
alter table public.naver_shopping_account_priority_requests
  drop constraint if exists
    naver_shopping_account_priority_req_agency_code_cohort_hash_key;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.19'
       or runtime_fingerprint is distinct from
         '631f2a556a1337ed9e9e9a72c8f07ed607928e97853b7d93611be04d97bfa13e'
  ) then
    raise exception 'naver_shopping_runtime_1_1_20_finite_target_identity_mismatch';
  end if;
  ...
  where runtime_version = '1.1.19'
    and runtime_fingerprint = '631f2a556a1337ed9e9e9a72c8f07ed607928e97853b7d93611be04d97bfa13e';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_20_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.19'
    and runtime_fingerprint = '631f2a556a1337ed9e9e9a72c8f07ed607928e97853b7d93611be04d97bfa13e';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_20_coordination_mismatch';
  end if;
end
...
begin
  if p_request_id is null
    ...
    or v_expected_runtime_version <> '1.1.20'
    or v_expected_runtime_fingerprint <>
      '4e0f5fbde16a892e44986b2325865f33d61bdf7a5a13d3d7adcd501608aa8e5b' then
