-- Runtime 1.1.25 (2026-09-12): rendered-order proof identity. A seller card
-- whose mall product id is not numeric carries no sellerProductId; the
-- rendered-order digest threw `renderedOrderProof.direct_identity` for every
-- window containing such a card (production 2026-09-12 01:55 KST, keyword
-- 배찜질기 — the same detail that 1.1.24 made visible). The digest now falls
-- back to the card's canonical product URL, the direct identity the strict
-- path already trusts (tools/naver-shopping-rank-collector/src/contract.mjs).
...
-- Apply right after the 1.1.25 server release is live and the coordination row
-- is idle on 1.1.24 (the release itself stops the 1.1.24 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_25_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.24'
    ...
    raise exception 'naver_shopping_runtime_1_1_25_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.24'
       or runtime_fingerprint is distinct from
         '56d274e272155d42771963fc003700b4478f4e94965305ff9ac88b8814c3fa33'
  ) then
    raise exception 'naver_shopping_runtime_1_1_25_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.25',
      runtime_fingerprint =
        '424d352b4667427cde8aa272d847cdf29c2c03c5b7760df6e38e5c8c18f2e05e'
  where runtime_version = '1.1.24'
    and runtime_fingerprint = '56d274e272155d42771963fc003700b4478f4e94965305ff9ac88b8814c3fa33';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_25_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.24'
    and runtime_fingerprint = '56d274e272155d42771963fc003700b4478f4e94965305ff9ac88b8814c3fa33';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_25_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.25';
