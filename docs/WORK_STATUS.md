...
### 2026-08-13 N쇼핑 상품 식별 중복 전역 보완 v1.1.2
...
- 페이지 간 강한 중복은 새 전체 8페이지 수집을 정확히 1회만 수행하고 두 번째에도 같으면 즉시 종료합니다. 행을 삭제·압축해 가짜 순위를 만들지 않으며 광고 제외 오가닉 300개·last-good 계약을 유지합니다.
...
- 로컬 구현과 회귀는 완료됐지만 이 항목의 DB·Production·Windows v1.1.2 반영 및 두 건 원자 300개 운영 결과는 아직 진행 중이므로 정상화 완료로 기록하지 않습니다.
