...
8. `Moment Insight N Shopping Rank` 버전 `1.1.11`, ID `pflggephankeefaeoaafkmggampnaefm`을 확인합니다.

## 자동 실행

- Windows 작업 스케줄러의 `\MomentInsight\NaverShoppingChrome` 작업이 로그인 직후와 10분 간격으로 `프로그램 개발` Chrome 프로필을 확인합니다.
- 확장은 baseline 10분마다 열린 DB cycle을 깨우고 한 번에 키워드 그룹 1개만 순차 수집합니다. 기존 키워드는 cycle당 1회이며 신규 키워드 1건 뒤 저장 cursor로 복귀합니다.
- 네트워크 제한·418·429와 보안확인은 전역 cooldown으로 멈추고 자동 우회하지 않습니다. 일반 tracker 오류는 해당 tracker의 재시도 시각까지만 격리합니다.
- CAPTCHA·보안확인 화면을 사람이 완료하면 다음 유한 wake부터 기존 순서로 재개합니다.
