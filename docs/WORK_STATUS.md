...
- `남자팬티` 쇼핑 표본은 대분류 ID `50000000`(패션의류)를 제공하고 있었지만, 기존 모바일 파서가 카테고리명만 읽고 ID를 버려 연령 API 호출이 `category_required`로 생략됐습니다.
- 허용된 대분류 ID를 표본 결과에 보존하고 키워드 프로필이 ID를 우선 사용하도록 수정했습니다. 이름 기반 기존 fallback은 유지합니다.
- 관리자·광고주의 기존 5개 세로 막대그래프와 tooltip·최신 완료 월 선택 로직은 변경하지 않았습니다.
- 로컬 운영 환경 실호출에서 `남자팬티` HTTP 200, category `50000000`, 2026-07 비중 `0.5 / 3.9 / 18.3 / 35.2 / 42.1%`, `profileStatus.age=ok`, warning 0을 확인했습니다. Production 반영은 `/health`·`/ready` release와 복구 태그 일치로 판정합니다.
- 사용자 지시에 따라 N 상품·N 플레이스 30일 추적과 Windows·DB 경계는 추가 요청 전까지 동결합니다. 이번 diff는 키워드 조회와 공유하는 mobile fallback parser에 검증된 `category1Id` 메타데이터만 추가하며, 30일 순위·광고 제외·300개·매칭·저장·스케줄·UI·migration은 변경하지 않습니다.
