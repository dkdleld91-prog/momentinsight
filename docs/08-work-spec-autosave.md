...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 배포 대기 | 운영팀·광고주 사이드바 UI 통일 | 운영팀 콘솔의 `운영 → 키워드·SEO → 순위 조회·추적 → 광고 조사` 분류를 광고주 화면에도 동일 적용. 두 역할의 사이드바 폭, 브랜드 마크, 메뉴 간격, hover·active, 인증 패널을 딥네이비 프리미엄 공통 스타일로 통일하고 모바일 메뉴 순서도 같은 정보 구조로 정렬 | 전체 `check:quality`, 공통 쉘 릴리즈 기준선, Preview 운영팀 1280px 실화면·가로 넘침 없음, 광고주 Preview DOM·계산 CSS·390px 모바일 규칙 확인. Preview는 Supabase 환경 미연결로 광고주 실제 로그인만 Production에서 최종 확인 | Preview 완료 · Production 최종 검증 대기 |
