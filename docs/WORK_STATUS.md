...
- 일반 Chrome 실화면에서 `온열찜질기` 페이지 1~8의 광고 4+오가닉 40 구성과 오가닉 1~320 연속, 상품 `12149720593` 정확 91위를 확인했다. 운영 API 실점검에서 공통 로그인 세션 장벽이 HMAC 워커보다 먼저 `401 SESSION_REQUIRED`를 반환하는 배선 결함도 찾아, 정확한 `/api/naver-shopping-local-worker`만 세션 장벽 밖에 두고 기존 HMAC·nonce 검증이 직접 인증하도록 수정했다. 네이티브 브리지는 설치됐지만 Chrome 보안 정책상 확장 프로그램의 사용자 1회 수동 로드가 남아 있다. 코드·계약 검사는 실제 DB 반영 증거가 아니므로 최근 `collection_id=pw-chrome-*`, `checked_count=300` snapshot 전까지 Production 배포는 대기한다.
