...
// Runtime 1.1.27 (2026-09-12): rendered-order proof identity falls back to the
...
const migrationName = "20260912150000_naver_shopping_runtime_1_1_27_same_page_twins.sql";
const priorMigrationName = "20260912060000_naver_shopping_runtime_1_1_26_rendered_identity_fallback.sql";
...
test("migration moves only the runtime identity pins from 1.1.26 to 1.1.27", () => {
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_27_requires_completed_account_priority'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_27_requires_idle_control_plane'/u);
  ...
  assert.match(migration, new RegExp(`set runtime_version = '1\\.1\\.27',\\s+runtime_fingerprint =\\s+'${NEW_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, new RegExp(`where runtime_version = '1\\.1\\.26'\\s+and runtime_fingerprint = '${OLD_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, /check \(runtime_version = '1\.1\.27'\)/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_27_finite_target_identity_mismatch'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_27_target_mismatch'/u);
  assert.match(migration, /runtime_version = null,\s+runtime_fingerprint = null/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_27_coordination_mismatch'/u);
  ...
  assert.match(progress, /expected_runtime_version constant text := '1\.1\.27';/u);
  ...
  assert.doesNotMatch(progress, /1\.1\.26|6033788f/u);
...
test("the runtime literal audit still passes with the 1.1.27 progress gate as the only carrier", () => {
