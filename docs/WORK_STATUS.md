...
- N 상품·N 플레이스 30일 핵심 기능 잠금 완료: 운영팀·광고주 추적 함수 4개와 상품·플레이스 서버, 크론, 플레이스 수집기, GitHub 워크플로, 순위 DB 마이그레이션을 해시로 고정했다. 관련 핵심 코드가 바뀌거나 새로운 순위 마이그레이션이 추가되면 `check:quality`·`check:release`가 즉시 실패한다. 정상 잠금은 4개 함수·20개 파일·11개 마이그레이션을 통과했고, 가상 함수 변조 차단 self-test와 전체 `check:release`에서 API·서버 162/162, 수집기 51/51, 역할 parity·기준선·공개 빌드·CSP·Production 인증 18/18을 통과했다. 현재 배포 미승인 상태다.
...
- 홈페이지 서비스 안내 팝업 기능명 통일: 내부 메뉴와 달랐던 `네이버 상품순위`·`네이버 30일 순위`·`네이버 플레이스 30일 순위`만 `N 상품 순위`·`N 30일 순위`·`N 플레이스 30일 순위`로 변경했다. 팝업 크기·위치·노출·닫기·1주일 숨김 로직은 변경하지 않았다. 전체 `check:quality`와 기준선·공개 빌드·CSP를 통과했고 데스크톱 348×489px·모바일 347×489px, 넘침·브라우저 경고/오류 0건과 닫기 동작을 확인했다. 현재 배포 미승인 상태다.
