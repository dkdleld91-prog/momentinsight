...
- 공식 API 실호출에서 `남자팬티`는 category `50000000`만 단일 강한 후보였고 2026-07 비중은 `0.5 / 3.9 / 18.3 / 35.2 / 42.1%`였습니다. 모호·stale은 값을 만들지 않고 일부 실패·429는 남은 탐색을 즉시 중단합니다. 성공 프로필은 30분, 미확정·제한 결과는 5분 TTL로 재사용해 반복 호출을 차단하며 Production 재검증은 아직 남아 있습니다.
