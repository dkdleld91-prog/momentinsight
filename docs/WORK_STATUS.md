...
- 업무 운영 워크플로 1차 로컬 구현 완료·배포 대기: 총관리자와 운영팀에 `업무 운영` 화면을 추가해 월간 캘린더, 오늘·내일·다음 업무, 업무 등록·수정·삭제, 담당자·상태·우선순위·내부 메모를 관리한다. 광고주가 없는 운영팀도 내부 업무를 사용할 수 있고, 광고주가 연결된 업무에서만 `광고주에게 일정 공개`를 켤 수 있다. 공개 기본값은 비공개이며 공개 시에도 광고주 API에는 공개 제목·일정·상태·공개 안내만 전달하고 내부 메모·계정 범위 값은 반환하지 않는다.
- 구현·검증 상태: `/api/work-items`, 역할별 서버 범위, 감사 로그, `schedule_items` 확장 마이그레이션, 광고주 `공유된 업무 일정` 화면을 추가했다. 업무 전용 단위검사 5/5, 서버 계약 29/29, 전체 API·서버 232/232, 플레이스 수집기 51/51, 보호 잠금 21함수·23파일·11마이그레이션, 역할·양 화면 parity, 공개 빌드 9파일·CSP와 전체 `check:quality`, `git diff --check`가 통과했다. 앱 브라우저와 Chrome이 로컬 주소를 차단해 로그인 역할별 실화면 육안 검수는 미완료이며, Production DB 마이그레이션·push·배포는 현재 승인 범위가 아니어서 진행하지 않았다.
