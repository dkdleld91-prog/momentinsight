...
- 상태: 운영 메인 홈페이지를 현재 소스 기준으로 다시 보완했다. 문서·viewport 기반, 한 줄 모바일 헤더, 샘플 데이터 표기, 카드 위계, CTA 대비, 푸터를 정돈했고 팝업 크기·고정 위치·노출 동작은 보존했다.
- 차단 사항: 없음. 전체 품질검사, 1440×900·390×844 로컬 빌드, 팝업 닫기·1주일 숨김, 문서 가로 넘침·콘솔 오류 0건을 확인했다.
- 배포 상태: 사용자 요청에 따라 로컬 작업만 완료했다. push·Vercel Production 배포는 하지 않았고 운영 사이트는 계속 승인 커밋 `368408d` 기준이다. 거절된 `0886833`은 부활시키지 않았다.
