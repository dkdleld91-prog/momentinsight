...
create table if not exists public.naver_shopping_worker_coordination (
  lane_key text primary key check (lane_key = 'global'),
  ...
  check ((lease_worker_id is null) = (lease_token is null)),
  check ((lease_worker_id is null) = (lease_until is null))
);
...
begin
  ...
  if normalized_worker_role = 'primary' then
    ...
    set primary_worker_id = normalized_worker_id,
        primary_seen_at = current_time,
        updated_at = current_time
    where lane_key = 'global';
    current_row.primary_worker_id := normalized_worker_id;
    current_row.primary_seen_at := current_time;
  end if;

  if current_row.cooldown_until is not null
    and current_row.cooldown_until > current_time then
  ...
  if normalized_worker_role = 'standby'
    ...
    and current_row.primary_seen_at > current_time - make_interval(secs => primary_stale_seconds) then
    return jsonb_build_object(
      'granted', false,
  ...
  if current_row.lease_until is not null
    and current_row.lease_until > current_time
  ...
  set lease_worker_id = normalized_worker_id,
      ...
      lease_until = current_time + make_interval(secs => lease_seconds),
      cooldown_until = null,
...
begin
  update public.naver_shopping_worker_coordination
  set lease_until = current_time + make_interval(
  ...
  where lane_key = 'global'
    ...
    and (cooldown_until is null or cooldown_until <= current_time);
...
begin
  ...
  set lease_worker_id = null,
      ...
      cooldown_until = greatest(
        coalesce(cooldown_until, current_time),
        current_time + make_interval(secs => cooldown_seconds)
      ),
