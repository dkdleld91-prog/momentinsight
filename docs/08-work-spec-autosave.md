...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | N 상품·N 플레이스 30일 핵심 기능 변경 잠금 | 다른 기능 개발이 두 30일 추적의 운영팀·광고주 UI 함수, 서버 추적·크론, 플레이스 수집, 워크플로, 순위 DB 마이그레이션을 우발적으로 바꾸면 품질·릴리스 검사를 즉시 차단. 새 순위 마이그레이션도 자동 탐지하며 명시적 승인 없는 잠금 기준 갱신·우회를 금지 | 정상 잠금 4개 함수·20개 파일·11개 마이그레이션 통과, 가상 함수 변조 차단 self-test, 전체 `check:release`: API·서버 162/162, 수집기 51/51, 역할 parity·기준선·공개 빌드·CSP·Production 인증 18/18 통과 | 배포 미승인 |
| 완료 | N 상품·플레이스 30일 전체 갱신 사이트 공통 보장 | `mml93-a01`은 운영 실측 표본일 뿐 기능 기준 계정이 아님을 코드·문서에 명확히 구분. 운영팀·광고주는 현재 세션의 광고주 범위에서 동일하게 갱신하고, 정기 크론은 계정 필터 없이 사이트 전체 대기열을 처리하는 계약을 상품·플레이스 양쪽 회귀 검사로 고정 | `6fd007d`와 실제 코드 3개 커밋 대조. 임의 `agency-b02` 범위 및 사이트 전체 크론, 상품 27/27·플레이스 46/46, 역할 parity·기준선·전체 `check:release`·Production 인증 18/18 통과 | 배포 미승인 |
| 배포 대기 | 홈페이지 서비스 안내 기능명 N 표기 통일 | 팝업의 순위 기능명 3개만 내부 메뉴와 동일한 `N 상품 순위`·`N 30일 순위`·`N 플레이스 30일 순위`로 변경. 팝업 크기·노출·닫기·1주일 숨김과 기능 로직은 보존 | 전체 `check:quality`, 기준선, Vercel 공개 빌드·CSP 검사 통과. 데스크톱 348×489px·모바일 347×489px, 가로 넘침·브라우저 경고/오류 0건, 닫기 정상 | 배포 미승인 |
