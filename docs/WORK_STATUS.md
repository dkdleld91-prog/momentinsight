...
- 상품 SEO 5대 자동 진단 보완·운영 반영 완료: 운영팀·광고주 공통 엔진을 `상품명 적합도 25점·상품정보 적합도 20점·리뷰 경쟁력 20점·등록정보 완성도 20점·순위/트래픽 15점`으로 간결하게 재구성했다. 상품명은 키워드·50자·반복/홍보 문구, 상품정보는 상위 오가닉 세부 카테고리·브랜드·제조사, 리뷰는 상위 최대 5개 평균, 등록정보는 태그 10개·항목별 고시·공개 상세 이미지 8컷을 실제 확인 근거가 있을 때만 평가한다. 가격·할인·배송·리뷰 포인트와 검색 수요·경쟁은 SEO 점수에서 제외했다.
- 검증·보호·배포 상태: SEO/공개상품 전용 29/29, 전체 API·서버 211/211, 플레이스 수집기 51/51, 서버 계약 27/27, Production 인증 18/18, 역할 parity·기준선·공개 빌드/CSP와 로컬·Vercel Production 전체 `check:release`가 통과했다. 데스크톱·390px에서 5개 결과 카드·가로 넘침 0을 육안 확인했다. 코드 `f0ccbff`·Production `dpl_FA2KBDtbHGrWYa3p2zkkv7pQVd97`을 운영 별칭에 반영했고 `/health`·`/ready`는 릴리스 `f0ccbff5ea7f`·서울 `icn1`·Supabase ready다. 비인증 세션·SEO API는 401이며 운영 관리자·광고주 파일은 로컬 검증 빌드와 SHA-256 및 바이트가 일치한다. `온열찜질기` 11위 회귀는 100점이 아니며 트래픽 보완이 최우선이다. 키워드 조회·N 상품 순위·N 상품/플레이스 30일 추적은 기존 잠금 해시를 갱신하지 않고 13함수·21파일·11마이그레이션 보호 상태를 유지한다. 외부 실상품 직접 재호출은 네이버 429로 제한됐으며 추정값 없이 공개 소스 제한으로 처리한다.
