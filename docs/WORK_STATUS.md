...
- 정확한 1~300위 수집은 전용 Mac의 독립 persistent 브라우저 프로필과 사용자가 직접 완료한 네이버 로그인 세션에서 수행한다. HMAC 서명·유효시간·1회용 nonce·lease·원자 DB commit을 강제하고 부분·광고·중복·순위 공백·collection 충돌은 전체 거부한다.
...
- 유료 외부 수집기·카드·자동 결제는 사용하지 않는다. 개인 Chrome 프로필, 비밀번호·쿠키 서버 전송, CAPTCHA 자동 우회도 허용하지 않는다.
- 코드·계약 검사는 실상품 300위 수집 성공이나 배포 증거가 아니다. 최근 `collection_id=pw-*`, `checked_count=300` snapshot과 광고 제외·연속 순번·exact/원부 일치가 아직 확인되지 않았으므로 실수집 정상화와 Production 배포는 대기 상태다.
