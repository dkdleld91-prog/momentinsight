-- Runtime 1.1.32 (2026-09-19): a finite market that repeats products across pages.
-- The first collection-evidence-v2 row (2026-09-19 00:30 KST, 일신한일의료기 탄소매트,
-- `provider_partial_window:288_300`) showed the market as data: 289 organic rows, raw ranks
-- 1..289 without a gap, 278 identities, the same seller product listed at the end of page 3
-- and one row into page 4 (and again across pages 5-6 and 6-7). Trace:
-- `p1 provider_duplicate_identity:4:1:page_overlap:3 -> stable-proof`, the same on pass 2,
-- `finite allow=true arbitration=false`, then the stable parse ended at 288/300. A finite
-- market with cross-page repeats could reach neither proof. Now the stable-proof branch hands
-- a partial stable window to the finite arbitration, finite candidates keep cross-page repeats
-- as rank slots, and the two-capture finite digest (which carries every slot) is accepted as
-- the proof of those repeats by the contract and the server, exactly as the rendered-order
-- proof is for a full window (1.1.29). No third proof may ride along.
...
-- Apply right after the 1.1.32 server release is live and the coordination row
-- is idle on 1.1.31 (the release itself stops the 1.1.31 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_32_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.31'
    ...
    raise exception 'naver_shopping_runtime_1_1_32_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.31'
       or runtime_fingerprint is distinct from
         '099bf53085a118ee3917c5f5596e72a08efe91faa3247b9091ddb27241149116'
  ) then
    raise exception 'naver_shopping_runtime_1_1_32_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.32',
      runtime_fingerprint =
        '62ad583be7a9d57b8fce77b91f810b324d57cedcbb9a120676deeb9a703681cc'
  where runtime_version = '1.1.31'
    and runtime_fingerprint = '099bf53085a118ee3917c5f5596e72a08efe91faa3247b9091ddb27241149116';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_32_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.31'
    and runtime_fingerprint = '099bf53085a118ee3917c5f5596e72a08efe91faa3247b9091ddb27241149116';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_32_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.32';
