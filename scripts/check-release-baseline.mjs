...
const checks = {
  ...
  rankTrackingCompactActionColumn: [adminSource, clientSource].every((source) => source.includes("mi-rank-track-main")
    ...
    && source.includes("mi-rank-track-meta-line")
    && source.includes("grid-template-columns: minmax(0, 1fr) max-content")
    && source.includes("grid-template-columns: 92px minmax(96px, 1fr)")
    && source.includes("grid-template-columns: minmax(116px, 0.42fr) minmax(142px, 0.52fr) minmax(0, 1.35fr)")
    && source.includes("padding-left: 120px;")
    && source.includes('<div class="mi-rank-track-meta-line">')
    && /mi-rank-row-actions \{[^}]*justify-self: end;[^}]*min-width: max-content;[^}]*padding-left: 2px;[^}]*\}/.test(source)
    && source.includes("font-size: 10px")
  ...
  rankTrackingTypographyReduced: [adminSource, clientSource].every((source) => /mi-rank-ops-row \{[\s\S]*?font-size: 11px;/.test(source)
    ...
    && /mi-rank-keyword-volume \{[\s\S]*?font-size: 11\.5px;/.test(source)
    && /mi-rank-product-info \{[\s\S]*?font-size: 11\.5px;/.test(source)
    && /mi-rank-product-title \{[\s\S]*?font-size: 11\.5px;/.test(source)
