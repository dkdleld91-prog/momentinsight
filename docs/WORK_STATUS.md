...
- 운영 홈 여백·정보 계층 재설계 로컬 완료·배포 대기: 기존 기능을 한 화면 안에 압축해 카드가 작고 빼곡해 보이던 문제를 해소하고, 사용자 요청에 따라 `운영 루틴 안내 → 이번 달 운영 현황 → 빠른 실행 → 공개 전 확인` 순서로 재배치했다. 운영 루틴은 제목 바로 아래의 전체 폭 타임라인으로 올리되 내부 여백을 줄였고, 현황은 밝은 독립 영역, 빠른 실행은 넓은 2×2 카드, 공개 전 확인은 전체 폭 2열 목록으로 유지했다. 실제 매출·보고서 상태 계산과 각 화면 이동은 그대로이며 운영 홈 밖 기능과 `client.html`, 보호된 5대 핵심 기능·DB·서버 계약은 변경하지 않았다.
- 검증·배포 상태: 로컬 운영팀 세션에서 안내가 현황·실행보다 앞서는 DOM 순서, 1440×900 데스크톱, 390×844 모바일 `scrollWidth=390`, 매출·보고서 현황 카드의 대상 화면 이동을 확인했다. 새 순서 기준선, 총관리자·연결 운영팀·단독 운영팀·광고주·해제 운영팀 5상태, 운영팀·광고주 parity, 보호 잠금 21함수·23파일·11마이그레이션, API·서버 224/224, 플레이스 51/51, 서버 계약 28/28, Production 인증 18/18과 전체 `check:release`가 통과했다. `client.html`과 보호 기능·DB·서버는 변경하지 않았으며 명시적 배포 승인 전에는 push·Production 배포하지 않는다.
