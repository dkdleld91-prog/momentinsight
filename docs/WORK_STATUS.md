...
### 2026-08-25 N쇼핑 runtime1.1.13 candidate6·run provenance (첫 자연 atomic300 반영, 24시간 관측 중)

- 8분 cadence는 단일 global lane·maxJobs1에서 장기 상한이 7.5그룹/시간이라 개선 전 8.75~8.77을 넘는 것이 구조적으로 불가능합니다. 8분 활성화는 중단했고, 같은 원자300·순서·단일 실행 계약에서 이론상 목표를 넘을 수 있는 최소 정수 cadence인 6분으로 우회안을 고정했습니다.
- Chrome alarm의 제한된 trigger를 native host가 다시 검증하고 local worker의 HMAC signed payload, server allowlist, DB progress RPC로 전달합니다. 실제 work claim 뒤 `navigating`에 진입한 run만 `naver_shopping_worker_runs`에 기록하고, 같은 run ID의 worker/trigger/runtime/fingerprint 불일치는 transaction을 예외로 되돌립니다. 표는 forced RLS·service-role select/insert only이며 update/delete 권한이 없습니다. 이는 정상 배포된 서명 워커 내부의 운영 출처 증거이지, service-role 또는 서명 주체가 침해된 경우에도 위조 불가능한 Chrome alarm 외부 증명은 아닙니다. 그 수준의 독립 증명은 현 구조에서 불가능하므로 성능 판정은 exact worker/runtime/fingerprint/trigger와 run당 group 정확히1을 함께 요구하고 누락·불일치는 성공0으로 처리합니다.
- runtime1.1.13 정확한 13파일 fingerprint는 `cde647ea615e807730cd39b5e10efb4fff5805d4b7181afc0db97315995f98f6`, service worker SHA-256은 `ffc84031a09408aaeba77249e8628ddc421902ed3d68086d0eb2b69628a424b0`입니다. candidate DB/API/Chrome은 exact 6분만 성공으로 인정하고 baseline은 exact10만 인정합니다.
- TDD는 candidate6/runtime RED와 provenance 누락 RED를 각각 확인한 뒤 GREEN했습니다. 최종 N30 집중 340/340, server contract71/71, release baseline PASS, 보호 lock 23함수·102파일·50migration 및 self-test, diff-check가 통과했고 전체 `check:release`도 core1071/place51/shopping64/Production auth18로 exit0입니다. `npm audit --omit=dev`는 기존 PPTX 경로의 `image-size` high DoS 2건으로 실패하며 N30 보안 무결점으로 과장하지 않습니다.
- migration 감사에서 coordination row만 먼저 잠근 뒤 constraint DDL이 더 강한 table lock을 기다리는 lock-order P1을 RED로 확인했습니다. `lock_timeout=5s`와 transaction 첫 단계의 `ACCESS EXCLUSIVE` table lock으로 순서를 고정해 GREEN 및 독립 재감사를 통과했습니다. 5초 안에 잠금을 못 얻으면 migration 전체가 rollback되는 fail-closed 계약입니다.
- main commit은 `4fcba2a953b6`입니다. 현재 main 전체를 Production에 올리면 개인 일정 API까지 합쳐 Vercel Hobby의 함수 12개 제한을 초과하므로 그 경로는 실패로 기록하고 재시도하지 않았습니다. 기존 Production `a0f68d3`에 N30 변경만 올린 release commit `c3e809d67844`를 Preview `dpl_BaRMgAkXSuYzRkbD3N8nyxEJzQpj`에서 Ready·함수11개로 확인한 뒤 Production `dpl_C9spGKA1VA7z15Ny2A3ZCizzNgkN`으로 승격했습니다. 운영 `/health`·`/ready`는 HTTP200, release `c3e809d67844`, live icn1·Supabase ready입니다. 개인 일정 변경은 이번 배포에 포함하지 않았습니다.
- 완전 idle에서 `20260824165332_naver_shopping_runtime_1_1_13_candidate_6_minute_cadence`를 운영 DB에 1회 적용했습니다. 실제 PostgreSQL에서 provenance 표 forced RLS, service-role SELECT/INSERT only, exact baseline10/candidate6 constraint, 관련5함수 SECURITY INVOKER·빈 search_path·service-role execute only를 확인했습니다. advisor WARN은 security2·performance23으로 적용 전 대비 증가0이고 대상 WARN0입니다.
- Windows 첫 일반 실행은 `Disable-ScheduledTask` 권한 거부로 파일 교체 전 중단됐습니다. 같은 updater SHA `fbd11aad98c5b4c9567e171652187186dc266c5f5c4de0ac635c6c772c19345e`를 관리자 권한으로 1회 재실행해 `N30_ELEVATED_EXIT=0`과 `MI_EXTENSION_UPDATE_OK`, release `c3e809d67844`, version1.1.13, syntax13, loaded extension/native registry 동기화, exact fingerprint를 확인했습니다.
- 첫 자연 provenance run `c9e710c8-5fe8-4ad8-a25b-267154459334`은 exact worker/runtime/fingerprint와 `run_trigger=rank-catch-up`으로 기록됐습니다. event9977 group →9978 tracker →9979 commit, collection `pw-chrome-1787594315521-45c224f25432e87ddfc1` snapshot1은 checked300·official collector·organic policy/evidence·adExcluded=true·excluded30·top5 organic/non-ad 위반0이고 matched=false·rank null입니다. 이 성공의 DB anchor는 `2026-08-24T17:58:36.220664Z`입니다.
- 잔여 partial 대상 `1114f3af-c30c-4975-9b79-ecec9cfbf031`은 자연 만료·순번에서 run `34ae471b-0f9a-4d4a-a9ad-0883ed24cf3a`로 재진입해 event9993 group →9994 tracker →9995 `provider_partial_window:30_300` →9996 quarantine로 유한 종료됐습니다. post-claim snapshot은0이고 기존 snapshot 수58·`last_checked_at=2026-07-31T00:19:49.58Z`·rank null·check_count58·found_count0은 그대로이며, 다음 격리는 `2026-08-25T18:29:09.508685Z`입니다. 부분 결과를 성공이나 새 atomic300으로 저장하지 않은 fail-closed 증거입니다.
- 뒤이은 자연 grouped run `28ce3a7a-072d-4fde-9ada-cdbcb290cd2c`은 event10077 group(member2) →10078·10079 tracker →10080·10081 commit으로 끝났습니다. shared collection `pw-chrome-1787597316296-b8e372f9400fb925490c`에서 commit2·valid atomic snapshot2·collection1이고 두 snapshot 모두 checked300·official collector·organic/adExcluded입니다. `2026-08-24T18:55:01.913113Z` 집계는 provenance6(catch-up5·remote1), same-cycle 중복0·실행 겹침0이며 운영은 streak5·anchor 유지·baseline10·candidate false, heartbeat fresh, closed/null·processing0·lane/run/lease/stage/job/tracker/probe 해제입니다. 가장 이른 candidate 판정은 `2026-08-25T17:58:36.220664Z` 이후이고 성공6회도 아직 미충족입니다. candidate setter와 wake/cursor/order/quarantine/lease 조작은0회입니다.
- `2026-08-24T19:26:48.081245Z` 후속 SELECT-only 감사에서 provenance10행은 catch-up8·remote2·exact identity mismatch0입니다. 새 catch-up 3회는 group당1·fully terminal이고 claim 간격이 각각 600초이며, 중간 remote1회는 별도 분리했습니다. event10084 이후 commit6/6은 snapshot 정확1·checked300·official collector·organic policy/evidence·adExcluded·top-item non-ad/organic 조건을 모두 충족했고 invalid0, open/incomplete0, same-cycle 중복0, 실행 겹침0입니다. 운영은 streak9로 성공6회 조건을 충족했지만 anchor는 그대로이고 24시간 조건이 false라 candidate_eligible=false입니다. baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle이며 candidate setter와 상태 조작은 계속0회입니다.
- `2026-08-24T19:55:58.933147Z`까지 추가 catch-up 3회는 각 group1·tracker1·commit1·failure0으로 종료됐고 commit/snapshot3/3이 같은 atomic300 조건을 통과했습니다. claim 간격은 계속 600초, open/incomplete·same-cycle 중복·overlap0입니다. 누적 provenance13(catch-up11·remote2)·identity mismatch0, streak12이며 anchor·baseline10·candidate false·완전 idle은 유지됩니다. 24시간 gate가 유일하게 남아 candidate6는 호출하지 않았습니다.
- `2026-08-24T20:26:17.449412Z`까지 catch-up3·remote1이 추가됐습니다. 네 run은 각 group1·fully terminal이고 commit/snapshot5/5가 atomic300 조건을 통과했으며 invalid/open/incomplete·same-cycle 중복·overlap0입니다. catch-up 간격은 600초이고 네 cursor tuple 모두 strict-forward입니다. 누적 provenance17(catch-up14·remote3)·identity mismatch0, streak16이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle은 유지됩니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T20:56:17.731412Z`까지 catch-up 3회가 더 group1·tracker1·commit1·failure0으로 종료됐습니다. commit/snapshot3/3 valid atomic300, gap600·600·600초, cursor strict-forward3/3이고 invalid/open/incomplete·same-cycle 중복·overlap0입니다. 누적 provenance20(catch-up17·remote3)·identity mismatch0, streak19이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T21:26:44.446527Z`까지 catch-up3·remote1이 추가됐습니다. 네 run은 group1씩 fully terminal이고 tracker/commit8·failure0, matching snapshot8/8 valid atomic300입니다. catch-up gap600·600·600초·cursor strict-forward4/4, invalid/open/incomplete·same-cycle 중복·overlap0입니다. 누적 provenance24(catch-up20·remote4)·identity mismatch0, streak23이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T21:56:50.815466Z`까지 catch-up3이 더 group1씩 fully terminal됐습니다. tracker/commit7·failure0, matching snapshot7/7 valid atomic300, gap600·601·600초, cursor strict-forward3/3이고 invalid/open/incomplete·same-cycle 중복·overlap0입니다. 누적 provenance27(catch-up23·remote4)·identity mismatch0, streak26이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T22:27:14.864925Z`까지 catch-up3·remote1이 추가됐고 네 run은 group1씩 fully terminal, tracker/commit5·failure0, matching snapshot5/5 valid atomic300입니다. 동시대 remote가 사이에 있는 catch-up gap은 645·554·600초로 관측됐으며 remote는 성능 분자에서 분리합니다. cursor strict-forward4/4, invalid/open/incomplete·same-cycle 중복·overlap0입니다. 누적 provenance31(catch-up26·remote5)·identity mismatch0, streak30이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T23:00:58.818677Z`까지 catch-up4·remote1이 추가됐습니다. 첫 감사 때 진행 중이던 remote와 직후 catch-up도 자연 terminal까지 기다려 최종 run5·commit/snapshot6/6 valid atomic300·open0을 확인했습니다. catch-up gap600·600·643초 중 마지막 구간에는 동시대 remote가 있고, cursor strict-forward5/5·invalid·same-cycle 중복·overlap0입니다. 누적 provenance36(catch-up30·remote6)·identity mismatch0, streak35이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T23:27:39.481349Z`까지 catch-up2가 group/tracker/commit 각2·failure0으로 종료됐습니다. matching snapshot2/2 valid atomic300, gap600초, cursor strict-forward2/2이고 invalid/open·same-cycle 중복·overlap0입니다. 누적 provenance38(catch-up32·remote6)·identity mismatch0, streak37이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-24T23:58:53.748785Z`까지 catch-up4·remote1이 추가됐습니다. 최초 감사 때 진행 중이던 마지막 catch-up도 자연 terminal·lane 해제까지 확인해 최종 run5·tracker/commit5·failure0, matching snapshot5/5 valid atomic300·open0입니다. catch-up gap600·600·600초, cursor strict-forward5/5, invalid·same-cycle 중복·overlap0입니다. 누적 provenance43(catch-up36·remote7)·identity mismatch0, streak42이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-25T00:28:58.442450Z`까지 09:00 KST 정시 run1과 catch-up3이 각 group/tracker/commit1·failure0으로 종료됐습니다. matching snapshot4/4 valid atomic300, catch-up gap600·600초, cursor strict-forward4/4이고 invalid/open·same-cycle 중복·overlap0입니다. provenance47은 catch-up39·remote7·rank-0900 1이며 정시 run은 향후 candidate 성능 분자에서 별도 제외합니다. streak46·anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했고, 정시 enqueue로 pending70인 것은 처리 중 누수가 아닙니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-25T01:03:53.420824Z`까지 remote2·catch-up3이 추가됐습니다. 다섯 run은 group/tracker/commit 각5·failure0, matching snapshot5/5 valid atomic300이며 catch-up gap599·601·600초, cursor strict-forward5/5, invalid/open·same-cycle 중복·overlap0입니다. 누적 provenance52(catch-up42·remote9·rank-0900 1)·identity mismatch0, streak51이고 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. remote9와 rank-0900 1은 향후 성능 분자에서 제외하며 24시간 gate 전이라 candidate6는 호출하지 않았습니다.
- `2026-08-25T02:02:24.809274Z`까지 remote2·catch-up6이 추가됐습니다. 여덟 run은 group8·tracker/commit10·failure0, matching snapshot10/10 valid atomic300이고 cursor 위반·open·same-cycle 중복·overlap0입니다. catch-up gap은 600초 4구간과 remote가 사이에 있는 648·552초로 분리됩니다. 누적 provenance60(catch-up48·remote11·rank-0900 1)·identity mismatch0, streak59이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-25T02:32:57.490657Z`까지 remote2·catch-up2가 추가됐습니다. 기존 잔여 partial `12f5330a…`는 자연 만료·cycle37 마지막 순번에서 `provider_partial_window:137_300`으로 유한 종료됐고 post-claim snapshot0, 기존 snapshot71·last_checked/rank/check_count/found_count를 보존한 채 `2026-08-26T02:10:35.927243Z`까지 격리됐습니다. cycle37은 group58=distinct58·tracker74=distinct74·commit73·failure1로 자연 완료됐고 10분 뒤 cycle38이 시작돼 첫 grouped run을 commit/snapshot2/2 valid atomic300으로 마쳤습니다. 이번 run4·group4·tracker5는 commit4·typed failure1·open/overlap0이고 성공 snapshot4/4 invalid0입니다. 누적 provenance64(catch-up50·remote13·rank-0900 1)·identity mismatch0, streak59→remote2 뒤61→격리 실패에서 보존→후속 성공62이며 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. 이는 전역 비활성화가 아니라 tracker 단위 fail-closed 격리입니다.
- `2026-08-25T03:46:47.965329Z`까지 cycle38에서 catch-up7·remote1이 추가됐습니다. run8·group8·tracker/commit10·failure0, matching snapshot10/10 valid atomic300이고 cursor 위반·open·same-cycle 중복·overlap0입니다. catch-up gap은 601·599·600·600·600·600·600초이며 remote는 별도 분리됩니다. 누적 provenance72(catch-up57·remote14·rank-0900 1)·identity mismatch0, streak70이고 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle을 유지했습니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-25T03:59:26.931799Z` SELECT-only 재감사에서 catch-up `337462b5…`·`e242e848…`와 remote `dd22786f…`가 추가로 terminal됐습니다. 세 run은 group3·tracker/commit4·failure0이며 matching snapshot4/4가 checked300·official collector·organic-only evidence·adExcluded=true·excluded45·top5 organic/non-ad 조건을 통과했습니다. cursor tuple은 세 번 모두 strict-forward이고 실행 overlap0, cycle38 누적 group12=distinct claim/fingerprint12·tracker16=distinct16·commit16·failure/open/중복0입니다. 누적 provenance74(catch-up58·remote15·rank-0900 1)·identity mismatch0, streak73이고 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. 24시간 gate 전이므로 candidate6는 호출하지 않았습니다.
- `2026-08-25T04:12:19.503480Z`까지 catch-up `0f8f27bb…`가 group/tracker/commit 각1·failure/open0으로 추가 terminal됐고 matching snapshot1/1은 checked300·official collector·organic-only evidence·adExcluded=true·excluded6·top5 organic/non-ad 조건을 통과했습니다. cursor tuple도 strict-forward이며 cycle38 누적 group13=distinct claim/fingerprint13·tracker17=distinct17·commit17·failure/open/중복0입니다. 누적 provenance75(catch-up59·remote15·rank-0900 1)·identity mismatch0, streak74이고 anchor·baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다.
- `2026-08-25T04:19:38.180199Z`까지 다음 자연 catch-up `8e5cfb4f…`가 event10499 group →10500 tracker →10501 commit으로 유한 종료됐습니다. collection `pw-chrome-1787631516118-3e527c89579336ecc8d1`의 matching snapshot1은 checked300·official collector·organic-only evidence·adExcluded=true·excluded60·collection 일치·top-item 위반0이며 rank1입니다. cursor는 strict-forward이고 cycle38 누적 group14=distinct claim/fingerprint14·tracker18=distinct claim/tracker18·commit18·failure/open/atomic invalid/중복0, anchor 이후 실행 overlap0입니다. 누적 provenance76(catch-up60·remote15·rank-0900 1)·identity mismatch0·run당 group 위반0, streak75이고 baseline10·candidate false·fresh heartbeat·closed/null·processing0·lane/run/lease/stage/job/tracker/probe 완전 idle입니다. anchor+24시간 전이므로 setter는 호출하지 않았습니다.
- `2026-08-25T04:24:02.817585Z`까지 remote `bc87c1bc…`도 event10503 group →10504 tracker →10505 commit으로 유한 종료됐습니다. collection `pw-chrome-1787631692481-adf9543e7d97efc659d7` snapshot1은 checked300·official collector·organic-only evidence·adExcluded=true·excluded45·collection 일치·top-item 위반0·rank95이고 cursor sort300→400 strict-forward입니다. cycle38 누적 group15=distinct claim/fingerprint15·tracker19=distinct tracker19·commit19·failure/open/중복0, 누적 provenance77(catch-up60·remote16·rank-0900 1)·identity mismatch0, streak76이며 baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. remote는 향후 candidate 처리량 분자에서 제외하고 setter는 호출하지 않았습니다.
- `2026-08-25T04:30:28.258261Z`까지 다음 자연 catch-up `e59a2f09…`를 진행 중 page4에서 성공으로 선보고하지 않고 terminal까지 관측했습니다. event10508 group(member2) →10509·10510 tracker →10511·10512 commit, shared collection `pw-chrome-1787632158902-78ad9a91c1d8ed4fd567`의 snapshot2/2는 checked300·official collector·organic-only evidence·adExcluded=true·excluded30·collection 일치·top-item 위반0이며 rank237/미발견입니다. cursor는 동일 sort400 안에서 created_at 순방향으로 strict-forward입니다. cycle38 누적 group16=distinct claim/fingerprint16·tracker21=distinct tracker21·commit21·failure/open/중복0, provenance78(catch-up61·remote16·rank-0900 1)·identity mismatch0, streak77이고 baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. anchor+24시간 전이라 setter는 호출하지 않았습니다.
- 고정 checkpoint `2026-08-25T04:36:17.584413Z`의 Production read-only `n30_baseline10_fixed_wall_v1`은 anchor부터 wall `38261.363749초` 동안 provenance group78(catch-up61·remote16·rank-0900 1), missing/identity mismatch0을 재현했습니다. exact catch-up61 중 known residual partial failure2를 제외한 fully-terminal atomic300 group59, bootstrap 제외58이므로 fixed-wall 처리량은 `5.457202 group/hour`입니다. run당 group·tracker/commit 중복·atomic invalid·open/incomplete·overlap은0이고 600초 배수±120초 grid 위반도0입니다. 이는 candidate6 전 baseline 참고값일 뿐 속도 향상 증거가 아니며, remote·정시 run과 partial failure는 분자에서 제외했습니다.
- `2026-08-25T04:42:26.246630Z` 읽기 전용 재감사에서 다음 자연 catch-up `f9a01a99…`이 event10517 group(member4) →10518~10521 tracker4 →10522~10525 commit4로 terminal됐습니다. shared collection `pw-chrome-1787632717073-cb774775483104d5ba2e` snapshot4/4는 checked300·official collector·organic-only evidence·adExcluded=true·excluded45·top-item 위반0이고 ranks6/46/8/25입니다. cursor는 이전 sort400 `74fb18e0…`의 created_at `2026-07-08`에서 현재 sort400 `60864aa5…`의 `2026-07-10`으로 strict-forward입니다. cycle38 누적 group17=distinct claim/fingerprint17·tracker25=distinct25·commit25·failure/open/중복0, provenance79(catch-up62·remote16·rank-0900 1)·identity mismatch0, streak78이며 baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. gate 전이라 setter는 호출하지 않았습니다.
- `2026-08-25T04:49:38.782193Z`에는 catch-up `55dc17bc…`을 page5 진행 중 상태에서 성공으로 세지 않고 event10527 group →10528 tracker →10529 commit terminal까지 확인했습니다. collection `pw-chrome-1787633318002-4b336137ade998308749` snapshot1은 checked300·official collector·organic-only evidence·adExcluded=true·excluded45·top-items5/위반0·rank157입니다. cursor는 sort400 `60864aa5…`에서 sort500 `6bfd9d69…`로 strict-forward이고 두 tracker의 processing/quarantine는 null입니다. cycle38 누적 group18=distinct claim/fingerprint18·tracker26=distinct26·commit26·failure/open/중복0, provenance80(catch-up63·remote16·rank-0900 1)·identity mismatch0, streak79이며 terminal 뒤 baseline10·candidate false·fresh heartbeat·closed/null·processing0·완전 idle입니다. gate 전이라 setter는 호출하지 않았습니다.
- candidate 전환 뒤 판정을 반복 가능한 단일 계약으로 고정하기 위해 파라미터형 `BEGIN READ ONLY` 성능 감사 SQL 생성기를 추가했습니다. activation/observed 시각과 cadence360/600만 가변이며 worker/runtime/fingerprint는 현재 배포 exact identity로 고정해 다른 정상 형식 값도 거부합니다. fixed-wall 120분·post-bootstrap18·`>8.77`과 provenance, 모든 exact run의 group1·claim/commit tracker 집합·event 순서·atomic300·failure0·중복·grid·overlap·control-plane idle을 한 결과로 반환하며 setter·DML·mutable 현재시각은 포함하지 않습니다. cycle identity 누락, 같은 cycle의 두 번째 normal/resume 이후 cursor NULL reset, malformed cursor, coordination `updated_at > observedAt`, 비배열 `top_items`도 각각 fail-closed합니다. Node test 4/4와 line/branch/function coverage 100%를 통과했고 Production 고정 baseline checkpoint도 cycle identity missing0·cursor evidence invalid0·wall38261.363749초·post-bootstrap58·5.457202 group/hour를 동일하게 반환했습니다.
- cursor 증거는 기존 ledger 한계까지 보수적으로 판정합니다. 같은 claim의 tracker 중 하나라도 cursor 이하이거나 sort 증거가 잘못되면 `cursor_nonforward_or_fallback_count`로 세므로 정상 grouped sibling이나 cycle-tail fallback도 성공을 막을 수 있습니다. baseline 값2는 알려진 partial `1114…`·`12f5…`가 cycle 끝 fallback으로 sort3700 cursor 뒤에서 각각 sort600·100으로 자연 선택된 두 typed failure와 정확히 일치합니다. 현재 ledger에는 `seedTrackerId`·`selectionMode(strict|fallback)`이 없어 실제 seed 순서 위반과 합법 fallback을 완전히 구분하는 것은 불가능합니다. candidate gate는 이 보수 지표도0일 때만 통과시키므로 false-negative 가능성은 명시하되 불확실한 순서를 성공으로 과장하지 않습니다.
- 전체 `npm test`에 새 감사 test를 포함해 실제 실행했으며 새 test 4/4를 포함한 총 1075개 중 1074개가 통과했습니다. 유일한 실패는 이 변경과 무관한 기존 `google-calendar-api.test.mjs`의 자동 syncToken 기대값으로, 현재 committed HEAD에서도 단독 재현됩니다. 보호 범위인 개인 일정 코드는 수정하지 않았고 전체 suite PASS로 보고하지 않습니다. N30 생성기 집중 test·coverage·보호 lock과 Production read-only compile은 모두 통과했습니다.
- `2026-08-25T05:19:03.127222Z` 운영 SELECT-only는 runtime/fingerprint/worker exact, heartbeat18.21초, last atomic27.21초, streak83, anchor 유지, baseline10·candidate false, circuit closed·reason/cooldown null, processing0·lane/run/lease/stage/job/tracker/probe 완전 idle입니다. candidate_eligible=false이고 setter·wake·cursor·order·quarantine·lease DML은 호출하지 않았습니다. 알려진 residual은 자연 시각과 순번을 기다리며, 가장 늦은 `12f5…`의 격리 만료·자연 terminal을 확인하기 전에는 120분 failure0 창을 시작하지 않습니다.
- 현재 HEAD에서 N30 관련 6개 test file을 다시 실행해 exit0, server contract71/71, release baseline PASS, 보호 lock 23함수·102파일·50migration PASS를 확인했습니다. 이어 Production `BEGIN READ ONLY`에서 candidate6·baseline canonical setter 문장과 coordination row-lock·최종 exact 판정식을 `EXPLAIN/WHERE false`로 compile했습니다. marker `n30_candidate6_transaction_template_dry_run_v1`은 `setter_called=false`, fixture raw exact candidate/6=true, 실제 post baseline10·processing0·idle=true, transition=false를 반환해 실제 setter DML이 없음을 확인했습니다.
- 전환 당일 수작업 SQL 차이를 없애기 위해 canonical candidate6·baseline10 단일 transaction SQL 생성기를 TDD로 고정했습니다. 후보 모드는 직전 SELECT 감사에서 얻은 exact run UUID·collection ID·last-success 시각을 필수 입력으로 받고, coordination lock 안에서 동일 provenance와 group1·claim/commit 집합·failure0·snapshot checked300·official source·organic-only/adExcluded·top-item non-ad까지 다시 검증합니다. raw exact 응답, updated-at 범위, pre/post cadence, processing0·lane/run/lease 완전 idle 중 하나라도 다르면 exception으로 setter DML 전체를 rollback합니다. `SET LOCAL ROLE service_role`, 5개 GUC의 transaction-local 저장·순서, setter 직후 raw 원형 보존, postflight/final 판정식은 section 단위 회귀로 고정했습니다.
- 첫 Production read-only compile은 SQL 특수식 `coalesce`를 `pg_catalog.coalesce` 함수처럼 호출한 실제 오류를 반환해 성공으로 보고하지 않았습니다. 회귀를 RED로 추가한 뒤 unqualified SQL `coalesce`로 수정했고 candidate/baseline compile probe가 통과했습니다. 최신 exact `rank-1500` run `23402b87…`·collection `pw-chrome-1787637643954-6835c570390aab84d495`로 hard-gate를 재현하면서 ledger `excluded_ad_count`가 NULL이고 provenance `started_at`이 group claim보다 114ms 늦는 현재 계약도 확인했습니다. 성능 감사와 동일하게 snapshot의 nonnegative `excludedAdCount`를 검증하고 provenance 기록 지연은 5초 이내로 제한한 뒤 `atomicProofOk=true`를 얻었습니다. 그 사이 coordination의 latest proof가 다음 collection으로 바뀌어 전체 `eligible=false`가 유지됐으므로 stale proof는 fail-closed합니다.
- cross-execution exactly-once는 현재 DB에 attempt ledger가 없어 생성 SQL 하나만으로 보장할 수 없습니다. 따라서 생성기는 transaction당 setter 호출1회만 보장하고, 실제 실행은 요청 전에 외부 `attempted=true`를 영구 기록하며 timeout·거부·응답 유실·committed-unknown을 포함해 어떤 결과도 재호출하지 않는 계약을 결과에 명시합니다. 이 한계를 성공으로 포장하지 않으며 아직 attempted=false·setter 호출0회입니다.
- 독립 적대 감사에서 최초 전환 proof가 같은 claim의 다른/NULL run terminal, tracker별 snapshot 중복, 비배열 `top_items` 평가 순서를 놓치는 P1 3건을 확인해 커밋을 보류했습니다. claim 전체 tracker/commit/failure와 run→claim orphan을 양방향으로 거부하고, claim tracker=commit tracker=collection 전체 snapshot tracker 단일 집합을 요구하며, `excludedAdCount` cast와 `jsonb_array_elements`를 CASE로 보호했습니다. 수정 후 독립 재감사는 generator/test P0/P1 blocker0, Production `BEGIN READ ONLY` 재컴파일도 통과했습니다.
- `2026-08-25T06:17:10.583227Z` candidate read-only probe는 exact run `29a885d2-5e5e-46cb-adfb-930b0df8bcf4`·collection `pw-chrome-1787638209849-ee88bd2aa494efdd2569`·last-success `06:10:10.547919Z`에서 강화된 claim/run/collection 전체 proof가 `atomicProofOk=true`임을 반환했습니다. anchor24h가 아직 false라 `eligible=false`, 실제 post는 baseline10이라 `transitionAccepted=false`입니다. candidate/baseline 각각 setter 참조1개는 `EXPLAIN ... WHERE false`뿐이고 실행 대입은 fixture라 setter 호출·cadence 변경은0회입니다.
- 최종 scoped 검증은 전환+성능 감사 10/10, 전환 생성기 자체 line/branch/function 100%, baseline PASS, server contract71/71, 보호 lock 23함수·102파일·50migration, syntax·diff-check PASS입니다. 전체 `npm test`는 새 전환 회귀6/6를 포함해 1080/1081이며, 유일한 실패는 보호 대상 개인 일정의 기존 syncToken 기대/실제 timeMin·timeMax 불일치입니다. 개인 일정 코드를 건드리지 않았고 전체 PASS로 보고하지 않습니다. read-only probe 뒤 운영은 baseline10·candidate false이고 setter DML은0회입니다.
- 현재 운영 proof는 배포 뒤 baseline10에서 첫 자연 official atomic300으로 생성된 anchor `2026-08-24T17:58:36.220664Z`를 기준으로 진행 중입니다. anchor+24시간·성공6회와 모든 exact/idle gate를 충족해야 candidate6를 총1회 전환하며, 이후 120분 이상·post-bootstrap `rank-catch-up` 18그룹 이상의 fixed-wall 실측이 8.77그룹/시간을 초과할 때만 속도 향상 완료입니다. 불확실하거나 미달이면 baseline10 1회 복귀·성공0입니다.
...
- 두 번째 post-migration run `063af0d5-b542-4476-a0ea-837684072bd4`는 event9872 group →9873 tracker →9874 commit으로 끝났습니다. collection `pw-chrome-1787581438278-412a5f97707022515ca5` snapshot1은 checked300·official·collection 일치·organic target/policy/evidence·adExcluded·광고30건 제외·top5 위반0·rank8이고 streak는 63→64로 정확히 1회 증가했습니다. mutable wake는 `rank-cron-cycle` requested/consumed `14:22:26.677Z`·updated `14:22:32.338Z`, claim은 `14:22:33.113Z`라 별도 remote-wake handoff의 강한 증거입니다. append-only ledger에는 trigger가 없어 exact 원인으로 단정하지 않으며 향후 candidate8 scheduled-compatible 성능 표본에서는 제외합니다. `14:24:56Z` cycle36 group30=distinct30·fingerprint 중복0, tracker43=distinct43·중복0·commit43·failure0이고 closed/null·processing0·lane/run/lease 해제·baseline10·candidate false입니다.
- 다음 정시-compatible run `2cf2087c-8470-4845-b37e-0050b1c38d89`는 event9877 group(member2) →9878·9879 tracker2 →9880·9881 commit2로 끝났습니다. 앞 정시 event9868과 claim 간격은 `600.211초`이고 마지막 mutable wake는 5분 전 remote handoff 값이라 이 회차에는 새 wake 증거가 없습니다. shared collection `pw-chrome-1787581746657-9bbf7a0f007c09cadc2e` snapshot2/2는 checked300·official·organic target/policy/evidence·adExcluded·광고30건 제외·top5 위반0, ranks1/22입니다. streak는 tracker 수와 무관하게 group당 1회인 64→65이고 cursor tuple은 sort1200에서 created_at `2026-06-26T14:51:22.956194Z`→`2026-07-10T06:04:35.817547Z`로 strict forward입니다. `14:29:42Z` cycle36 group31=distinct31·fingerprint 중복0, tracker45=distinct45·중복0·commit45·failure0, processing0·closed/null·lane/run/lease/stage/job/tracker/probe 해제·baseline10·candidate false입니다.
- cycle36의 실제 roster는 tracker76·distinct fingerprint group60이며 시작 시 eligible73·quarantined3입니다. `14:34:21Z` 기준 claim31그룹 뒤 roster 잔여는 29그룹/31 tracker이고 현재 eligible26·ineligible3입니다. 잔여 ineligible은 이미 typed failure를 실증한 `c0cc…`(94/300, 격리 `2026-08-25T07:29:00.724542Z`)와 관측 대기 중인 `1114…`(30/300, sort600, `2026-08-24T15:33:02.119249Z`)·`12f5…`(138/300, sort100, `2026-08-24T19:18:58.276397Z`)입니다. 두 잔여 대상은 각각 event9676·9674 `roster_state=quarantined`로 cycle에 포함됐지만 claim/terminal은 아직0입니다. 선택 함수는 격리 만료 전 대상은 제외하고, cursor 뒤 seed가 없으면 eligible 미claim을 처음부터 다시 고르므로 자연 만료·fallback 또는 다음 cycle의 실제 event만 회복/typed failure 증거로 인정합니다. 완료 예상시각은 보장하지 않고 wake/cursor/order/quarantine를 조작하지 않습니다.
- 후속 정시-compatible run `c1c78e1d-0324-49a4-b7b8-361c8bfcfdb8`은 event9883 group →9884 tracker →9885 commit으로 끝났고 직전 정시 event9877과 claim 간격은 `599.934초`입니다. collection `pw-chrome-1787582304839-fb6f9d1e0cf8bd5678a7` snapshot1은 checked300·official·collection 일치·organic target/policy/evidence·adExcluded·광고45건 제외·top 위반0·rank8입니다. streak65→66, cursor sort1200→1300 순방향이며 cycle36 group32=distinct32·중복0, tracker46=distinct46·중복0·commit46·failure0입니다. `14:39:03Z` processing0·closed/null·lane/run/lease/stage/job/tracker/probe 해제·baseline10·candidate false이고 이 회차도 candidate8 성능으로 사용하지 않습니다.
- `14:42:14Z` post-migration 전체 상호검증은 group4=distinct claim/run/fingerprint4, tracker claim5·commit5·distinct snapshot5, fully complete group4/4·incomplete0·failure0·원자 proof 위반0입니다. grouped run은 claim2=commit2·collection1로 정확히 집계되며 배포 직전 streak62→현재66 증가량4가 group4와 일치합니다. 모든 collection은 claim별1개이고 terminal durations는 41.688·85.165·87.510·45.758초입니다. closed/null·processing0·lane/run/lease 해제·baseline10·candidate false를 함께 확인했습니다.
- candidate 전환의 canonical authority는 service-role/postgres-only `public.mi_set_naver_shopping_worker_cadence('candidate')`이고 `/api/naver-rank-trackers`의 owner-only `worker-cadence`는 이 함수를 호출하는 세션 어댑터입니다. 자동 관측은 owner 세션·접속 코드를 만들거나 우회하지 않고, gate 시각에 Supabase 제어 경로의 단일 DB transaction에서 canonical setter를 정확히 1회만 실행합니다. 호출 직전 requestStartUtc와 응답 직후 responseEndUtc를 남기며, transaction-local startedAt/raw와 같은 global row lock 안의 cadence/updated_at/idle 최종 resultset을 함께 캡처합니다. timeout·응답 유실이면 DB가 이미 commit됐을 수 있으므로 재호출0, 상태 SELECT만 수행하고 raw 결과 없이 성공으로 판정하지 않습니다.
...
- 다섯 번째 연속 정시-compatible run `8f0e1e77-f0ec-4bae-8dff-f403ca435450`은 event9895 group →9896 tracker →9897 commit으로 끝났습니다. 직전 event9891과 claim 간격은 `600.165초`, claim→terminal은 `44.632초`이고 wake row는 계속 불변입니다. collection `pw-chrome-1787584103628-9e58d3d5b78c0890a076` snapshot1은 checked300·official·collection 일치·matched=false·organic policy/evidence·adExcluded·광고30건 제외·top 위반0입니다. streak68→69, cursor tuple은 sort1500의 created_at `2026-07-08T05:01:48.647378Z`→`2026-08-14T06:49:16.159237Z` strict-forward입니다. cycle36 group35=distinct35·중복0, tracker49=distinct49·중복0·commit49·failure0이며 잔여25그룹/27트래커 중 collectable22·격리3입니다. terminal 뒤 processing0·closed/null·lane/run/lease/stage/job/tracker/probe 해제·baseline10·candidate false입니다.
- 여섯 번째 정시-compatible run `672dfe20-546b-46d7-b1bc-2936ca97754e`은 event9899→9901로 끝났고 직전 정시 event9895와 claim 간격은 `599.857초`, claim→terminal은 `41.420초`입니다. collection `pw-chrome-1787584700273-0084b4fe13bab7d133cd` snapshot1은 checked300·official·collection 일치·matched=false·organic policy/evidence·adExcluded·광고60건 제외·top-item 위반0입니다. streak69→70, cursor sort1500→1600 strict-forward이고 cycle36 group36=distinct36·tracker50=distinct50·commit50·failure/open0, terminal 뒤 processing0·closed/null·lane/run/lease 해제·baseline10·candidate false입니다.
- event9903→9905 run `a35559bc-a092-40f7-b738-fe2edbbeba98`은 직전 정시 claim 뒤 `113.291초`에 시작됐습니다. mutable wake `rank-cron-cycle`이 `15:19:26.336Z` 요청·소비되고 `15:19:31.793Z` 갱신된 뒤 `15:19:32.144Z` claim돼 remote-wake handoff의 동시대 강한 증거입니다. collection `pw-chrome-1787584814418-c0f724fea4e59720fc46` snapshot1은 checked300·official·organic/adExcluded·광고45건 제외·top-item 위반0·rank18이고 streak70→71, cycle group37/tracker51/commit51·중복/실패0, cursor sort1600→1700, lane 해제를 확인했습니다. 정상 atomic 성공이지만 정시 성능 분자에서는 제외합니다.
- 일곱 번째 정시-compatible run `e0aca251-65fb-4dc2-bef5-837e8c0ee87c`은 event9907→9909로 끝났고 앞 정시 event9899와 claim 간격은 `600.646초`, claim→terminal은 `42.207초`입니다. collection `pw-chrome-1787585301706-f9c593760f16ff31b7f4` snapshot1은 checked300·official·collection 일치·matched=false·organic policy/evidence·adExcluded·광고45건 제외·top-item 위반0입니다. streak71→72, cursor sort1700 유지 내 created_at 순방향이고 cycle36 group38=distinct38·tracker52=distinct52·commit52·failure/open0입니다. `15:30:19Z` exact runtime/fingerprint·baseline10·candidate false, processing0·closed/null·lane/run/lease/stage/job/tracker/probe 해제를 재확인했습니다.
- 성능 판정 SQL은 activation부터 고정 wall time의 group을 claim별 correlated subquery로 집계해 grouped tracker×commit 조인 증폭을 제거하고, 각 tracker의 terminal 정확1·atomic snapshot·cursor·cycle 중복을 별도 검증하도록 독립 실행했습니다. baseline 재현 고정시각 `15:26:23.815705Z`에서 wall4220.816초, raw group9/fully-terminal9, 동시대 remote/early 2건 제외 뒤 scheduled-compatible7(bootstrap 뒤6), atomic/failure/cursor 위반0입니다. scheduled fixed-wall은 `5.970410 group/hour`, remote 포함 raw는 `7.676241 group/hour`이며 모두 candidate 전 수치입니다. activation은 setter와 같은 transaction의 transaction-local startedAt로 캡처한 exact DB timestamp만 쓰고 계속 바뀌는 현재 coordination.updated_at은 금지하며, mutable wake의 과거 attribution을 동시대 claim ID로 보존하지 못하면 성능 판정을 fail-closed합니다.
- `maxJobs=1`에서 8분 정시 cadence의 장기 산술 상한은 `7.5 group/hour`이고 첫 60분 bootstrap을 포함해도 최대8개이므로, remote wake를 제외한 현재 계약으로 기존 혼합 기준 `8.77 group/hour` 초과는 구조적으로 불가능합니다. candidate8은 순수 baseline10 정시 상한6보다 빠를 가능성은 있지만 합의한 `>8.77` 성공 기준으로는 향상이라고 보고할 수 없습니다. gate 뒤 실험을 하더라도 최소60분·post-bootstrap6회 실측이 기준 미달이면 baseline10으로 복귀하고 성공0으로 기록합니다.
- partial `1114f3af…`는 `15:33:02.119249Z`에 자연 격리가 만료돼 eligible/pending이 됐지만 current cycle claim/terminal은 계속0입니다. 만료 뒤 첫 정시 run `2b2a41af-6dbf-4543-af6a-5f9108e3d063`은 selector가 cursor 뒤 sort1800을 정상 우선해 event9911→9913으로 종료됐습니다. 앞 정시 event9907과 claim 간격 `599.444초`, claim→terminal `84.320초`이고 collection `pw-chrome-1787585943263-2fe23af7a8e89102fd49` snapshot1은 checked300·official·collection/organic/adExcluded 일치·광고45건 제외·top-item 위반0입니다. streak72→73, cursor sort1700→1800 strict-forward, cycle36 group39=distinct39·tracker53=distinct53·commit53·failure/open0, lane/run/lease 해제입니다. target을 앞당기거나 순서를 바꾸지 않고 실제 fallback/다음-cycle terminal을 계속 기다립니다.
- 다음 자연 정시 run `bd734f33-2f45-4d70-9c7d-852159d1bd08`도 event9915→9917로 종료됐습니다. 앞 정시 event9911과 claim 간격은 `599.881초`, claim→terminal은 `41.540초`이고 collection `pw-chrome-1787586500364-b24f0ecfd696394dab90` snapshot1은 checked300·official·collection 일치·matched-aware organic·adExcluded·광고30건 제외·top-item 위반0입니다. streak73→74, cursor는 동일 sort1800의 created_at `2026-07-10T06:14:28.356207Z`→`2026-08-14T06:50:00.792201Z`로 strict-forward입니다. cycle36 group40=distinct40·tracker54=distinct54·commit54·failure0이고 잔여20그룹/22트래커 중 현재 claimable18·격리2이며 terminal 뒤 processing0·closed/null·lane/run/lease/stage/job/tracker/probe 해제·baseline10·candidate false입니다.
- 검증 문서 commit `e56337c0983921011037329da347e6103a13faba`를 `main`에 push하고 Production에 재배포했습니다. Vercel deployment `dpl_2MdDkmSN2FmNUb2u2XtoLcVQxW7z`는 Ready·alias `https://insight.momentlabs.co.kr`이고, `2026-08-24T15:44:07Z` `/health`는 HTTP200·live·release `e56337c09839`·icn1, `/ready`는 HTTP200·Supabase ready·missingCount0을 반환했습니다. 배포 뒤 운영 SELECT-only 표본도 exact runtime/fingerprint·baseline10·candidate false를 유지하며 setter/DML은0회입니다.
- gate 실행 절차 재감사에서 서버 어댑터가 candidate 응답만 exact candidate/8로 확인하고 baseline rollback은 `accepted/activated`만 확인해 잘못된 mode/minutes도 복귀 성공으로 보고할 수 있는 P1을 발견했습니다. baseline exact-negative 회귀를 먼저 추가해 71/72 RED와 server contract69/70 RED를 확인한 뒤, exact `mode=baseline/minutes=10`이 아니면 409로 fail-closed하도록 최소 보완해 72/72·70/70 GREEN을 확인했습니다. 보호 잠금 hash는 대표님의 명시된 N30 안정화 범위에서 해당 서버 파일 1개만 갱신했고 23함수·100파일·49 migration 및 self-test가 통과했습니다.
- setter 뒤 별도 네트워크 SELECT는 heartbeat/claim이 coordination `updated_at`을 덮을 수 있으므로 폐기했습니다. 실제 candidate/rollback은 하나의 DB transaction에서 시작시각과 raw setter JSON을 transaction-local 값으로 보존하고, 같은 global row lock이 유지되는 동안 direct coordination·processing 상태를 최종 한 resultset으로 캡처한 뒤 commit합니다. candidate raw exact candidate/8 또는 rollback raw exact baseline/10, bounded updated_at, post mode/minutes, processing0·완전 idle이 모두 참일 때만 성공이며 timeout·응답유실은 재호출0·SELECT-only 상태 감사·성공판정0입니다.
- `2026-08-24T16:20:21.927Z~16:20:23.542Z` Production read-only probe는 `BEGIN READ ONLY`·transaction-local GUC·final SELECT·trailing COMMIT 한 호출 뒤 marker `n30_read_only_transaction_result_v1` 최종 resultset이 반환됨을 확인했습니다. `16:23:05.849Z~16:23:08.384Z`에는 실제 공용 template을 `READ ONLY`·`EXPLAIN`·`WHERE false`로 PostgreSQL compile해 marker `n30_candidate_template_dry_run_v1`, `setter_called=false`, transition/activation false, baseline10 불변을 확인했습니다. 실제 setter write는 계속0회입니다.
- `2026-08-24T16:40:15.481389Z` 최신 SELECT-only 감사는 exact runtime1.1.12/fingerprint, heartbeat43.694초, anchor age11.149523시간, streak80·baseline10·candidate false이며 anchor24h만 미충족입니다. event9941/run `f70c0131…`의 snapshot은 checked300·official collector·organic/adExcluded·excluded45·top5 위반0입니다. cycle36 group46=distinct claim/fingerprint46, tracker60=distinct60·commit60·failure/open/중복0, normal/resume order44/44 strict-forward·cursor sort2200이고 processing0·closed/null·lane/run/lease/stage/job/tracker/probe 완전 해제입니다. `1114…`는 자연 격리 만료·due이나 cycle terminal0, `12f5…`·`c0cc…`는 아직 격리 중이며 수동 상태 변경0회입니다.
...
- 현재 로컬 증거: 의도한 RED 뒤 local/native 124/124, handler 묶음, runtime/durable+isolation migration, server contract 68/68, release baseline `ok:true`, syntax·`git diff --check`가 GREEN입니다. 전체 `check:release`도 core1012/1012·place51/51·shopping64/64·Production auth18/18로 exit0이고, 보호 lock은 23함수·99파일·48 migration 및 self-test·fresh exact-match가 통과했습니다. commit `d655eb080d55016349ea4bda38007e947971c4ca`를 `main`에 push했고 Production `/health`·`/ready`가 release `d655eb080d55`·Supabase ready를 반환했습니다.
...
- Production 전환 직전 13:37:32 KST에 old runtime이 동일 group tracker 2건을 claim했고, 서버 전환 뒤 stale runtime 제어 호출이 fail-closed되면서 tracker lease가 `14:12:32.191 KST`까지 남았습니다. 강제 해제 없이 14:15:45 KST 자연 만료 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null을 확인했습니다.
- 완전 idle에서 운영 migration이 `20260824051736` partial-window isolation → `20260824051739` runtime1.1.12 → `20260824051741` exact candidate gate 순서로 적용됐습니다. 대상 함수 4개는 모두 SECURITY INVOKER·빈 search_path·PUBLIC/anon/auth 실행 불가·service_role 실행 가능입니다. Advisor WARN은 security2·performance23으로 기존과 같고 신규 대상 함수 WARN은 0입니다.
- Windows 관리자 updater는 exact commit `d655eb080d55016349ea4bda38007e947971c4ca`에 대해 `MI_EXTENSION_UPDATE_OK`, version1.1.12, syntax13, `loaded_extension_synced=true`, `native_host_registry_synced=true`, fingerprint `862b3779…e8e`를 반환했습니다. 첫 두 시도의 설치경로 updater 부재와 process-local 실행정책 차단은 각각 `CommandNotFoundException`·`PSSecurityException`으로 확인해 성공으로 세지 않았고, exact GitHub 파일 SHA-256 `fbd11aad…345e` 검증 뒤 해당 관리자 프로세스에만 Bypass를 적용한 실행이 성공했습니다.
- 공식 updater가 예약작업을 복구한 뒤 수동 wake 없이 첫 정상 1.1.12 run `0ac338d3…`가 14:30:32 KST group claim → tracker claim → 14:31:16 KST commit으로 끝났습니다. collection `pw-chrome-1787549476269-b0a0995cb5ec84c118d4`는 checked300·official collector·organic evidence/policy·adExcluded=true·광고45건 제외·rank64이고, cursor sort600→700, latest run group/tracker/commit 각1·failure0입니다. cycle #35는 group24/24·run24/24·tracker36/36로 중복0, active74·paused0·격리3·processing0, circuit closed·cooldown/lane/run/lease/stage/probe null이며 업데이트 뒤 wake 요청은 0입니다. ledger에는 trigger명이 없어 updater 직후 복구 시작과 정시 trigger 중 어느 쪽인지는 단정하지 않습니다.
- 새 stability anchor는 `2026-08-24T05:31:17.200373Z`(`2026-08-24 14:31:17.200373 KST`), streak1, baseline10, candidate false입니다. 가장 이른 candidate8 판정은 anchor+24시간인 `2026-08-25 14:31:17.200373 KST`이며 새 원자 성공6회와 나머지 idle gate가 함께 충족되기 전에는 8분 전환이나 속도 향상 완료로 판정하지 않습니다.
- 두 번째 정상 run `13c47ff5…`도 14:37:39→14:39:03 KST event9491 group claim → 9492 tracker claim → 9493 commit으로 끝났습니다. collection `pw-chrome-1787549943784-9856f3cb2f29096ce37a`은 checked300·official collector·organic-only·adExcluded=true·광고30건 제외·stable-full-window·rank29이고 cursor sort700→800입니다. cycle #35는 group25=distinct25=run25, tracker37=distinct37, commit35·failure/deferred0이며 이후 잠깐의 claiming lane도 신규 event 없이 해제돼 processing/lane/run/lease/stage/probe null·circuit closed입니다. anchor는 유지되고 streak2·baseline10·candidate false입니다.
- partial-window 대상 3건은 현재 cycle에서 아직 claim되지 않았고 격리 만료는 각각 `2026-08-24 16:19:56 KST`, `2026-08-25 00:33:02 KST`, `2026-08-25 04:18:58 KST`입니다. 현재 cursor sort800 뒤 claim 가능한 group은 9개이며, wake/cursor/quarantine를 건드리지 않고 다음 cycle과 자연 만료 순번에서 proof 보존 또는 typed failure를 확인합니다.
- candidate8 전환은 모든 gate가 참인 완전 idle에서 canonical cadence RPC를 정확히 1회만 호출하고, activation request-start·response-end UTC와 응답의 accepted/activated/mode/minutes, 응답 직후 SELECT한 cadence_mode/cadence_minutes/updated_at 및 idle 상태를 한 묶음으로 남깁니다. timeout·network·불완전/비정상 응답처럼 결과가 ambiguous하면 재호출하지 않고 활성화·성공으로 보고하지 않으며, 이후 SELECT로 관측한 DB 상태와 불확실성만 사실대로 기록합니다.
- candidate8 이후 속도는 DB 전환시각부터 checkpoint까지 고정 wall time의 fully-terminal distinct run/group만 세고 실행 중·전환 전 group과 active-duration은 제외합니다. 최소 60분과 remote-wake 단독 조기 claim을 제외한 scheduled-compatible group 6개를 모두 확보하고 open·중복0을 확인한 뒤 표본 수·commit/failure·remote wake 포함/제외 영향을 공개해 8.75~8.77과 비교합니다. 8.75 미만은 성공0, 8.75~8.77은 기준선 동등으로만 기록합니다. 일반 품질·시스템 실패는 baseline10·last-good fail-closed를 확인하되, DB 승인 strict `provider_partial_window:<1..299>_300`은 tracker-only 격리·candidate/proof·last-good 보존을 확인한 terminal 예외이며 atomic300 성공에는 포함하지 않습니다.
- 세 번째 정상 run `6a6823a4…`는 14:47:38→14:48:23 KST event9495 group → 9496 tracker → 9497 commit으로 종료됐습니다. collection `pw-chrome-1787550503430-51044388e8d8a43fe510`은 checked300·official collector·organic-only·adExcluded=true·광고30건 제외·rank27이고 cursor sort800→900입니다. cycle #35는 group26=distinct26=run26, tracker38=distinct38, commit36·failure/deferred0이며 processing/lane/run/lease/stage/probe null·circuit closed·cooldown null, updater 뒤 wake0입니다. anchor는 유지되고 streak3·baseline10·candidate false입니다.
- 2026-08-24 14:53:46 KST SELECT-only 재확인에서 event9497 뒤 신규 event는 0건입니다. heartbeat 14:53:31 KST, exact runtime/fingerprint, anchor·streak3·baseline10·candidate false가 유지되고 active74·paused0·quarantine3·processing0, circuit closed·cooldown/lane/run/lease/stage/probe null, cycle 중복0입니다. partial-window 3건은 anchor 뒤 claim/failure/snapshot 0건으로 아직 자연 격리 중입니다.
- 네 번째 정상 run `4b50752a…`는 14:57:38→14:59:02 KST event9499 group → 9500 tracker → 9501 commit으로 종료됐습니다. collection `pw-chrome-1787551142509-7e45fed6d5c996f4bfdf`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·stable-full-window·rank27이며 top5 광고/비오가닉 위반0, cursor sort900→1000 순방향입니다. cycle #35는 group27=distinct27=run27, tracker39=distinct39, commit37·failure/deferred0이고 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. anchor는 유지되고 streak4·baseline10·candidate false이며 partial-window 3건의 anchor 뒤 claim/failure/commit/snapshot은 아직 0입니다.
- 다섯 번째 정상 run `8e97fa6b…`는 15:00:00→15:00:45 KST event9503 group → 9504 tracker → 9505 commit으로 종료됐습니다. collection `pw-chrome-1787551245117-324adf2a00eb021016d3`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·rank33이며 top5 광고/비오가닉 위반0, cursor sort1000→1100 순방향입니다. 이 snapshot의 cross-page proof marker는 null이므로 stable proof로 확대하지 않습니다. cycle #35는 group28=distinct28=run28, tracker40=distinct40, commit38·failure/deferred0이고 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. anchor 유지·streak5·baseline10·candidate false이며 partial-window 경로는 아직 미실증입니다.
- 여섯 번째 run `00616e1a…`는 15:06:32→15:07:17 KST event9507→9509, collection `pw-chrome-1787551637738-15fc524ae50d3d5f5aa2`로 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·rank7·top5 위반0을 commit했고 cursor sort1100→1200입니다. cross-page proof 실제 값은 null입니다.
- 21초 뒤 시작한 일곱 번째 grouped run `a5e15aaf…`도 앞 run과 겹치지 않고 15:07:38→15:09:09 KST event9512→9516으로 종료됐습니다. collection `pw-chrome-1787551749474-1d0ac01af2c58ec68485`의 tracker/snapshot 2행 모두 checked300·official·organic·adExcluded=true·광고30건 제외, rank1·22, top10 위반0, cross-page proof `stable-full-window-v1`입니다. 대표 cursor는 같은 sort1200 안에서 created_at `2026-06-26→2026-07-10` 순방향이고 두 번째 sort1500 tracker는 같은 collection group 멤버입니다. cycle #35는 group30=distinct30=run30, tracker43=distinct43, commit41·failure/deferred0입니다. terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null, exact identity·anchor 유지·streak7·baseline10·candidate false입니다. 성공6회 조건만 충족했고 anchor+24시간은 아직 미충족이며 partial-window 경로도 미실증입니다.
- 여덟 번째 run `f3f6db46…`은 15:17:38→15:18:26 KST event9518→9520, collection `pw-chrome-1787552306189-759e36c7713137580259`로 checked300·official·organic-only/evidence·adExcluded=true·광고45건 제외·rank7·top5 위반0을 commit했습니다. cross-page proof는 실제 null이고 cursor sort1200→1300 순방향입니다. cycle #35는 group31=distinct31=run31, tracker44=distinct44, commit42·failure/deferred0이며 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. exact identity·anchor 유지·streak8·baseline10·candidate false이고 partial-window 3건은 아직 전부 격리 중입니다.
- 아홉 번째 run `837521a2…`는 15:27:39→15:28:22 KST event9522→9524, collection `pw-chrome-1787552902160-6e9a1df7e21fc53982a5`로 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0을 commit했습니다. 결과는 `matched=false`·rank null·tracking source `not_found`이고 cross-page proof도 실제 null이며, 순위 성공으로 확대하지 않습니다. cursor sort1300→1400 순방향, cycle #35 group32=distinct32=run32·tracker45=distinct45·commit43·failure/deferred0입니다. terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null, exact identity·anchor 유지·streak9·baseline10·candidate false이며 partial-window 3건은 아직 격리 중입니다.
- 열 번째 run `7143c260…`은 15:37:38→15:38:25 KST event9526→9528, collection `pw-chrome-1787553505441-caf04304c21d163f16d2`로 checked300·official·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0을 commit했습니다. 결과는 matched false·rank null·tracking source `not_found`, cross-page proof null이고 cursor sort1400→1500 순방향입니다. cycle #35 group33=distinct33=run33·tracker46=distinct46·commit44·failure/deferred0이며 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. exact identity·anchor 유지·streak10·baseline10·candidate false이고 partial-window 3건은 아직 전부 격리 중입니다.
- 열한 번째 run `8b3276a7…`은 15:47:39→15:48:21 KST event9530→9532, collection `pw-chrome-1787554101574-d52ef97b9305331ebd4b`로 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0을 commit했습니다. matched false·rank null·tracking source `not_found`, cross-page proof null이며 cursor는 sort1500 안에서 created_at `2026-07-08→2026-08-14` 순방향입니다. cycle #35 group34=distinct34=run34·tracker47=distinct47·commit45·failure/deferred0이고 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. exact identity·anchor 유지·streak11·baseline10·candidate false이며 partial-window 3건은 아직 격리 중입니다.
- 열두 번째 run `ff91859f…`는 15:57:32→15:58:16 KST event9534→9536, collection `pw-chrome-1787554696338-ab5293b9b7560680fcae`로 checked300·official·organic·adExcluded=true·광고60건 제외·top5 위반0, matched false·rank null·`not_found`·proof null을 commit했고 cursor sort1500→1600입니다.
- 앞 terminal 8초 뒤 비중첩 시작한 열세 번째 run `ba34d540…`도 15:58:24→15:59:06 KST event9538→9540, collection `pw-chrome-1787554746845-8dd863906a3b19f3d778`로 checked300·official·organic·adExcluded=true·광고45건 제외·top5 위반0·matched true·exact rank19·proof null을 commit했고 cursor sort1600→1700입니다. cycle #35 group36=distinct36=run36·tracker49=distinct49·commit47·failure/deferred0이고 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. exact identity·anchor 유지·streak13·baseline10·candidate false이며 partial-window 3건은 아직 격리 중입니다.
- 열네 번째 run `104e3511…`은 16:07:39→16:08:23 KST event9542→9544, checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·상위 광고/비오가닉 위반0을 commit했습니다. 결과는 matched false·rank null·`not_found`, proof null이고 cursor는 sort1700 안에서 `(created_at,id)`가 `2026-07-10…/2c78…→2026-08-14…/524e…` 순방향입니다. cycle #35 중복·failure0, terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe/current tracker/job null, exact identity·anchor 유지·streak14·baseline10·candidate false입니다.
- 첫 partial 대상 `c0cc…`는 16:19:56.817985 KST 자연 만료 예정이며 16:10 KST 기준 앞선 신규 group0·현 cycle 미claim입니다. 신규 등록·cycle 변화가 없다면 만료 뒤 첫 자연 claim의 우선 후보지만 미래 상태로 단정하지 않고 SELECT-only로 확인합니다. 나머지 `1114…`·`12f5…` 만료는 각각 8월25일 00:33:02·04:18:58 KST이고 현재 순번을 미래 확정값으로 확대하지 않습니다.
- 만료 전 열다섯 번째 run `b5d4e261…`은 event9546→9548, collection `pw-chrome-1787555877229-201e38a109e2cf7360b6`으로 checked300·official·organic-only/evidence·adExcluded=true·광고45건 제외·not_found·top5 위반0·`stable-full-window-v1`을 commit했고 cursor tuple `(1700,524e…)→(1800,2026-07-10…,f384…)`로 전진했습니다. 열여섯 번째 run `446f6524…`도 event9550→9552, collection `pw-chrome-1787555928421-84afa5742848c05c3f33`으로 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·not_found·top5 위반0을 commit했고 proof 값은 없으며 cursor tuple `(1800,f384…)→(1800,2026-08-14…,c7c…)` 순방향입니다. Run1 뒤 직접 idle 표본은 없지만 7.52초 뒤 Run2가 비중첩 정상 claim했고, Run2 뒤 16:19:36 KST에는 lane/run/lease/stage/job/tracker null을 직접 확인했습니다.
- `c0cc…`는 16:19:56.817985 KST 자연 만료 뒤 eligible-new 정확히1건·position1이 됐고, 자연 run `573c3945…`에서 event9554 group(new) →9555 tracker →9556 `job_failed: provider_partial_window:94_300` →9557 quarantine_set으로 단1회 terminal 됐습니다. 다음 격리는 `2026-08-25 16:29:00.724542 KST`, retry8입니다. 이 target은 snapshot 총0·신규0, last checked/current/best/worst null·check/found0로 부분 순위·30일 이력을 쓰지 않았습니다. global last-good은 앞 Run2 collection의 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외 상태를 유지했습니다.
- raw failure RPC envelope는 event details에 저장되지 않아 `cadenceProofPreserved` 원문 응답 자체는 조회할 수 없습니다. 대신 같은 failure RPC의 last_failure/code·격리 갱신, 배포 함수의 exact partial/runtime/fingerprint predicate와 return branch, anchor `14:31:17.200373 KST`·streak16·baseline10·last-good 불변을 함께 대조해 보존 branch 적용을 확인했습니다. cycle #35 group40=distinct40·tracker53=distinct53, c0 group/claim/terminal 각1이고 terminal 뒤 processing0·circuit closed·cooldown/lane/run/lease/stage/probe null입니다. 이로써 strict partial-window 실운영 격리·전역 cadence proof/last-good 보존 경로는 첫 1건에서 확인됐으며, 24시간·candidate8·후속 8분 성능 검증은 계속 남습니다.
- partial terminal 뒤 첫 자연 run `1afb3b60…`은 16:37:39 KST event9559 group(`priority=resume`, `resumeCursorBefore=true`, cursor `(1800,c7c…)`) →9560 tracker sort1900 →16:39:05 event9561 commit으로 끝났습니다. collection `pw-chrome-1787557145976-3288ac6e9775777e996b`은 snapshot1·checked300·official·organic-only/evidence·adExcluded=true·광고45건 제외·not_found·`stable-full-window-v1`·top5 위반0입니다. cursor는 `(1900,2026-07-10…,720f…)`로 전진하고 resume=false로 복귀했으며 c0cc 재claim0입니다. cycle #35 group41=distinct41·tracker54=distinct54·commit51·failure1, anchor 유지·streak16→17·baseline10이고 last-good은 이 atomic collection으로 정상 갱신됐습니다. 16:39:39·16:40:02 KST 직접 terminal 표본에서 processing0·circuit closed·cooldown null·run/lane/lease/stage/job/tracker/probe null·exact identity를 확인했습니다. strict partial 오류 뒤 자연 resume·원자성·순서·중복0·lane 해제 복구까지 운영 증거가 연결됐습니다.
- 다음 normal run `75e81855…`도 16:42:32→16:43:17 KST event9563→9565, collection `pw-chrome-1787557397706-39d484a6b400e293ecbc`으로 snapshot1·checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·not_found·proof null을 commit했습니다. cursor는 sort1900 내 `(2026-07-10…,720f…)→(2026-08-14…,0f2e…)` 순방향, resume=false입니다. cycle #35 group42=distinct42·tracker55=distinct55·commit52·failure1이고 16:46:07 KST terminal 표본에서 processing0·circuit closed·cooldown null·run/lane/lease/stage/page/job/tracker/probe 해제를 확인했습니다. streak18·candidate false입니다.
- pre-candidate fixed-wall 진단은 16:44:46 KST checkpoint 기준 anchor→checkpoint terminal distinct group18(성공17/partial failure1) / 2.224671h = `8.091084 group/h`, 첫 post-anchor claim→마지막 terminal은 같은18그룹 / 2.094066h = `8.595717 group/h`입니다. active-duration을 쓰지 않았고 중복 terminal claim0입니다. 두 값 모두 baseline10 전환 전 진단으로 기존 8.75~8.77보다 높지 않으므로 candidate8 성능·속도 향상 증거로 사용하지 않습니다. 현재 확인된 candidate 미충족은 anchor age 2.224671h<24h이며 나머지 exact/idle/freshness gate는 충족했습니다.
- normal run `4c61901b…`은 16:47:38→16:49:06 KST event9567→9569, collection `pw-chrome-1787557746293-ddf4c7e567d5060fbb3c`으로 snapshot1·checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·matched true·exact rank246·`stable-full-window-v1`을 commit했습니다. cursor/resume는 `(1900,0f2e…,false)→(2000,8589…,false)` 순방향이고 cycle #35 group43=distinct43·tracker56=distinct56·commit53·failure1, streak19입니다.
- 직후 event9571 `priority=new`는 partial 재claim이 아니라 cycle 시작 뒤 등록된 신규 tracker `5f2e2b4d…`·keyword `당뇨`였습니다. run `941c4b0d…`이 16:49:32→16:50:54 KST event9571→9573, collection `pw-chrome-1787557854370-d019e9871f151b43a7ba`으로 snapshot1·checked300·official·organic-only/evidence·adExcluded=true·광고45건 제외·not_found·`stable-full-window-v1`을 commit했습니다. new 우선순위이므로 cursor `(2000,8589…)`를 유지하고 resume false→true, partial3 재claim 각0입니다. anchor 유지·streak20·last-good 정상 갱신, cycle group44=distinct44·tracker57=distinct57·commit54·failure1이고 16:51:41 KST final processing0·circuit closed·cooldown null·run/lane/lease/stage/page/job/tracker/probe null입니다.
- 신규 tracker 뒤 run `409c110f…`가 16:57:38→16:58:23 KST event9575 `priority=resume` →9576 기존 tracker sort2100 →9577 commit으로 종료돼 기존 cursor 복귀를 확인했습니다. collection `pw-chrome-1787558303450-15a698be1728ba7d244c`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·not_found·상단 위반0이며 cross-page proof는 실제 null입니다. cursor/resume은 `(2000,8589…,true)→(2100,09e8…,false)`로 순방향 복귀했고 partial3 재claim은 각0입니다. anchor 유지·streak21·baseline10·candidate false, cycle group45=distinct45·tracker58=distinct58·commit55·failure1이고 16:59:22 KST processing0·circuit closed·cooldown null·run/lane/lease/stage/page/job/tracker/probe null을 확인했습니다.
- 17:03 KST 독립 SELECT-only gate 점검에서 exact runtime/fingerprint, heartbeat 2.404초, streak21, 최근 atomic300 4분10초, baseline10, processing0·circuit closed·cooldown null·lane/run/lease/stage/probe 해제를 재확인했습니다. candidate=false의 유일한 미충족 조건은 anchor 경과 2시간31분<24시간이며 가장 이른 판정은 `2026-08-25 14:31:17.200373 KST`입니다. 남은 partial `1114…`·`12f5…`는 각각 `2026-08-25 00:33:02.119249 KST`·`04:18:58.276397 KST`까지 격리되어 현재 eligible=false, 현 cycle 미claim·anchor 이후 snapshot/terminal0입니다. 실제 향후 claim 차수는 자연 cycle 진행에 따라 달라 단정하지 않습니다.
- 17:07:38 KST event9578/9579는 cycle 시작 뒤 등록된 신규 `당뇨쌀` tracker `4a544e31…`을 authoritative roster에 `new_after_start`로 추가한 뒤 같은 normalized keyword가 cycle #35에서 이미 원자 commit된 사실을 확인해 다음 cycle로 명시 이월한 경로입니다. raw reason `group_claim_limit/100`은 이 guard의 공통 audit label이며 실제 active 동일 그룹은 2 tracker라 100개 초과가 아닙니다. 신규 tracker는 claim/lease/snapshot/rank/history/retry 변조0, cursor sort2100 불변이고 cycle deferred1=distinct1입니다.
- defer 뒤 자연 run `fe16b2b1…`은 17:17:39→17:19:07 KST event9581 normal→9582 tracker→9583 commit으로 계속 진행됐습니다. collection `pw-chrome-1787559547477-5538a040fde549b602aa`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·matched true·rank39·`stable-full-window-v1`입니다. cursor는 sort2100 안에서 `(09e8…,2026-07-08…)→(7bde…,2026-07-10…)` 순방향이고 cycle group46=distinct46=run46·tracker59=distinct59·commit56·failure1·deferred1로 중복0입니다. 17:20 KST terminal에서 streak22·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다. deferred 신규 tracker의 다음 cycle 실제 claim·atomic300은 cycle #36 자연 시작 전이라 아직 증명하지 않습니다.
- 다음 자연 run `e6f6b55e…`도 17:27:39→17:29:03 KST event9585 normal→9586 tracker→9587 commit으로 끝났습니다. collection `pw-chrome-1787560143328-22c92ea5712c4b8dc2d2`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·matched true·rank14·`stable-full-window-v1`입니다. cursor `(sort2100,7bde…)→(sort2200,e0e4…)` 순방향, cycle group47=distinct47=run47·tracker60=distinct60·commit57·failure1·deferred1로 중복0이고 event9579 이월 tracker 재claim0입니다. 17:29:30 KST 뒤 streak23·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 17:29:31 KST fixed-wall 갱신은 anchor→checkpoint 실제 terminal group23(성공22·실패1, deferred1 제외)/2.970514h=`7.742767 group/h`, 첫 post-anchor claim→마지막 terminal 동일23/2.856739h=`8.051138 group/h`입니다. terminal group claim·tracker terminal 중복0이며 deferred를 수집 처리량에 포함하지 않았습니다. 둘 다 baseline10이고 기존 8.75~8.77보다 낮아 candidate8 속도 향상 증거가 아닙니다.
- 자연 run `12368750…`은 17:37:38→17:39:06 KST event9589 normal→9590 tracker→9591 commit으로 끝났습니다. collection `pw-chrome-1787560746962-a8eafa464d233a1f2051`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·matched true·rank22·`stable-full-window-v1`입니다. cursor `(sort2200,e0e4…)→(sort2300,14ac…)` 순방향, cycle group48=distinct48=run48·tracker61=distinct61·commit58·failure1·deferred1로 중복0이고 event9579 이월 tracker 재claim0입니다. 17:40 KST terminal에서 streak24·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 17:45:40 KST cycle35 SELECT-only snapshot의 cursor 뒤 eligible은 12 tracker/10 normalized group이고 wrap-around 잔여0이었습니다. 자연 cycle 완료 표식까지 최소 group terminal10회+no-seed 확인1회가 필요하며 신규 등록·repair·격리 만료·실패에 따라 늘 수 있습니다. event9579 이월 `4a544e31…`은 현 상태가 유지되면 cycle36 비격리 neverChecked 1순위지만 실제 claim 전에는 무누락 복구로 단정하지 않습니다.
- 자연 run `7be37526…`은 17:47:38→17:49:04 KST event9593 normal→9594 tracker→9595 commit으로 끝났습니다. collection `pw-chrome-1787561344223-356557852af2c4a0aeef`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·matched true·rank27·`stable-full-window-v1`입니다. cursor `(sort2300,14ac…)→(sort2400,026c…)` 순방향, cycle group49=distinct49=run49·tracker62=distinct62·commit59·failure1·deferred1로 중복0이고 event9579 이월 target 재claim0입니다. 17:50 KST terminal에서 streak25·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 자연 run `1b61aa06…`은 17:57:39→17:58:24 KST event9597 normal→9598 tracker→9599 commit으로 끝났습니다. collection `pw-chrome-1787561904600-6688be950ae986376a11`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·matched true·rank52이며 cross-page proof는 실제 null입니다. cursor `(sort2400,026c…)→(sort2600,15f4…)` 순방향, cycle group50=distinct50=run50·tracker63=distinct63·commit60·failure1·deferred1로 중복0이고 event9579 이월 target 재claim0입니다. 17:59 KST terminal에서 streak26·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 자연 run `1ad0d2c0…`은 18:07:39→18:08:22 KST event9601 normal→9602 tracker→9603 commit으로 끝났습니다. collection `pw-chrome-1787562502620-8d1fd9a10fcea5919a6d`은 snapshot1·checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·matched true·rank10이며 cross-page proof는 실제 null입니다. cursor `(sort2600,15f4…)→(sort2700,5ae9…)` 순방향, cycle group51=distinct51=run51·tracker64=distinct64·commit61·failure1·deferred1로 중복0이고 event9579 이월 target 재claim0입니다. 18:09 KST terminal에서 streak27·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 최근 normal claim 6회(event9581·9585·9589·9593·9597·9601)의 간격은 600.508·599.432·599.837·600.633·600.093초, 평균 `600.1006초=10.0016767분`입니다. 고유 run/group 각6·중복0, 실행 겹침0이고 coordination도 baseline10입니다. 이는 baseline10 scheduler 실동작 증거이며 candidate8 활성화나 속도 향상 증거가 아닙니다.
- 18:12 KST cycle35 snapshot의 cursor 뒤 잔여는 9 tracker/7 normalized group, wrap/new 잔여0입니다. uninterrupted baseline10만 적용한 산술 완료 범위는 10:22~10:32Z로 partial `1114…` 만료15:33Z보다 약5시간 이르지만 repair·신규·실패·중단에 따라 달라 보장하지 않습니다. event9579 이월 target은 현재값 기준 cycle36 비격리 neverChecked 1순위입니다.
- grouped run `107f0c2f…`은 18:17:38→18:18:23 KST event9607 group(member3)→9608~9610 tracker3→9611~9613 commit3으로 끝났습니다. shared collection `pw-chrome-1787563103439-5d66484002e1d57a8e75`의 snapshot3/3은 모두 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·위반0, rank18·7·71이고 proof는 모두 실제 null입니다. cursor seed `(sort2700,5ae9…)→(sort2800,fae8…)` 순방향이며 같은 keyword의 sort2900·3000 멤버도 동일 cycle terminal됐습니다. cycle group52=distinct52=run52·tracker67=distinct67·commit64·failure1·deferred1로 중복0, streak28(group당+1)·baseline10·candidate false이고 18:19 KST processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- natural run `b52cf06c…`은 18:27:39→18:28:26 KST event9615→9617, collection `pw-chrome-1787563706898-939026a3e0f6060a949f`로 checked300·official·organic-only/evidence·adExcluded=true·광고30건 제외·not_found·proof null을 commit했고 cursor sort2800→3100입니다. terminal 5.239초 뒤 별도 handoff run `c566b09f…`이 18:28:32→18:29:14 event9619→9621, collection `pw-chrome-1787563754095-9d695ad977bf00bd379a`으로 동일 원자300 계약·광고30건 제외·not_found·proof null을 commit해 cursor sort3100→3200으로 이어졌습니다.
- 두 run은 서로 다른 run/claim/group/tracker/collection이고 첫 terminal 뒤 두 번째 claim이라 겹침0·중복0입니다. mutable wake의 `rank-cron-cycle` request/consume과 remote poll 시각은 두 번째가 별도 1-job remote-wake handoff였다는 강한 증거지만 append-only ledger에는 trigger명이 없어 정확 trigger를 확정하지 않습니다. fixed-wall 실제 terminal group에는 두 성공을 각각 세되 `2그룹/5.239초`나 candidate8 cadence로 해석하지 않습니다. 최종 cycle group54=distinct54·tracker69=distinct69·commit66·failure1·deferred1, streak30·baseline10·candidate false이고 18:30:41 KST processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제, event9579 target 재claim0입니다.
- 다음 자연 run `7d336b89-c0f9-4599-bb94-cf41769acc90`은 18:37:38→18:38:19 KST event9623 group(normal) →9624 tracker `b6b91032…` sort3300 →9625 commit으로 끝났습니다. collection `pw-chrome-1787564299467-601ced5de9cb36155f5b`, snapshot `e2ec91ea…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·not_found·proof null이고 top5 광고/비오가닉 위반0입니다. cursor sort3200→3300 순방향, cycle group55=distinct55=run55·tracker70=distinct70·commit67·failure1·deferred1로 같은 cycle 중복0입니다. cycle의 terminal 미대응 tracker claim 2건은 Production 전환 직전 old-runtime event9484/9485뿐이고 lease가 자연 만료된 기존 장부이며, anchor 이후 tracker claim34건은 terminal 미대응0입니다. 18:38:56~18:41:20 KST에 exact identity·streak31·baseline10·candidate false, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 직접 확인했습니다.
- 18:38:47 KST 독립 candidate 감사에서 heartbeat age15.695초, 최근 atomic300 age27.195초, exact runtime/fingerprint·streak31·baseline10·closed/null/완전 idle 조건은 모두 충족했습니다. anchor `2026-08-24 14:31:17.200373 KST` 경과4.125058시간만 24시간에 미달해 `candidate_eligible=false`이고, 가장 이른 판정 `2026-08-25 14:31:17.200373 KST`까지 약19시간52분30초가 남았습니다. cadence RPC는 호출하지 않았습니다. 18:42:01 KST fixed-wall은 anchor 뒤 terminal distinct group31(성공30·partial failure1)/4.179031h=`7.417986 group/h`, 첫 post-anchor claim→마지막 terminal은 `7.728318 group/h`이며 baseline10 사전 관측으로 기존 8.75~8.77 대비 향상 증거가 아닙니다. anchor 뒤 snapshot33/collection30의 atomic300·source·organic·adExcluded·top-item 위반은 0입니다.
- 18:43 KST 잔여 경로 재감사에서 `1114…`·`12f5…`는 각각 8월25일 00:33:02·04:18:58 KST까지 자연 격리 중이며 새 claim/terminal/snapshot0입니다. legacy last-good은 `1114…` checked41·rank null, `12f5…` checked40·rank1과 current/best/worst1로 그대로 보존됐습니다. deferred `4a544e31…`도 event9578/9579 뒤 claim/terminal/snapshot0·active neverChecked·retry/error/quarantine/processing null입니다. cycle35가 아직 active라 cycle36 reclaim은 발생하지 않았고, 실제 다음 cycle claim과 atomic300 또는 typed failure 전에는 무누락 회복으로 판정하지 않습니다.
- 자연 run `db345422-68d7-4c20-a072-90fea023e261`은 18:47:38→18:48:22 KST event9627 group(normal) →9628 tracker `c0be23ff…` sort3500 →9629 commit으로 끝났습니다. collection `pw-chrome-1787564902091-1ec46d65ff423a292598`, snapshot `83d7bc9c…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·matched true·rank18·proof null이고 top5 위반0입니다. cursor sort3300→3500 순방향, cycle35 group56=distinct56=run56·tracker71=distinct71·commit68·failure1·deferred1로 중복0입니다. 18:49:52 KST terminal 표본에서 processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제, streak32·baseline10·candidate false를 확인했습니다. ledger에 trigger명이 없어 exact trigger는 단정하지 않습니다.
- `2026-08-24 18:50:08 KST` fixed-wall은 anchor 뒤 terminal distinct group32(성공31·partial failure1)/4.314118h=`7.417503 group/h`, 첫 post-anchor claim→마지막 terminal은 `7.658035 group/h`입니다. anchor 뒤 snapshot34/collection31의 checked300·official source·pw-chrome·organic policy/evidence·adExcluded 위반0, top-item 위반0이며 여전히 baseline10 사전 관측으로 8.75~8.77 대비 향상 증거가 아닙니다.
- 18:48:50 KST cycle35 미처리 실값은 cursor 뒤 collectable 2 tracker/2 group(sort3600·3700), wrap/new0, processing0입니다. 별도로 current-cycle deferred `4a544e31…` 1건과 아직 격리 중인 unrostered partial `1114…`·`12f5…` 2건이 있습니다. 방해가 없다는 산술상 sort3600·3700 terminal과 no-seed cycle-complete 판정까지 최소 자연 기회3회지만 보장값이 아닙니다. `4a…`는 새 cycle에서 new-priority group 후보라는 projection일 뿐 실제 reclaim은 아직0이고, partial2건도 자연 만료 뒤 새 cycle 순번의 회복/typed failure 전에는 판정하지 않습니다.
- 다음 자연 run `86c06f0e-1e6a-4346-a885-595f87a89393`은 18:57:38→18:58:22 KST event9631 group(normal) →9632 tracker `95987fe5…` sort3600 →9633 commit으로 끝났습니다. collection `pw-chrome-1787565502914-6371771fbe1c949c6e3e`, snapshot `48ccdc05…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고17건 제외·matched true·rank40·proof null이고 top5 위반0입니다. cursor sort3500→3600 순방향, cycle35 group57=distinct57=run57·tracker72=distinct72·commit69·failure1·deferred1로 중복0입니다. 18:59:03 KST processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제, streak33·baseline10·candidate false를 확인했습니다.
- 18:59:50 KST fixed-wall은 post-anchor terminal distinct group33(성공32·partial failure1)/4.476009h=`7.372533 group/h`, 첫 post-anchor claim→마지막 terminal은 `7.594040 group/h`입니다. anchor 뒤 snapshot35/collection32의 atomic/source/pw-chrome/organic/adExcluded/top-item 위반은0이고 baseline10 사전값이라 속도 향상으로 세지 않습니다. cycle35의 실제 collectable은 이제 sort3700 1 group과 이후 no-seed 완료 판정만 남았습니다.
- cycle35의 마지막 collectable run `faaa6d7b-7b92-4aa6-8af7-fd72f4021fe7`은 19:07:38→19:08:18 KST event9635 group(normal) →9636 tracker `89fec545…` sort3700 →9637 commit으로 끝났습니다. collection `pw-chrome-1787566098828-f7ac37338d4e43718551`, snapshot `7f2c6cd8…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·matched true·rank3·proof null이고 top5 위반0입니다. cursor sort3600→3700 순방향, cycle35 group58=distinct58=run58·tracker73=distinct73·commit70·failure1·deferred1로 중복0입니다. 19:09:18 KST processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제, tracker error/lease null·retry0, streak34·baseline10을 확인했습니다. 실제 collectable은0이지만 cycle status는 아직 active이고 다음 자연 no-seed 완료 event 전에는 cycle 완료로 보고하지 않습니다.
- 19:09:54 KST fixed-wall은 post-anchor terminal group34(성공33·partial failure1)/4.643655h=`7.321739 group/h`, 첫 post-anchor claim→마지막 terminal은 `7.537057 group/h`입니다. anchor 뒤 snapshot36/collection33의 atomic/source/pw-chrome/organic/adExcluded/top-item 위반0이며 baseline10 사전값입니다.
- 다음 자연 no-seed tick에서 19:17:38.879 KST event9638 `cycle_completed`가 cycle35에 정확히1회 기록됐고 coordination status/completed_at도 같은 값으로 전환됐습니다. 최종 roster76=distinct76, group58=distinct58, tracker claim73=distinct73, commit70·failure1·deferred1·completed1입니다. terminal 미대응 claim2는 이미 확인한 Production 전환 전 old-runtime event9484/9485뿐이며 자연 lease 만료 뒤 live processing/lane/run/lease는0입니다. 완료 details도 scheduled/distinct claimed73/73·committed70·failed1·repair0으로 일치하고 cursor sort3700은 유지됐습니다. 19:19:54 KST까지 cycle36 시작·`4a544e31…` reclaim event는0이므로 같은 tick 회복으로 과장하지 않고 다음 자연 기회에서 확인합니다.
- cycle35 완료 2분53초 뒤 무조작 cycle36 `f8e661b9…`가 19:20:32.140 KST event9639로 시작했고 active76·eligible73·quarantine3을 roster했습니다. cycle35에서 이월된 `4a544e31…`(neverChecked sort200)과 기존 동일 normalized keyword `bc1411c6…`(sort400)은 같은 fingerprint로 roster된 뒤 event9718 `priority=new/memberCount2` →9719·9720 동일 claim/run tracker2 →19:21:58.234 KST event9721·9722 동일 collection commit으로 회수됐습니다. collection `pw-chrome-1787566918234-6aa3b5e8981a372a13b7`의 snapshot2/2는 checked300·official collector·organic-only/evidence·adExcluded=true·광고37건 제외·top5 위반0이고 target rank10·companion rank26입니다. ledger cross-page proof는 두 건 `stable-full-window-v1`, snapshot item proof 필드는 실제 null입니다.
- 이월 target은 last_checked/rank10·check/found1·best/worst10으로 정상화됐고 deferred_at·processing/error/quarantine가 null, retry0입니다. cycle36 group1=distinct1=run1·tracker2=distinct2·commit2·failure/deferred0이며 new-priority라 normal cursor null·resume=true가 유지된 정상 분리 경로입니다. 19:23:05 KST processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제, streak35·baseline10·candidate false를 확인했습니다. 이로써 같은 cycle 중복 방지로 이월된 신규 tracker가 다음 cycle 첫 group에서 무누락 원자300으로 회수되는 실제 경로까지 확인했습니다.
- 19:23:40 KST fixed-wall은 post-anchor terminal group35(성공34·partial failure1) 기준 `7.182305 group/h`, 첫 claim→마지막 terminal `7.386059 group/h`이고 snapshot38/collection34의 atomic/source/pw-chrome/organic/adExcluded/top-item 위반0입니다. 여전히 baseline10 사전값이며 candidate8 속도 증거가 아닙니다.
- 신규 우선 회수 뒤 첫 자연 후속 run `7a9f15fc-91d2-4a51-96d0-842662ed8f60`은 19:27:38.876→19:28:19.593 KST event9725 `resume/member2` →9726·9727 tracker sort100·200 →9728·9729 commit으로 종료됐습니다. shared collection `pw-chrome-1787567299593-73b3ec169d3e54f34851`의 snapshot2/2는 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0이며 둘 다 matched false·rank null·proof null입니다. 미발견 결과를 순위 성공이나 stable proof로 확대하지 않습니다. cycle36 누적 group/claim/run2=distinct2, tracker4=distinct4·commit4·failure/deferred0으로 같은 cycle 중복0이고, cursor는 null/resume=true에서 대표 sort100으로 진입해 resume=false가 됐습니다. 19:29:51.845 KST exact runtime/fingerprint·heartbeat19.802초·streak36·baseline10, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다. candidate8 RPC는 호출하지 않았습니다.
- 19:33 KST 독립 gate 감사에서 runtime/fingerprint exact, Windows heartbeat28.745초, 최근 atomic300 약4분40초, streak36, baseline10, circuit closed·reason/cooldown null, processing0·lane/run/lease/stage/page/job/tracker/probe null이었습니다. anchor age는 5시간1분43초라 유일한 미충족은 24시간이며 earliest는 `2026-08-25 14:31:17.200373 KST`, candidate false·RPC0회입니다. 같은 시각 `1114…`·`12f5…`는 cycle36에 quarantined roster만 있고 claim/terminal/snapshot0이며 각각 8월25일 00:33:02·04:18:58 KST 자연 만료 전입니다. 실패 뒤 신규 snapshot0이라 기존 값은 보존됐지만 그 최신 과거 snapshot 자체는 legacy checked41/40이므로 atomic300 last-good으로 과장하지 않습니다. Production `/health`·`/ready`도 19:33:42 KST release `d655eb080d55`, live·Supabase ready를 유지했습니다.
- 다음 자연 normal run `2deb1ed4-7951-483b-b755-86c037162c1f`은 19:37:38.827→19:38:20.227 KST event9732 group(member2, cursorBefore sort100) →9733·9734 tracker2 →9735·9736 commit으로 끝났습니다. shared collection `pw-chrome-1787567900227-e13e1dfc6bf8c357f19d`의 snapshot2/2는 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0, rank241·104, proof null입니다. cursor tuple은 같은 sort100에서 created_at `2026-06-26T01:08:18.538194Z→2026-06-26T14:47:57.900053Z`로 strict forward이고 resume=false입니다. cycle36 누적 group/claim/run3=distinct3·tracker6=distinct6·commit6·failure/deferred0, 해당 run terminal2/open0입니다. 19:40:12 KST streak37·baseline10, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 재확인했습니다.
- 19:44:06 KST fixed-wall은 anchor 뒤 fully-terminal distinct group37(성공36·partial failure1), distinct claim/run37/37·open0입니다. anchor→관측 `7.096725 group/h`, 첫 post-anchor claim→최신 terminal `7.383118 group/h`이고 snapshot42/collection36의 checked300·official source·pw-chrome·organic/adExcluded/top-item 위반0, 같은 cycle group/tracker 중복0입니다. baseline10 사전값이며 개선 전 8.75~8.77보다 낮아 candidate8 성능으로 세지 않습니다.
- 다음 자연 normal run `946f8dae-36a6-478b-aca0-5cb042e8a0af`은 19:47:38.856→19:48:20.256 KST event9738 group(member1) →9739 tracker `099e1cd0…` →9740 commit으로 종료됐습니다. collection `pw-chrome-1787568500256-c425b34a232b8ed390bd`, snapshot `0badd30b…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0·rank48·proof null입니다. cursor tuple은 같은 sort100에서 created_at `2026-06-26T14:47:57.900053Z→2026-06-29T04:07:14.953018Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run4=distinct4·tracker7=distinct7·commit7·failure/deferred0, 해당 run terminal1/open0입니다. 19:50:18 KST streak38·baseline10, tracker/lookup processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- cycle36 순번 감사에서 현재 cursor 뒤 collectable은 53그룹/66트래커, duplicate-defer 후보0·pending repair0입니다. `1114…`·`12f5…`의 cycle36 quarantined roster는 tracker `worker_last_cycle_id`를 바꾸지 않으며 두 대상은 last_checked가 있어 `new`가 아니라 만료 후 cursor 뒤 또는 wrap의 `normal`/상황별 `resume`으로만 진입합니다. 10:55Z 현재 둘 다 worker_last_cycle_id=cycle33·claim/terminal/snapshot0이고 same-key current-cycle 처리0입니다. 신규·repair·remote wake·실패·cooldown·지연이 없다는 baseline10 조건부 projection은 `12f5…` 약19:47Z, `1114…` 약19:57Z wrap claim이지만 미래 보장값으로 사용하지 않습니다.
- 다음 자연 normal run `7115b2eb-0aa9-45ca-8e19-7bbae6d40bcb`은 19:57:38.889→19:59:03.389 KST event9742 group(member1) →9743 tracker `7f84a275…` →9744 commit으로 끝났습니다. collection `pw-chrome-1787569143389-060e416dcffe13499d0c`, snapshot `f3f1066a…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고60건 제외·top5 위반0·matched false/rank null·`stable-full-window-v1`입니다. cursor tuple은 같은 sort100에서 created_at `2026-06-29T04:07:14.953018Z→2026-06-29T09:40:01.786604Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run5=distinct5·tracker8=distinct8·commit8·failure/deferred/open0, 해당 run terminal1/open0입니다. 20:00:09 KST streak39·baseline10, tracker/lookup processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- cycle36 event9725→9732→9738→9742 자연 claim 간격은 599.951·600.029·600.033초로 baseline10 scheduler 실동작과 일치합니다. 그 다음 회차는 20:05:32 KST로 직전 claim보다473.480초 뒤였고, mutable wake row는 `rank-cron-cycle` 요청 20:04:33.646→소비 updated_at20:05:31.897 KST를 보여 claim 0.472초 전 handoff와 일치합니다. 이는 별도 remote wake의 강한 동시대 증거지만 append-only ledger에 trigger 필드가 없어 exact trigger를 확정하지 않습니다. 전체 baseline fixed-wall에는 terminal group1건으로 포함하되 짧은 간격 자체를 candidate8 cadence 증거로 세지 않습니다.
- 해당 normal run `8f2e0210-210a-4741-9b78-35bf7f0de078`은 event9747 group(member2) →9748·9749 tracker2 →20:06:15.960 KST event9750·9751 commit으로 끝났습니다. shared collection `pw-chrome-1787569575960-c48aa7309463d8ed84cc`, snapshot `d103a699…` rank84·`47a6a481…` rank183은 모두 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0·proof null입니다. 대표 cursor tuple은 같은 sort100에서 created_at `2026-06-29T09:40:01.786604Z→2026-07-01T07:58:47.711363Z` strict forward이고 다른 sort3400 member도 동일 collection으로 terminal됐습니다. cycle36 누적 group/claim/run6=distinct6·tracker10=distinct10·commit10·failure/deferred/open0, 해당 run terminal2/open0입니다. 20:06:58 KST streak40·baseline10, tracker/lookup processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 정규 tick run `ce507ab4-b1cc-4d0f-864e-19e1e7f8f59a`는 앞 remote-wake run terminal 82.735초 뒤인 20:07:38.695 KST event9753 group(normal/member1) →9754 tracker `1e712fd1…` →20:09:02.134 KST event9755 commit으로 끝나 두 run 겹침0을 확인했습니다. collection `pw-chrome-1787569742134-802bdbe55b44b5182b51`, snapshot `15799fae…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·matched false/rank null·`stable-full-window-v1`입니다. cursor tuple은 같은 sort100에서 created_at `2026-07-01T07:58:47.711363Z→2026-08-11T06:32:49.851713Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run7=distinct7·tracker11=distinct11·commit11·failure/deferred/open0입니다. 20:09:48 KST streak41·baseline10, tracker/lookup processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 20:12:41 KST event9755 상한 fixed-wall은 anchor 뒤 fully-terminal distinct group41(성공40·partial1), distinct claim/run41/41·open0입니다. anchor→관측 `7.205660 group/h`, 첫 claim→최신 terminal `7.423402 group/h`, snapshot47/collection40의 atomic/source/pw-chrome/organic/adExcluded/top-item 위반0·same-cycle group/tracker 중복0입니다. rank-cron-cycle remote-wake 영향이 포함된 baseline10 사전값이므로 순수 cadence나 candidate8 개선값으로 사용하지 않습니다.
- 다음 정규 run `bca8140a-3e56-47e7-9ad3-03984e05879c`은 앞 run terminal 뒤516.889초, claim-to-claim 약600.328초인 20:17:39.023 KST event9757 group(normal/member1) →9758 tracker `5f2e2b4d…` →20:19:02.874 KST event9759 commit으로 겹침 없이 종료됐습니다. collection `pw-chrome-1787570342874-5b0a88b1b49fb2c84693`, snapshot `f97f663a…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·matched false/rank null·`stable-full-window-v1`입니다. cursor tuple은 같은 sort100에서 created_at `2026-08-11T06:32:49.851713Z→2026-08-24T07:48:46.077570Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run8=distinct8·tracker12=distinct12·commit12·failure/deferred/open0입니다. 20:20:02 KST streak42·baseline10, tracker/lookup processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 20:24:26 KST 독립 SELECT-only gate 재검증은 runtime1.1.12/fingerprint exact·Windows heartbeat55.148초·최근 atomic300 323.432초·streak42·baseline10·closed/null·processing0·완전 idle을 확인했고, `candidate_eligible=false`의 유일한 미충족은 anchor 경과5.886시간<24시간입니다. 가장 이른 전환 시각은 `2026-08-25 14:31:17.200373 KST`이고 cadence RPC는 0회입니다. event9759까지 fully-terminal group42=성공41+typed partial1, distinct group/claim/run42/42/42·open0, snapshot48/collection41의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle 중복0입니다. anchor→관측 `7.139991 group/h`, 첫 claim→최신 terminal `7.381441 group/h`로 remote wake 영향이 포함된 baseline10 사전값이며 candidate8 개선값이 아닙니다.
- 같은 시각 잔여 partial `1114…`·`12f5…`는 각각 `2026-08-25 00:33:02.119249 KST`·`04:18:58.276397 KST` 자연 만료 전이고 cycle36 새 claim/terminal/snapshot/collection0입니다. 실패 뒤 신규 snapshot0이며 기존 legacy checked41/rank null·checked40/rank1과 last_checked/이력 수가 보존됐습니다. 현재 cursor 기준 `1114…`는 만료를 가정해야만 after-cursor 15번째, `12f5…`는 wrap 1번째이나 실제 격리 중이라 claimable하지 않습니다. 자연 회복이나 새 typed failure로 보고할 증거는 아직 없습니다.
- 다음 자연 run `b84cc226-650e-44dd-98d1-ccfd70f4191d`은 직전 terminal 뒤516.537초인 20:27:39.411 KST event9761 group(normal/member1) →9762 tracker `2dd39f10…` sort200 →20:28:21.311 KST event9763 commit으로 overlap0 종료됐습니다. collection `pw-chrome-1787570901311-518caa26bd4702aae80c`, snapshot `0d6d99b2…`는 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·rank59·contract valid입니다. cursor tuple은 `(sort100,2026-08-24T07:48:46.077570Z,5f2e…)→(sort200,2026-06-26T14:48:06.757674Z,2dd3…)` strict forward, resume=false입니다. cycle36 누적 group/claim/run9=distinct9·tracker13=distinct13·commit13·failure/open0이고 20:29:32 KST streak43·baseline10, processing0·circuit closed·cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 20:30:48 KST event9763 상한 fixed-wall은 fully-terminal group43=성공42+typed partial1, distinct group/claim/run43/43/43·open0입니다. anchor→observed `7.176305 group/h`, first claim→latest terminal `7.356630 group/h`, snapshot49/collection42의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. remote wake 영향 포함 baseline10 사전값이며 candidate8 개선값이 아닙니다.
- 다음 자연 run `25c3b786-7b0b-40d1-b606-0a81030560a5`은 직전 claim 뒤599.442초인 20:37:38.853 KST event9765 group(normal) →9766 tracker `c7095689…` →20:39:00.061 KST event9767 commit으로 claim→terminal81.208초·overlap0 종료됐습니다. collection `pw-chrome-1787571540061-8aea0eb7389643d4c90f`, snapshot `3517fe07…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·matched false/rank null·`stable-full-window-v1`입니다. cursor tuple은 같은 sort200에서 `2026-06-26T14:48:06.757674Z→2026-06-29T03:29:57.201268Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run10=distinct10·tracker14=distinct14·commit14·failure/open0이며 20:41:22 KST streak44·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 20:42:19 KST event9767 상한 fixed-wall은 fully-terminal group44=성공43+typed partial1, distinct group/claim/run44/44/44·open0입니다. anchor→observed `7.115221 group/h`, first claim→latest terminal `7.305938 group/h`, snapshot50/collection43의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. baseline10 사전값으로 candidate8 개선값이 아닙니다.
- 다음 자연 run `4f8d493d-9389-4d75-8979-704f3ff8178d`은 직전 claim 뒤599.969초인 20:47:38.822 KST event9769 group(normal) →9770 tracker `3296e4e6…` →20:49:01.706 KST event9771 commit으로 claim→terminal82.884초·overlap0 종료됐습니다. collection `pw-chrome-1787572141706-7f68443074234c7363d9`, snapshot `84c0b6b1…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·matched false/rank null·`stable-full-window-v1`입니다. cursor tuple은 같은 sort200에서 `2026-06-29T03:29:57.201268Z→2026-07-04T06:23:03.409082Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run11=distinct11·tracker15=distinct15·commit15·failure/open0이며 20:50:03 KST streak45·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 20:50:46 KST event9771 상한 fixed-wall은 fully-terminal group45=성공44+typed partial1, distinct group/claim/run45/45/45·open0입니다. anchor→observed `7.114765 group/h`, first claim→latest terminal `7.270234 group/h`, snapshot51/collection44의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. baseline10 사전값으로 candidate8 개선값이 아닙니다.
- 다음 자연 run `f8e92f54-aae1-47fa-baf9-cce0399fe6a9`은 직전 claim 뒤600.034초인 20:57:38.856 KST event9773 group(normal) →9774 tracker `bbc35960…` →20:58:23.046 KST event9775 commit으로 claim→terminal44.190초·overlap0 종료됐습니다. collection `pw-chrome-1787572703046-41a17c10b93506756ec5`, snapshot `fb98d013…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·matched false/rank null입니다. cursor는 sort200→300 strict forward입니다.
- event9775 terminal 9.038초 뒤 별도 normal run `986eba85-18f5-4fae-bfb5-659b1ca158a1`이 20:58:32.084 KST event9777 group →9778 tracker `6623318b…` →20:59:12.862 KST event9779 commit으로 overlap0 종료됐습니다. mutable wake row의 `rank-cron-cycle` requested/consumed20:58:15.030·updated20:58:31.722 KST와 claim 0.362초 전 handoff가 강하게 일치하지만 append-only ledger에 trigger가 없어 exact 원인은 단정하지 않습니다. collection `pw-chrome-1787572752862-4e8cc8fc8bc64c5e361f`, snapshot `cc594f3c…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고6건 제외·top5 위반0·rank1·proof null입니다. cursor는 같은 sort300에서 created_at `2026-06-26T14:48:13.489923Z→2026-06-29T04:31:17.324506Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run13=distinct13·tracker17=distinct17·commit17·failure/open0이고 21:02:02 KST streak47·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:01:59 KST event9779 상한 fixed-wall은 fully-terminal group47=성공46+typed partial1, distinct group/claim/run47/47/47·open0입니다. anchor→observed `7.217780 group/h`, first claim→latest terminal `7.390649 group/h`, snapshot53/collection46의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. remote wake 영향 포함 baseline10 사전값이므로 candidate8 개선값이 아닙니다.
- 원격 wake follow-on 뒤 정규 run `b393e2d1-4234-4fea-ab6f-a1718912c607`은 원래 scheduled event9773 claim 뒤600.070초인 21:07:38.926 KST event9781 group(normal) →9782 tracker `385aa03f…` →21:08:22.653 KST event9783 commit으로 claim→terminal43.727초·overlap0 종료돼 정규 10분 anchor/순서 복귀를 확인했습니다. collection `pw-chrome-1787573302653-a433c9e83e926ea76e83`, snapshot `96b79fcb…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고60건 제외·top5 위반0·rank2·proof null입니다. cursor는 같은 sort300에서 created_at `2026-06-29T04:31:17.324506Z→2026-07-06T03:17:39.223733Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run14=distinct14·tracker18=distinct18·commit18·failure/open0이고 21:09:16 KST streak48·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:10:00 KST event9783 상한 fixed-wall은 fully-terminal group48=성공47+typed partial1, distinct group/claim/run48/48/48·open0입니다. anchor→observed `7.222984 group/h`, first claim→latest terminal `7.370886 group/h`, snapshot54/collection47의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. 앞선 remote wake가 구간에 포함된 baseline10 사전값이므로 candidate8 개선값이 아닙니다.
- 다음 자연 run `02470238-4fbb-4ba8-8835-8859ba7b1183`은 직전 claim 뒤600.161초인 21:17:39.087 KST event9785 group(normal) →9786 tracker `bf0e733f…` →21:18:18.970 KST event9787 commit으로 claim→terminal39.883초·overlap0 종료됐습니다. collection `pw-chrome-1787573898970-6f598d222c4e01eaac94`, snapshot `41954f70…`은 checked300·official collector·organic-only/evidence·adExcluded=true·광고45건 제외·top5 위반0·rank82·proof null입니다. cursor는 sort300→400 strict forward, resume=false입니다. cycle36 누적 group/claim/run15=distinct15·tracker19=distinct19·commit19·failure/open0이고 21:19:10 KST streak49·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:19:56 KST event9787 상한 fixed-wall은 fully-terminal group49=성공48+typed partial1, distinct group/claim/run49/49/49·open0입니다. anchor→observed `7.194372 group/h`, first claim→latest terminal `7.337800 group/h`, snapshot55/collection48의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. 앞선 remote wake 영향 포함 baseline10 사전값으로 candidate8 개선값이 아닙니다.
- 21:22:11 KST 잔여 partial 재감사는 `1114…` 격리 잔여3:10:50·가상 해제 시 after-cursor 8번째, `12f5…` 잔여6:56:46·가상 해제 시 wrap 1번째를 확인했습니다. 둘 다 cycle36 claim/terminal·실패 뒤 신규 snapshot/collection0이고 기존 rank/last_checked/이력 수가 보존됐습니다. 신규/remote wake/repair/실패/cooldown/지연이 없다는 고정 조건에서만 wrap 후 약19:27Z/19:37Z claim projection이며 보장값이 아닙니다.
- 다음 자연 run `28ea195a-ba49-409b-914d-a5245ae4e250`은 직전 claim 뒤599.850초인 21:27:38.937 KST event9790 group(normal/member2) →9791·9792 tracker2 →21:28:21.317 KST event9793·9794 commit2로 claim→terminal42.380초·overlap0 종료됐습니다. shared collection `pw-chrome-1787574501317-9decc4a524ad55d6b0ab`, snapshot `5ee0ade1…` rank146·`498b91be…` rank null은 모두 checked300·official collector·organic-only/evidence·adExcluded=true·광고30건 제외·top5 위반0·proof null입니다. 대표 cursor는 같은 sort400에서 created_at `2026-06-26T14:48:41.643037Z→2026-07-08T02:08:19.493670Z` strict forward이고 companion sort1200도 같은 group/collection으로 terminal됐습니다. cycle36 누적 group/claim/run16=distinct16·tracker21=distinct21·commit21·failure/open0이고 21:29:20 KST streak50·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:30:05 KST event9794 상한 fixed-wall은 fully-terminal group50=성공49+typed partial1, distinct group/claim/run50/50/50·open0입니다. anchor→observed `7.163140 group/h`, first claim→latest terminal `7.304528 group/h`, snapshot57/collection49의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. 앞선 remote wake 영향 포함 baseline10 사전값으로 candidate8 개선값이 아닙니다.
- 다음 자연 run `38368b08-2f54-4f56-a0e2-4fdf67111ca2`은 직전 claim 뒤600.244초인 21:37:39.181 KST event9799 group(normal/member4) →9800~9803 tracker4 →21:38:21.341 KST event9804~9807 commit4로 claim→terminal42.160초·overlap0 종료됐습니다. shared collection `pw-chrome-1787575101341-b9710ade3a0645690992`의 snapshot4는 모두 checked300·official collector/item source·organic-only/evidence·adExcluded=true·광고45건 제외·top5 organic1~5/광고0입니다. cursor tuple은 같은 sort400에서 `2026-07-08T02:08:19.493670Z→2026-07-10T05:59:32.282672Z` strict forward, resume=false입니다. cycle36 누적 group/claim/run17=distinct17·tracker25=distinct25·commit25·failure/open0이고 21:39:22 KST streak51·baseline10, processing0·circuit closed·reason/cooldown null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:41:46 KST event9807 상한 fixed-wall은 fully-terminal group51=성공50+typed partial1, distinct group/claim/run51/51/51·open0입니다. anchor→observed7.174895381h=`7.108117581 group/h`, first claim→latest terminal7.011742500h=`7.273512968 group/h`, snapshot61/collection50의 atomic300·official/pw-chrome·organic/adExcluded·top-item 위반0·same-cycle group/tracker 중복0입니다. baseline10 사전값으로 candidate8 개선값이 아닙니다.
- 다음 정규 호환 run `7997d9da-b325-4f8b-992e-e6a2f9aa5f80`은 직전 claim 뒤599.794초인 21:47:38.975 KST event9809 group(normal/member1) →9810 tracker →21:48:24.738 KST event9811 commit으로 claim→terminal45.763초·overlap0 종료됐습니다. collection `pw-chrome-1787575704738-8bd8fae82bb052301b85`, snapshot `3d143878…`은 checked300·official collector/item source·organic-only/evidence·adExcluded=true·광고45건 제외·rank115·top5 organic1~5/광고0입니다. cursor tuple은 sort400→500 strict forward, resume=false이고 cycle36 group/claim/run18=distinct18·tracker26=distinct26·commit26·failure/open0입니다. 21:49:23 KST exact identity·streak52·baseline10, processing0·closed/null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 21:48:51 KST operations 재현값은 최근 atomic300 26초·Windows heartbeat20초, runtime/fingerprint exact, anchor·streak52·baseline10, closed/null·processing0·완전 idle입니다. `candidate_eligible=false`의 미충족은 anchor 경과 약7시간17분<24시간뿐이며 cadence RPC는 0회입니다. 21:50:10 KST event9811 상한 fully-terminal group52=성공51+typed partial1, distinct group/claim/run52/52/52·open0, anchor→observed `7.108843 group/h`, first claim→latest terminal `7.242993 group/h`입니다. snapshot62/collection51 품질 위반·same-cycle group/tracker 중복은0이며 baseline10 사전값입니다.
- 21:53:43 KST 독립 candidate 감사도 29개 predicate 중 `anchor_24h` 하나만 false였습니다. anchor 경과7.373947시간·잔여16.626053시간, streak52이고 exact identity·heartbeat·최근 atomic300·baseline10·closed/null·processing0·완전 idle은 모두 true입니다. 21:55:30 KST 잔여 partial 감사에서 `1114…`·`12f5…`는 각각 8월25일 00:33:02·04:18:58 KST 격리 전이며 cycle36 roster 외 claim/terminal/신규 snapshot/collection0, legacy rank·check/found·snapshot 수를 그대로 보존했습니다. 아직 실제 claim 순번·회복·새 typed failure는 없습니다.
- 다음 정규 호환 run `b9584877-959d-41c8-9e63-19f350c57f87`은 직전 scheduled claim 뒤599.853초인 21:57:38.828 KST event9813 group(normal) →9814 tracker →21:59:01.658 KST event9815 commit으로 claim→terminal82.830초·overlap0 종료됐습니다. collection `pw-chrome-1787576341658-23b17c8f258c796c158f`, snapshot `9b20407a…`은 checked300·official collector/item source·organic-only/evidence·adExcluded=true·광고45건 제외·not_found·`stable-full-window-v1`·top5 위반0입니다. cursor는 sort500 안에서 strict forward, resume=false이고 cycle36 group/claim/run19=distinct19·tracker27=distinct27·commit27·failure/open0입니다. 22:00:18 KST streak53·baseline10, processing0·closed/null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다. 22:00:50 KST event9815 상한 fully-terminal group53=성공52+typed partial1·open0, anchor→observed `7.073710 group/h`, first claim→latest terminal `7.204733 group/h`, snapshot63/collection52 품질 위반·same-cycle 중복0으로 baseline10 사전값입니다.
- 22:03~22:04 KST Production `/health`·`/ready`는 release `d655eb080d55`, live/ready·Supabase ready·missing0으로 재확인됐습니다. 운영 runtime/fingerprint exact이고 migration `20260824051736/39/41`이 모두 존재하며 관련 함수4개는 SECURITY INVOKER·빈 search_path·postgres/service_role execute-only입니다. event9815 상한 cycle36은 group/run/claim19/19/19·tracker27/27, open/orphan/duplicate terminal/member mismatch·cursor non-forward 모두0, coordination cursor exact·완전 idle입니다.
- 다음 grouped run `0bcbbdb8-fe3a-4d77-9b04-9d592160c5d1`은 직전 scheduled claim 뒤600.343초인 22:07:39.171 KST event9818 group(normal/member2) →9819·9820 tracker2 →22:09:01.497 KST event9821·9822 commit2로 claim→terminal82.326초·overlap0 종료됐습니다. shared collection `pw-chrome-1787576941497-d8391adcb4fbf8b1b208`의 snapshot2는 checked300·official collector/item source·organic-only/evidence·adExcluded=true·광고30건 제외·rank85/not_found·`stable-full-window-v1`·top5 위반0입니다. 대표 cursor는 sort500 내 strict forward, companion sort1400도 같은 atomic collection으로 terminal됐습니다. cycle36 group/claim/run20=distinct20·tracker29=distinct29·commit29·failure/open0이고 22:09:59 KST streak54·baseline10, processing0·closed/null·lane/run/lease/stage/page/job/tracker/probe 해제를 확인했습니다.
- 22:12:50 KST event9822 상한 fixed-wall은 fully-terminal group54=성공53+typed partial1·open0, distinct group/claim/run54/54/54입니다. anchor→observed `7.019819 group/h`, first claim→latest terminal `7.178086 group/h`, snapshot65/collection53의 atomic/source/pw-chrome/organic/adExcluded/top5 위반·same-cycle group/tracker 중복은0입니다. remote wake 영향 포함 baseline10 사전값으로 candidate8 개선값이 아닙니다.
- run `d501c3e6-f4f2-4138-9f96-f849f47048c0`은 event9827 `13:17:38.853Z` group(normal/member4) →9828~9831 tracker4 →9832~9835 `13:18:22.268Z` commit4로 끝났습니다. 직전 group 간격599.682초·claim→terminal43.415초·overlap0입니다. shared collection `pw-chrome-1787577502268-c706b1f44a66ffa81a6a`의 snapshot4는 모두 checked300·official collector/item source·organic-only·`naver_shopping_organic_list`·adExcluded true·excluded45이고 rank8/48/5/22, top-item20개 광고/비오가닉 위반0입니다. 대표 cursor는 `(500,2026-07-08…,97e5…)→(500,2026-07-10…,4722…)` strict tuple forward·resume false입니다. cycle36은 group/claim/run21=distinct21·tracker33=distinct33·commit33·failure/open/중복/orphan0이고 `13:24:48Z` processing0·closed/null·lane/run/lease/stage/job/tracker/probe null입니다.
- `2026-08-24T13:25:10Z` event9835 고정벽은 anchor 뒤 fully-terminal group55=atomic success54+typed partial1·open0입니다. anchor→observed `6.963775 group/h`, first natural claim→observed `7.058576 group/h`이며 최신 terminal 기준도 각각 `7.065103`·`7.162702 group/h`입니다. 모두 baseline10 pre-candidate이고 개선 전 8.75~8.77보다 낮아 속도 향상 증거가 아닙니다. operations는 exact runtime/fingerprint·streak55·baseline10·candidate false·closed/null·processing0·완전 idle이고 유일한 gate 미충족은 anchor 24시간입니다.
- `2026-08-24T13:21:13Z` 잔여 strict partial `1114f3af…`·`12f5330a…`는 각각 quarantine `15:33:02.119249Z`·`19:18:58.276397Z` 전이며 processing null입니다. legacy rank/check/found와 snapshot58/71은 보존되고 post-anchor claim/terminal은 아직0이므로 자연 만료·실제 cursor 순번 전에는 회복이나 typed failure를 단정하지 않습니다.
...
- TDD 뒤 N30 집중 289/289, collector 64/64, worker 68/68, handler 77/77, migration/cycle 27/27, server contract 65/65, release baseline, syntax·`git diff --check`가 통과했습니다. 보호 잠금은 승인된 6개 파일과 신규 migration 2개를 선택 갱신해 23함수·95파일·44 migration 및 self-test가 통과했습니다. 전체 `check:release`도 core 743/743·place 51/51·shopping 64/64·Production auth 18/18, public build 9파일·inline script 6개·CSP hash 4개로 exit0입니다. 기존 파일 전체 branch 80%라고 과장하지 않습니다.
- 12:17 KST 운영 SELECT-only 확인은 runtime 1.1.10 exact fingerprint, active74·paused0·quarantine3·processing0, circuit closed·cadence10·lane/run/lease null입니다. 12:01 KST `native_host_input_closed` 1회 뒤 12:02·12:13 KST 두 자연 회차가 각각 official collector·checked300으로 commit돼 Windows heartbeat가 복귀했습니다. migration 전 당시 streak2·candidate false였으며, v1.1.11 첫 atomic300부터 24시간+6회를 새로 셉니다.
- commit `eeba518ecfa2`를 GitHub `main`에 push했고 Production `/health`·`/ready`가 같은 release `eeba518ecfa2`와 Supabase ready를 반환했습니다. 이어 migration `20260821180001_naver_shopping_error_taxonomy_hardening`과 `20260821180002_naver_shopping_runtime_1_1_11`을 순서대로 적용했습니다.
- migration 직후 SELECT-only 상태는 이전 Windows heartbeat runtime `1.1.10`을 그대로 보고하며 `stability_started_at=null`·success streak0·cadence baseline10·candidate false, circuit closed, processing0, lane/run/lease null이었습니다. 이는 이전 proof를 상속하지 않는 의도된 fail-closed 초기화이며 이후 새 runtime의 첫 원자 성공부터 증거를 다시 셉니다.
- Windows 관리자 PowerShell에서 commit `eeba518ecfa2398b085ce26a73623d13b62c95f9` updater를 실행했습니다. 설치 파일 직접 검증은 version `1.1.11`, service-worker SHA-256 exact=true(`a0ef0019…713c`), native host `co.kr.momentinsight.naver_shopping`, registry exact=true, 예약 작업 enabled/running을 반환했습니다. 운영 heartbeat도 runtime `1.1.11`과 저장소 재계산 fingerprint `6461e835e840ff873711f38a223ab1a7a06b3e2945822a92cce49e50a295cf00`의 정확한 일치를 확인했습니다.
- 첫 v1.1.11 자연 run `01fceb68…`는 13:55:35 KST group claim → tracker claim → 13:56:59 KST commit으로 끝났습니다. collection `pw-chrome-1787374619637-b90cb4f65cbd4d4926ad`은 공식 collector·organic-only·광고 제외 true/45건·stable-full-window·`checked_count=300`이고 tracker 오류·retry·processing·quarantine가 모두 해제됐습니다. 새 stability anchor는 `2026-08-22 13:57:00.265984 KST`입니다.
- 후속 run `a327119e…`도 13:59:41 KST group/tracker claim → 14:01:07 KST commit으로 종료됐습니다. collection `pw-chrome-1787374867874-72715ebda704213a085c`은 같은 원자300 계약을 충족하고 rank22를 저장했습니다. cycle #28 group 46/46·tracker 59/59가 distinct라 same-cycle 중복0, cursor sort2200→2300 순방향이며 terminal 뒤 circuit closed·cooldown null·processing/lane/run/lease null입니다. 당시 success streak2·cadence baseline10·candidate false였고, 가장 이른 candidate8 판정은 `2026-08-23 13:57:00.265984 KST` 이후에도 새 원자 성공6회와 나머지 gate가 모두 충족된 경우뿐입니다. 업데이트 전 과거 미완결 ledger 2건이 있어 cycle 전체 terminal linkage 완전무결로는 보고하지 않습니다.
- 세 번째 순차 run `9220f034…`도 14:03:35 KST claim → 14:05:00 KST commit으로 끝났습니다. collection `pw-chrome-1787375100179-465541e9cd8ad74dd994`은 checked300·official collector·organic-only·adExcluded=true·광고45개 제외·stable proof이고 cursor는 sort2300→2400으로 전진했습니다. 이후 cycle group47/47·tracker60/60 distinct, processing0·lane/run/lease null, circuit closed·streak3·candidate false를 확인했습니다.
- 14:40 KST 자동 관측에서 runtime/fingerprint exact, heartbeat 정상, 새 anchor 뒤 success streak7까지 증가했지만 24시간 미경과로 cadence baseline10·candidate false를 유지했습니다. anchor 뒤 snapshot 8행은 checked300·official source·adExcluded 위반0, matched organic 위반0입니다. 최신 event7161→7163도 원자300 terminal이고 cycle group51/51·tracker66/66 distinct로 중복0, terminal 뒤 circuit closed·processing0·lane/run/lease null입니다. active74·paused0·격리3이며 첫 partial 자연 만료는 16:00 KST라 아직 재수집 결과가 없습니다.
- 운영 증거 문서 커밋 `267a9c21ed5e`도 GitHub `main`과 Production에 반영됐고 15:08 KST `/health`·`/ready`가 같은 release의 live·Supabase ready를 반환했습니다. 이 docs-only release는 기능 commit `eeba518`을 포함하며 N30 런타임 바이트를 추가 변경하지 않습니다.
- 15:25 KST SELECT-only 점검은 runtime1.1.11/fingerprint exact, heartbeat 12초, anchor `13:57:00.265984 KST`, streak12, baseline10·candidate false를 확인했습니다. anchor 뒤 snapshot13/collection11은 checked300·official source·organic-only·adExcluded 위반0, cycle #28 group56/56·tracker71/71 distinct로 중복0이며 terminal 뒤 active74·paused0·격리3·processing0, circuit closed·cooldown/lane/run/lease null입니다.
- candidate8 전환 자체도 외부 점검에만 의존하지 않도록 additive migration `20260822061741_naver_shopping_candidate_exact_identity_gate`를 반영했습니다. DB 행 잠금 안에서 exact fingerprint, Windows primary heartbeat 3분, 최근 atomic 성공15분, baseline10, 24시간·6회, processing0 및 모든 lane/run/stage/probe/cooldown 해제를 동시에 요구하고 두 번째 candidate 호출은 거부합니다. migration은 anchor/streak를 갱신하지 않습니다. TDD RED를 확인한 뒤 durable16/16, N30 집중290/290, server contract66/66, 전체 release(main744·place51·shopping64), 보호 잠금 23함수·96파일·45 migration 및 self-test, rollback SQL 실행 검증을 통과했습니다.
- RED checkpoint `ea27672`와 GREEN commit `f8a8ce8`을 GitHub `main`에 push했고 Production `/health`·`/ready`가 release `f8a8ce8eed0e`, live·Supabase ready를 반환했습니다. 06:33 UTC 자연 idle(exact runtime/fingerprint, processing0, lane/run/lease null)에서 migration을 적용했고 운영 함수 2개는 SECURITY INVOKER·빈 search_path·anon/auth 실행불가·service_role 실행가능입니다. 적용 뒤 anchor `04:57:00.265984Z`와 streak13은 보존됐고 baseline10·candidate false입니다. Advisor는 기존 security15(정보13·경고2), performance66(정보43·경고23) 그대로여서 신규 경고0입니다.
- 적용 후 첫 자연 run `39a02312…`도 06:39:42→06:40:27Z 정상 종료됐습니다. sort100 동일 키워드 tracker2건이 collection `pw-chrome-1787380827140-d8b0a5b42e090c8690e5`를 공유해 각각 checked300·official source·organic-only·adExcluded=true·excludedAds30, rank85/125를 기록했습니다. group1·tracker2·terminal2 모두 distinct, failure/open claim0이고 cursor는 같은 sort 안에서 순방향입니다. terminal 뒤 processing0·lane/run/stage/probe null·circuit closed, anchor 불변·streak14·baseline10·candidate false입니다.
- 첫 partial 대상 `아이쉘 차량용 거치대`는 강제 조작 없이 16:00:03 KST 격리 만료 뒤 16:09:42 자연 claim됐으나 16:10:56 `provider_partial_window:94_300`으로 typed failure 후 24시간 재격리됐습니다. 총 snapshot0·last_checked/current_rank null을 유지해 94건을 300건이나 순위로 저장하지 않았고, claim/group 중복0입니다. 이후 cycle이 sort200까지 정상 전진·commit해 순서와 lane 해제를 확인했으며 현재 circuit closed·cooldown null·processing/lane/run/lease null입니다. 실행시간74.6초와 v1.1.11 계약은 독립 2-pass 경로와 일치하지만 운영 event/snapshot에 pass 번호가 없어 직접 2-pass marker가 있다고 과장하지 않습니다.
- 이 typed failure는 8분 후보 안정성 증거를 의도대로 초기화했습니다. 다음 정상 atomic300이 만든 현재 anchor는 `2026-08-22T07:19:05.344010Z`(`16:19:05.344010 KST`), success streak32, cadence baseline10, candidate false입니다. 따라서 기존 `13:57 KST` 판정 시각은 더 이상 유효하지 않고 가장 이른 candidate8 판정은 `2026-08-23 16:19:05.344010 KST`입니다. 새 anchor 뒤 group32/terminal32·tracker42/commit42, failure/deferred/open0이며 snapshot42/collection32의 checked300·official source·organic evidence/policy·adExcluded 위반0, group/tracker/run/claim 중복0·순서 역행0·실행 겹침0입니다. 현재 짧은 실측 wall 처리량은 7.08 group/hour로 기존 기준 8.75~8.77보다 낮으므로 속도 향상 완료로 보고하지 않습니다.
- 나머지 partial 2건은 18:50·19:00 KST 격리가 만료됐지만 20:46 KST까지 자연 claim/terminal이 없습니다. cycle #29 시작 때 quarantined roster로 고정됐고 현재 cursor sort1700 기준 순번을 기다리는 상태입니다. 만료 후 snapshot0이며 `칼슘단백쌀`의 legacy checked40/rank1과 `키크는쌀`의 legacy checked41/rank null은 그대로 보존됐습니다. 실제 실행 전이므로 2-pass 회복·typed failure 어느 쪽도 판정하지 않습니다.
- 20:55 KST 재검증에서 anchor는 `16:19:05 KST`, streak34, baseline10·candidate false입니다. anchor 뒤 snapshot43/collection33은 원자300/source/pw-chrome/organic/adExcluded 위반0이고 cycle #29 group40/40·tracker53/53·run 중복0·cursor 역행0입니다. processing0·lane/run/lease/stage/probe null, circuit closed·cooldown null입니다. anchor 이후 fixed-wall 처리량은 7.397 group/hour로 기준 8.75~8.77보다 낮습니다. 잔여 partial은 cursor sort1800 앞 17개 group 처리 뒤 fallback에서 `칼슘단백쌀`→`키크는쌀` 순으로 자연 진입할 조건이며, 아직 event/snapshot0입니다.
...
- 다음 event7469→7471도 21:19:42→21:21:10 KST collection `pw-chrome-1787401270198-122004826fc8ef0d10b3`을 checked300·official source·organic-only·adExcluded=true·광고30건 제외로 commit했습니다. cursor sort1900→2000 순방향, cycle group/run43/43·tracker/terminal56/56로 중복0이며 terminal 뒤 processing·lane/run/lease/probe null·circuit closed입니다. streak37·baseline10·candidate false이고 잔여 partial은 전체 자연 순번 15/16·16/16으로 대기합니다.
- event7473→7475는 21:29:43→21:30:24 KST collection `pw-chrome-1787401824933-c000972ef91c68832ad7`을 checked300·official source·organic-only·adExcluded=true·광고30건 제외로 commit했습니다. cursor sort2000→2100 순방향, cycle 중복0이며 terminal 뒤 processing·lane/run/lease/probe null·circuit closed입니다. streak38·baseline10·candidate false이고 잔여 partial 앞에 자연 group13개, 전체 순번14/15·15/15가 남았습니다.
- event7477→7479는 21:39:43→21:41:07 KST collection `pw-chrome-1787402467184-719f38bdd93f560145a4`을 checked300·official source·organic-only·adExcluded=true·stable proof로 commit했습니다. cursor는 같은 sort2100 안에서 created_at 순방향이고 cycle #29 group45/45·tracker58/58·terminal58/58·open0으로 중복0입니다. terminal 뒤 processing·lane/run/lease/probe null·circuit closed, streak39·baseline10·candidate false입니다. 잔여 partial 앞에 자연 group12개, 전체 순번13/14·14/14가 남았습니다.
- event7481→7483도 21:49:43→21:51:15 KST collection `pw-chrome-1787403075058-59872a060c9b0f8f2946`을 checked300·official source·organic-only·adExcluded=true·stable proof·광고45건 제외로 commit했습니다. cursor sort2100→2200 순방향이고 cycle #29 group46/46·tracker59/59·terminal59/59·open0으로 중복0입니다. terminal 뒤 processing0·lane/run/lease/stage/tracker null·circuit closed·cooldown null, streak40·baseline10·candidate false입니다. 잔여 partial 2건은 여전히 claim/terminal/snapshot0이며 자연 순번12/13·13/13으로 대기합니다.
...
- 2026-08-23 구글식 일정 등록 창 배포: 제목·일정/할 일 탭(할 일 disabled)·종일 기본+시간 추가·시작~종료·반복 8종+맞춤·참석자 칩(+초대 메일)·Meet 토글·위치·설명·캘린더 선택(캐시 카탈로그). 연동 owner는 구글 우선 저장 후 MI 미러(반복=마스터 1개+인스턴스 수집, 이 일정만/모든 일정), 미연동은 legacy 로컬 확장 보존. sendUpdates 기본 none(다이얼로그만 명시). 마이그레이션 20260823090000(가산, 미적용 시 graceful). 테스트 853/853, 게이트 exit 0.
...
- 기능 commit `628e4ae0b2a9`을 GitHub `main`과 Production에 반영했고 `/health`·`/ready`가 같은 release와 Supabase ready를 반환했습니다. 운영 migration 3개를 overflow → transient recovery → runtime `1.1.9` 순서로 적용했고 Windows updater도 같은 commit·version으로 `loaded_extension_synced=true`, `native_host_registry_synced=true`를 반환했습니다.
- runtime heartbeat는 exact fingerprint `b89dcd5bb3ab88b3d6f0770640a67f8b94ff266910cef44c554758d5695b9832`입니다. migration 직후 cadence10·stability null·streak0으로 과거 proof가 초기화됐고, 기준 시각 뒤 별도 wake 요청이 없던 `normal` 실행은 2026-08-21 14:53:46 KST claim → 14:54:31 KST collection `pw-chrome-1787291671198-58f940f60934ff318d08`을 공식 collector·오가닉·광고 30개 제외·`checked_count=300`으로 저장했습니다. ledger에는 알람 trigger명이 없어 SELECT만으로 trigger 종류 자체까지 절대 증명하지는 않습니다. terminal 뒤 circuit closed, processing/lane/run/lease null입니다.
- 첫 성공으로 `stability_started_at=2026-08-21 14:54:31 KST`, success streak 1이 됐습니다. cycle #27 누적 group claim 28/서로 다른 group 28, tracker claim 40/서로 다른 tracker 40, commit 40, failure 0으로 같은 cycle 중복은 0입니다. cursor는 직전 sort1000 tracker에서 이번 claim의 sort1100 tracker로 일치해 전진했습니다. active 74·paused 0·격리 3·processing 0이며 cycle 잔여 34 tracker라 전체 cycle 완료는 아닙니다.
- 15:00 KST 다음 `normal` 회차도 event 6705→6707로 원자 300을 저장했습니다. collection `pw-chrome-1787292087822-95262b63bfac50c24be7`은 stable-full-window 증명, 공식 collector·오가닉·광고 30개 제외이며 terminal 뒤 success streak 2, circuit closed, processing/lane/run/lease null입니다. cycle #27 누적 group 29=distinct 29, tracker 41=distinct 41, commit 41, failure 0으로 중복 0을 유지합니다.
- 15:01:52 KST `rank-cron-cycle` wake 뒤 15:02:39 KST 회차와, 첫 14:53:46 KST claim에서 정확히 10분 뒤인 15:03:46 KST 회차가 가까이 이어졌습니다. 두 회차는 겹치지 않고 앞 terminal 15:03:25 뒤 21초 후 다음 claim이 시작됐으며 각각 원자300으로 끝났습니다. 뒤 90초간 즉시 5번째 실행은 없었습니다. 이 시각 관계는 cron wake와 baseline catch-up 인계에 부합하지만 ledger가 두 번째 trigger명을 저장하지 않아 해당 trigger 종류는 추론으로만 둡니다. 현재 streak4, cycle group 31=distinct31·tracker 44=distinct44·commit44·failure0, circuit closed·processing/lane/run/lease null이라 무한·병렬 실행 증거는 없습니다.
- 21:47 KST 누적 감사에서 cycle #27은 roster74·tracker claim74=distinct74로 전원 한 번씩 terminal에 도달했고, commit70+failure4=74 뒤 19:04 KST 완료됐습니다. group claim59=distinct59로 같은 cycle 중복은 0입니다. 실패는 `native_host_input_closed` 1건과 실제 검색 결과가 300 미만인 `provider_partial_window` 92·137·30건 각 1회이며 부분 snapshot은 0입니다. cycle #28은 19:09 KST 시작 뒤 group18=distinct18·tracker25=distinct25·commit25·failure0으로 순환 중입니다.
- 마지막 partial failure 때문에 후보 증거가 자동 초기화됐고 새 `stability_started_at`은 2026-08-21 19:10:08 KST, success streak는 18입니다. 따라서 가장 빠른 candidate8 가능 시각도 2026-08-22 19:10:08 KST 이후로 미뤄졌습니다. runtime 이후 snapshot 56행/46 collection은 checked300·official source·organic evidence/policy·adExcluded 위반 0, stable proof 18행, 민감 proof 저장 0입니다. 현재 active74·paused0·quarantine3·processing0, circuit closed·lane/run/lease null입니다.
...
- `2026년 8월` 같은 월 제목을 누르면 연도 이전·다음과 12개월을 고를 수 있는 접근 가능한 팝업이 열립니다. 월 선택은 조회 범위만 바꾸어 `GET /api/work-items`를 다시 호출하고 일정 생성·수정은 하지 않습니다. ESC·취소·포커스 이탈 시 닫힘도 보장합니다.
- TDD RED commit `a860df7`·`38424d7` 뒤 기능 commit `a451d0a`을 배포했습니다. 전체 `check:release`는 코어 631/631·플레이스 51/51·쇼핑 62/62·Production 인증 18/18, 공개 CSP build, N30 잠금 23함수·88파일·37 migration을 통과했습니다.
- Production `/health`·`/ready`는 release `a451d0ac803f`와 Supabase ready로 일치합니다. 로그인된 총관리자 대표실의 실측 너비는 달력 773px(0.700), 가까운 업무 331px(0.300)이고, 2025↔2026 연도 이동·10월 이동·8월 복귀·ESC 닫기를 확인했습니다. 기존 일정 2건은 그대로이며 생성·수정·삭제는 하지 않았습니다.

### 2026-08-20 대표실 개인 일정표 전환 Production 반영

- 대표 지시에 따라 새 공유 일정표 기능을 운영 화면에서 비활성화했습니다. 왼쪽 일정표 목록, 색상 선택, 공유 권한, 초대 코드, 코드 연결·나가기를 제거하고 월간 달력과 가까운 일정의 2열 구조로 넓혔습니다.
- 날짜 셀이나 `일정 추가`를 누르면 `개인 일정 등록` 패널이 열립니다. 기존 일정 유형·시작/종료·종일·상태·담당자·우선순위·내부 메모·광고주 공개와 서울 날짜 기준 매월 반복·포함 종료일은 유지합니다.
- `/api/work-items`와 admin/client 일정 조회·수정 경로는 `calendar_id IS NULL`인 개인 일정만 허용합니다. 공유 관련 action과 기존 shared ID의 생성·수정·삭제·음성 완료는 저장 전에 404로 종료합니다.
- 운영 DB는 읽기 전용으로 재확인했습니다. calendar/member/invite/shared schedule은 각각 0행이고 개인 일정 2행은 유지됩니다. 이미 적용된 공유 migration·테이블·RPC는 삭제하거나 되돌리지 않고 접근 불가능한 상태로 보존합니다.
- 로컬 대상 회귀 73/73, 전체 `check:release` 코어 629/629·플레이스 51/51·쇼핑 62/62·Production 인증 18/18, 공개 build CSP, 역할 상태·query parity, N30 보호 잠금 23함수·88파일·37 migration을 통과했습니다.
- 기능 commit `5ee9907`이 Production release `5ee99078a22c`로 반영됐고 `/health`·`/ready`와 Supabase ready가 일치합니다. 로그인된 총관리자 대표실에서 월간 달력+agenda 2열, 공유 UI 문구 0, `일정 추가`와 날짜 셀의 `개인 일정 등록` 패널, 매월 반복·광고주 공개 유지, calendar selector 0, browser warning/error 0을 확인했습니다. 운영 일정은 생성·수정·삭제하지 않았습니다.
...
- 공유 일정 쓰기는 membership과 calendar를 같은 DB transaction에서 잠그는 service-role 전용 RPC로 처리합니다. viewer·해제된 구성원·다른 calendar·stale `updated_at` 요청은 fail-closed하며, 광고주는 기존 일반 일정 수정 권한을 얻지 않습니다.
...
- 대표 지시(간결·프리미엄, 중복 기능 통합)로 대표실 레이아웃을 히어로→운영실→지표 스트립→실장 명령·대화 패널→일정표의 단일 리듬(18px)으로 정리했습니다. 실장의 "오늘 브리핑" 패널과 주간 목록을 제거하고 지표 4칸을 독립 스트립으로 승격, 앞 3칸은 기존 업무 요약과 동일한 `data-work-summary-filter` 속성을 부여해 클릭 시 아래 일정표 필터·활성 표시가 기존 위임 로직으로 그대로 동작합니다(클라이언트 무변경). 일정표 쪽 중복 요약 3칸은 embed 상태에서 숨기고 상단 헤어라인으로 구획했습니다.
- 검증: 새 viewHtml에 대해 클라이언트 계약·bind 필수 훅 10항목 재현 검사 전부 통과(에이전트/역할 6·음성 3버튼·입력/초안/결과/새로고침·지표 4+필터 3·태그 균형·인라인 핸들러 금지), viewHtml 합계 13,019/22,000·styleText 32,464/40,000, owner-tool 18/18. 게이트에서 server contract의 제품 문구("날짜가 확인되는 문장만") 누락 실패를 확인하고 계약 약화 없이 새 패널 설명에 복원해 전체 `npm run check:release` exit 0(인증 18/18)·`git diff --check` 통과. 시각 리듬 체감은 대표님 검수 대상입니다.

### 2026-08-20 대표실 통합 (업무 운영 + 실장)

- 대표 지시로 owner 세션에서 업무 운영 일정표와 실장 비서를 한 화면으로 합쳤습니다. 메뉴는 "운영 홈" 바로 아래 "대표실"(화면 id는 계약·테스트가 고정한 `owner-assistant` 유지, 라벨만 변경)이며, owner 페이로드 로드 시 정적 업무 운영 섹션을 대표실 하단으로 이동하고 owner의 기존 업무 운영 메뉴는 숨깁니다. 팀·광고주 세션은 페이로드가 없어 기존 그대로입니다.
- 이동은 복제가 아닌 단일 섹션 이동(root 전역 data-work-* 훅 중복 방지)이며, 마커 노드로 원위치를 기억해 `discardOwnerTool` 최상단에서 클래스(mi-view↔is-embedded)·메뉴 표시까지 복원합니다 — 탐사에서 확인한 "owner 뷰 제거 시 업무 운영 섹션 영구 소멸" 함정을 차단했습니다. 브라우저 실측 12/12(메뉴 위치·숨김·이동·클래스 교체·마커·재로드 멱등·복원 생존/위치/클래스/메뉴/마커 정리/앵커 제거) 통과.
- release 기준선 `adminMenuCount`(정적 소스 고유 스크린 13개)가 새 JS 셀렉터의 owner-assistant 리터럴로 실패하는 것을 확인하고, 기준선 약화 없이 셀렉터 인용부호 변경으로 해소했습니다(고유 13개 재확인). owner-tool 테스트 라벨 단정 1건 갱신 후 18/18, 전체 `npm run check:release` exit 0(인증 18/18), styleText 31,878/40,000, `bindOwnerAssistant` 해시 불변·파일 해시 갱신, CSP 교체, `git diff --check` 통과. 로그인 실화면의 대표실 렌더·전환 체감은 대표님 검수 대상입니다.

### 2026-08-20 실장 광고주 음성 전환·굿모닝 브리핑

- 승인된 계획(리스크 검증 9건 반영)대로 두 기능을 추가했습니다. ① "○○ 광고주로 바꿔줘/전환해줘"(호출어·상시 대기·타이핑 공통)로 owner 코드 API(`requestOwnerCodes` GET, 5분 캐시·ok 응답만 캐시)에서 활성 광고주를 이름·상호 부분일치로 찾아 스코프 입력을 바꾸고, `loadWorkItems` 성공(true 반환 신설)일 때만 전환 발화·실패 시 경고합니다. 0건·다중 일치는 이름 나열 안내, "내부/총관리자/내 일정으로" 복귀 지원, "내일 정기 미팅으로 변경" 같은 문장은 조사 필수 패턴으로 오발동을 차단합니다.
- ② 하루 첫 접속 시 `loadWorkItems` 성공 후에만 굿모닝 브리핑(시간대별 인사+오늘 일정)을 1회 발화하고 결과 피드에 카드(끄기 버튼, localStorage opt-out·날짜 게이트·차단 환경 off 폴백)를 남깁니다. placeholder 제거 로직을 공용 헬퍼로 추출해 채팅·인사 카드가 공유합니다.
- 실측: 전환 발화 5종 매치·비발동 6종(완료·브리핑·등록·무키워드·잡담·무공백 함정) null, 매칭 픽스처 1건/0건/비활성 제외/다중 2건, 굿모닝 게이트 off·같은 날 false·새 날 true — 전부 실제 코드 추출 실행값. 회귀 33/33(owner-tool 18+work-items 15), `bindOwnerAssistant` 해시 갱신, 서버 파일 무변경, CSP 교체, 전체 `npm run check:release`(인증 18/18), `git diff --check` 통과.
- 실마이크 전환 발화와 TTS 자동재생(브라우저가 첫 제스처 전 차단할 수 있어 이 경우 카드·상태만 표시)은 대표님 실화면 검수 대상입니다.

### 2026-08-20 프리미엄 톤 다운 (라이트 기조 복귀)

- 대표님 검수 피드백(색이 진함, 메인 홈페이지처럼 깔끔하게)에 따라 히어로와 음성 존의 딥네이비 블록을 라이트 화이트 카드로 되돌리고, 골드는 헤어라인·키커·활성 상태 점 표시 수준으로만 남겼습니다. 오피스 캔버스는 원래의 네이비를 유지하되 이번에 추가했던 골드 외곽선을 원래의 블루 톤으로 완화하고 그림자를 줄였습니다. 지표 스트립·패널 틴트·비서실장 골드 시그니처는 유지합니다.
- 측정값: styleText 31,748/40,000자, 서버 구문 OK, owner-tool 테스트 18/18, admin.html 무변경으로 CSP 해시 유지, 파일 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), `git diff --check` 통과. 육안 톤 검수는 대표님 확인 대상입니다.

### 2026-08-20 실장 화면 프리미엄 개편 1단계 (비주얼 폴리시)

- 대표님 승인 기획에 따라 실장 운영 비서 화면을 "딥네이비 이그제큐티브 라운지" 방향으로 리스타일했습니다. 로직 변경 없는 CSS 중심 개편입니다: 히어로를 네이비 그라디언트+샴페인 골드 밴드로, 지표 4칸을 헤어라인 구분 4열 계기판 스트립(30px tabular 숫자·대문자 자간 라벨·오늘 칸 골드 틴트)으로, 패널을 옅은 틴트+골드 헤더 바로 통합했습니다.
- 오피스는 하단 비네트·골드 틴트 외곽선·깊은 그림자를 더하고 스테이션을 유리(블러) 칩으로, 비서실장 캐릭터·스테이션·라벨을 샴페인 골드 시그니처로 바꿨으며 캐릭터 발밑 그림자를 추가했습니다. 음성 존은 네이비 글래스 스트립으로 승격해 기존 상태 클래스만으로 듣는 중 골드 파동(`mi-voice-pulse`)·상시 대기 골드 글로우+점 표시(`mi-voice-glow`, `:has` 컨테이너 강조)를 표현하고 reduced-motion에서 정지합니다.
- styleText가 상한에 근접해(잔여 1,084자) admin.html의 클라이언트 검증 상한을 30,000→40,000자로 상향했습니다(owner 전용 경계·나머지 캡 불변). 측정값: styleText 합계 32,450/40,000자, 클라이언트 상한 반영 확인, owner-tool 테스트 18/18, `bindOwnerAssistant` 해시 불변(파일 해시만 갱신), 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check` 통과. 실화면 육안 검수는 대표님 확인 대상입니다.

### 2026-08-20 실장 자유 대화(Anthropic API) 추가

- 대표님 승인(비용 발생 허용, 최소 비용 지향)에 따라 실장에 자유 대화를 추가했습니다. 기존 의도(브리핑·완료·등록)에 걸리지 않는 단일 자유 문장만 owner 전용 `assistant-chat` 액션으로 전달되며, 상시 마이크의 비호출 발화·여러 줄 메모는 외부로 전송하지 않습니다. 화면 배지·상태 문구를 "대화만 Anthropic API로 전송(학습 미사용)"으로 정직하게 교체했습니다.
- 서버는 `@anthropic-ai/sdk` 0.120.0(정확 버전 고정, 공급망 게이트 통과)으로 기본 `claude-haiku-4-5`(env `MI_ASSISTANT_CHAT_MODEL`로 교체 가능), max_tokens 700, 도구 없이 클라이언트가 보낸 열린 일정 스냅샷(최대 60건)을 시스템 프롬프트로 사용합니다. 실장은 등록·완료를 직접 실행하지 못하고 기존 음성 명령 방법을 안내하며, 응답은 카드 표시+음성 읽기, 대화 기록은 브라우저 내 12턴 유지입니다.
- 주입식 실측 6경로(키 없음 503 missing_api_key·빈 메시지 400·history 검증 400·정상 200 usage 매핑·429 전파·502 기본)와 owner-tool 테스트 18/18(신규 7블록: 모델 덮어쓰기·스냅샷 60건 상한 포함), 전체 `npm run check:release`(인증 18/18), 보호 잠금 2건 갱신, CSP 해시 교체, `git diff --check`를 통과했습니다. 브리핑 발화는 대표님 지시로 "…입니다"로 끝나도록 문형을 바꿨고 실코드 추출 실측으로 확인했습니다.
- Production은 `ANTHROPIC_API_KEY` Vercel 환경변수 설정 전까지 대화 시도 시 미연결 안내만 반환합니다(다른 기능 무영향). 키 설정과 실화면 대화·음성 검증은 대표님 확인 대상입니다.

### 2026-08-20 브리핑 의도 군말 허용 보강

- 대표님 실화면에서 "다음 주 일정 좀 알려 주세요"가 들림으로만 남는 결함을 확인했습니다. 원인은 브리핑 의도 정규식이 `일정`과 동사 사이의 "좀" 같은 군말을 허용하지 않아 판정에 실패한 것입니다("브리핑" 단어가 있을 때만 통과).
- 의도 패턴을 일정/업무(들) + 조사 + 군말(좀·한 번·다시·간단히·짧게) + 동사(알려·읽어·들려·말해·정리해)로 보강했습니다. 실제 파일에서 정규식을 추출해 측정한 결과 발동 6/6(대표님 발화 포함), 비발동 5/5(미루기·변경·등록 문장 오발동 없음)입니다.
- 변경이 잠금 함수 밖 헬퍼 상수라 보호 잠금 갱신은 없으며, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check`를 통과했습니다.

### 2026-08-20 범위 지정 음성 브리핑·주간 일정표

- 대표님 지시로 음성 브리핑을 범위 지정형으로 바꿨습니다. "다음 주 업무 브리핑 해주세요"처럼 다음 주/이번 주/내일/모레/다가오는 범위를 해석하고, 발화 내용은 건수 없이 해당 범위 일정을 "8월 25일 화요일 오전 10시 월간 보고서 검수" 형식으로 나열합니다(완료 제외·최대 8건·종일 일정은 시간 생략, 없으면 "○○ 일정이 없습니다"). 브리핑 읽기 버튼은 기본 오늘 범위입니다.
- 오늘 브리핑 아래 목록을 날짜 헤더가 있는 주간 일정표(오늘~7일, 최대 10건)로 확장했습니다. 오늘 헤더는 항상 표시하고 일정이 없으면 "일정 없음"을 명시하며, 각 행의 완료 버튼은 유지됩니다.
- 검증은 admin.html에서 실제 함수 블록을 추출 실행해 측정했습니다: 범위 파싱 4/4, 다음 주 창 2026-08-24~30 정확, 발화 3종(다음 주 2건 나열·오늘 빈 범위·내일 1건) 기대 일치, done·범위 밖 제외 확인. `bindOwnerAssistant` 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check` 통과. 실화면 음성 재검수는 대표님 확인 대상입니다.

### 2026-08-20 음성 명령 확정 인식 후 발사로 교정

- 대표님 실화면에서 "브리핑 해줘"가 첫 음절 "브"만 입력된 채 실행되는 결함을 확인했습니다. 원인은 호출어 경로가 잠정(interim) 인식 결과에서 즉시 발사되어, "실장님 브…" 시점에 명령을 "브"로 확정해버린 것입니다.
- 호출어 명령·직접 명령 모두 확정(final) 인식 결과에서만 발사하도록 통일했습니다. 잠정 결과는 들림 표시와 "네, 듣고 있습니다" 안내에만 쓰고, 문장이 확정되면 호출어 유무에 따라 전체 명령으로 실행합니다.
- 이벤트 시퀀스 재현 5/5 일치: 스크린샷 케이스(잠정 "실장님 브" 미발사 → 확정 "실장님 브리핑 해 줘"에서 전체 명령 발사), 호출어 없는 직접 브리핑, 잡담 확정 미발사, 실장님-명령 분리 발화, 완료 직접 명령. `bindOwnerAssistant` 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check` 통과. 실제 마이크 재검수는 대표님 확인 대상입니다.

### 2026-08-20 상시 대기 직접 명령 실행

- 대표님 실화면에서 상시 대기가 "브리핑 해 줘 …"를 들림으로만 표시하고 실행하지 않는 문제를 확인했습니다. транскript에 `실장` 토큰 자체가 없어(음성엔진 누락) 호출어 의존이 실패한 것이 원인입니다.
- 상시 대기 중에는 호출어 없이도 확정(final) 인식 결과가 실행 가능한 명령이면 바로 실행하도록 바꿨습니다. 실행 대상은 브리핑 의도와 문장 끝 완료 명령으로 한정하고, 일반 대화·등록형(날짜) 문장은 계속 호출어나 타이핑을 요구합니다. 브리핑 음성 출력 중에는 새 명령을 받지 않아 스피커 소리 재유입 재발동을 차단하며, 완료 처리의 확인창은 유지됩니다.
- 측정값 10/10 일치: 대표님 실제 발화 "브리핑 해 줘"·"다가오는 업무 브리핑 해 줘" 실행 판정, "오늘 일정 알려줘"·완료문 2종 실행, 잡담·등록문·"완료된 것 같은데 확인해봐" 등 5종 비발동. `bindOwnerAssistant` 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check` 통과. 실제 마이크 재검수는 대표님 확인 대상입니다.

### 2026-08-20 실장 상시 호출 대기 전환

- 대표님 지시로 "버튼을 먼저 누르고 부르는" 30초 유한 대기를 "한 번 켜두면 실장님 호출만으로 실행되는" 상시 대기 토글로 바꿨습니다. 설정은 브라우저 localStorage(`mi-owner-assistant-standby`)에 기억되어 다음 접속에서 자동 재개되고, 버튼은 켜기/끄기 토글로 동작합니다.
- 안전 경계: 상시 인식은 owner 화면이 활성이고 탭이 보일 때만 세션을 만들며(`document.hidden`이면 대기), 화면 이탈·로그아웃은 기존 voice controller 해제 경로로 정지하고, 마이크 권한 거부 시 설정을 자동으로 끕니다. 명령 실행 뒤에는 새 인식 세션으로 재시작해 과거 발화가 재발동하지 않습니다. 기존 "백그라운드 무한 재시작 없음" 기준은 대표님 지시로 이 경계 안의 상시 재시작으로 대체합니다.
- 구 정책을 단정하던 음성 테스트를 새 불변식(명시 opt-in 키·토글 양방향·visibility 게이트·standbyLoop·권한 거부 자동 해제·30초 타이머 부재)으로 교체해 owner-tool 11/11을 유지했습니다. `bindOwnerAssistant`·`owner-assistant-server-surface` 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check`를 통과했습니다. 실제 마이크 상시 인식·자동 재개는 대표님 재검수 대상입니다.

### 2026-08-20 실장 음성 브리핑 명령·호출어 누적 인식 수정

- 대표님 실화면 재검수에서 "실장님 …" 발화가 `호출 대기 중 · 들림:`으로만 남는 결함을 확인했습니다. 원인은 wake 인식이 `event.resultIndex` 이후 조각만 검사해 앞 조각의 호출어가 유실되는 것으로, 30초 세션의 전체 누적 결과를 검사하도록 수정했습니다. 이제 "실장님"과 명령을 나눠 말해도 이어집니다.
- "브리핑 좀 해줘"류 명령이 아무 동작도 하지 않던 공백을 메웠습니다. 브리핑 의도(브리핑/오늘·다가오는 일정·업무 알려·읽어·들려·말해)를 실행 경로 초입에서 감지해 기존 `브리핑 읽기`를 실행하며, 음성·타이핑 공통입니다. 읽기 내용에 `다가오는 업무 N건`을 추가했습니다.
- 측정값: 브리핑 의도 발동 5/5(대표님 실제 발화 "최근 다가오는 업무 브리핑 좀 해 주세요" 포함), 비발동 4/4(완료·등록 문장 오발동 없음), 누적 транскript 재현 케이스 매칭·라우팅 일치. `bindOwnerAssistant` 잠금 해시 갱신, 전체 `npm run check:release`(인증 18/18), CSP 해시 교체, `git diff --check` 통과. 실제 마이크 발화 인식은 대표님 재검수 대상으로 남깁니다.

### 2026-08-20 브리핑 오늘 일정 목록·실장님 호출 인식 보완

- 대표님 실화면 검수 피드백을 반영해 오늘 브리핑 목록을 우선순위 혼합(지연+오늘+내일)에서 오늘 시작 일정만 시간순 최대 8건으로 바꾸고 빈 상태 문구를 `오늘 일정이 없습니다.`로 정리했습니다. 지표 카드는 기존 범위를 유지하며 안내 문구에 목록 기준을 명시했습니다.
- 호출어 정규식이 `실장` 뒤 호칭을 허용하지 않아 "실장님"이 인식되지 않던 결함을 수정했습니다. 새 패턴은 문장 경계의 `실장/실장님/실장아`만 호출로 보고, 인식 후에는 입력만 채우던 기존 동작 대신 250ms 뒤 초안 실행을 자동 시작합니다(완료 처리는 기존 확인창 승인 유지). 측정값: 호출 8케이스 전부 기대 일치 — "실장님 테스트 일정 완료로 해줘"→명령 추출, 단독 "실장님"→대기 응답, "어제 실장님이 말했던 것"·"시장님 안녕하세요"→비발동.
- 보호 잠금 `owner-assistant-interaction`·`owner-assistant-server-surface` 해시를 대표님 지시 범위에서 재갱신했고, 잠금 검사, 전체 `npm run check:release`(Production 인증 18/18), CSP script-src 해시 교체, `git diff --check`를 통과했습니다. 실제 마이크 인식과 로그인 화면 동작은 자동검사로 대체하지 않으며 대표님 재검수 대상으로 남깁니다.

### 2026-08-20 비서 호칭 실장 전환·말로 하는 일정 완료 명령

- 대표님 지시로 총관리자 비서 호칭을 자비스에서 실장으로 전환했습니다. 메뉴 `실장 운영 비서`, 조직 카드 `비서실장`과 5개 `담당` 라벨, 음성 호출어(`실장`)와 상태 문구까지 서버 전달 surface와 화면 문자열 전체를 바꿨고 두 파일 모두 `자비스` 잔존 0건을 확인했습니다. 내부 식별자(`owner-assistant` screen·data 속성·감사 로그 액션)는 유지합니다.
- 입력창·음성으로 "○○ 완료로 해줘"류 문장을 말하면 해당 미완료 업무를 완료 처리하는 명령을 추가했습니다. 해석은 서버 `parseAssistantCompletion`이 담당하며 날짜·시간이 파싱되는 문장은 기존 초안 경로가 우선이라 기존 등록 동작은 변하지 않습니다. 화면은 응답의 `completions`를 받아 제목 부분일치로 대상을 찾고, 정확히 1건일 때만 확인창 승인 뒤 기존 `assistant-complete` 낙관적 잠금 경로로 완료 처리합니다. 0건·복수 일치는 저장 없이 안내만 남깁니다.
- 보호 잠금은 대표님 명시 지시에 따라 `owner-assistant-server-surface`(파일)와 `owner-assistant-interaction`(bindOwnerAssistant) 두 해시를 갱신했고 잠금 검사·self-test를 통과했습니다. 파서 실측값(광고주 미팅/주간 보고서/광고 세팅 추출, 비완료 문장 비발동, 혼합 입력 초안1+완료1)을 확인했으며 owner-tool 11/11, work-items 15/15, 전체 `npm run check:release`(Production 인증 18/18), CSP 해시 교체, `git diff --check`를 통과했습니다. 실행 테스트값 검증 전 커밋·배포하지 않는다는 지시를 준수했습니다.
- 로그인된 `mml93-a01` 실화면에서 음성 호출어 "실장" 인식과 완료 명령 1건 실사용은 자동검사로 대체하지 않으며 미검증으로 남깁니다.
...
- 조직·음성 보완 commit `df4089405a77`을 Production에 반영했습니다. owner assistant 8/8, API·서버 550/550, 플레이스 51/51, 쇼핑 62/62, 역할 상태·query parity, server contract 56/56, Production 인증 18/18, 공개 CSP, 보호 잠금과 전체 `npm run check:release`, `git diff --check`를 통과했습니다. 앱 Browser가 localhost를 `ERR_BLOCKED_BY_CLIENT`로 차단해 실제 렌더 캡처와 마이크 권한 허용 실기는 확보하지 못했으므로, 이를 자동검사로 대체해 정상이라고 보고하지 않습니다.
...
- 19:06 KST 조직·음성 보완 release `df4089405a77`의 `/health`·`/ready` 일치와 Supabase ready를 확인했습니다. `/admin`·`/client` 200, 비인증 `/api/owner/tool` 401, 정적 owner-assistant DOM 0, Production `Permissions-Policy`의 `microphone=(self)`·`camera=()`도 확인했습니다. 이는 exact-owner 서버 경계와 배포 증거이며 로그인된 `mml93-a01`의 조직 육안 렌더·실제 음성 인식/읽기·일정 저장 증거는 아닙니다.
...
- 11:10 KST Production `68e6200ad826` ready와 세 migration 적용을 확인했습니다. 공개 worker 함수는 SECURITY INVOKER·빈 search path·postgres/service_role 전용이고, 내부 snapshot 장부 trigger만 service-role ledger 기록을 위해 SECURITY DEFINER·빈 search path·postgres 전용입니다. 이번 세 migration의 신규 함수 관련 advisor 경고는 없습니다.
...
- 13:09 KST v1.1.8 이후 자연 작업은 14 distinct group/19 tracker 전부 commit, failure·open claim·중복 0입니다. collection 14건은 일반 10·stable proof 4이며 snapshot 19건의 atomic300/source/evidence/policy/adExcluded 위반과 proof 민감정보 저장은 0입니다. cycle #10 누적 18 group/25 tracker, commit 23·fail 2(모두 v1.1.8 전)이고 9/9 agency가 진입했습니다. 잔여 stale/never 17건은 과거 page-overlap 16·partial 1이며 모든 eligible 대기는 현재 cursor 앞에 있어 starvation 징후가 없습니다.
- 13:12 KST SELECT-only 재확인은 cycle #10 누적 19 distinct group/26 tracker, group·tracker 중복 0, 9/9 agency 진입을 유지했습니다. v1.1.8 이후 14 group/19 tracker는 failure 0·전부 commit, stable proof는 5건으로 늘었고 snapshot 19건의 원자 계약 위반·proof 민감정보 저장은 0입니다. stale24는 16건(과거 page-overlap 15·partial 1)으로 감소했으며 pending 46건은 모두 cursor 앞(ahead 46, behind/equal 0)입니다. heartbeat·circuit·processing·lane/run/lease는 정상 해제 상태입니다.
- 13:23 KST 다음 자연 회차까지 cycle #10은 20 distinct group/28 tracker, commit 26·fail 2(모두 v1.1.8 전), group·tracker 중복 0으로 전진했습니다. v1.1.8 이후 15 group/21 tracker가 failure 0으로 전부 commit했고 stable proof는 7건입니다. snapshot 21건/15 collection의 checkedCount/source/evidence/policy/adExcluded 위반과 proof 민감정보 저장은 0이며, pending 44건은 모두 cursor 앞(ahead 44, behind/equal 0)입니다. stale24 13·stale48 11로 감소했고 runtime heartbeat 38초·circuit closed·failure streak 0·processing/lane/run/lease null입니다.
- 13:30 KST 다음 `normal` group은 tracker 4건을 collection `pw-chrome-1786768255133-3dde14b9fe04774cacee` 하나로 함께 원자 300 commit했습니다. cycle #10 누적 21 distinct group/32 tracker·commit 30·fail 2, group·tracker 중복 0이며 pending 40건은 모두 cursor 앞(ahead 40, behind 0)입니다. v1.1.8 이후 16 group/25 tracker는 failure 0·전부 commit했고 snapshot 25건/16 collection의 원자·source·evidence·policy·광고 제외 위반은 0입니다. terminal heartbeat 22초·success streak 17·circuit closed·processing/lane/run/lease null입니다.
- 13:40 KST 다음 `normal` group도 collection `pw-chrome-1786768855262-ee75e6a14f5ba82eb91a`으로 원자 300 commit했습니다. cycle #10 누적 22 distinct group/33 tracker·commit 31·fail 2, group·tracker 중복 0이고 pending 39건은 모두 cursor 앞(ahead 39, behind 0)입니다. v1.1.8 이후 17 group/26 tracker는 failure 0·전부 commit했으며 snapshot 26건/17 collection의 원자 계약 위반은 0입니다. cursor는 sort 600으로 전진했고 circuit closed·processing/lane/run/lease null입니다.
- 13:50 KST 다음 `normal` group은 collection `pw-chrome-1786769451798-1ded6bcb4c6ca3e4df61`으로 원자 300 commit하고 과거 격리를 해제했습니다. cycle #10 누적 23 distinct group/34 tracker·commit 32·fail 2, 중복 0이며 v1.1.8 이후 18 group/27 tracker는 failure 0·전부 commit했습니다. pending 38건 중 eligible 37건은 cursor 앞이고, behind 1건은 cycle 시작 시부터 `provider_partial_window:37_300`으로 20:10 KST까지 격리된 roster 항목이라 starvation이 아닙니다. snapshot 27건/18 collection의 원자 계약 위반은 0이고 circuit closed·processing/lane/run/lease null입니다.
- 14:00 KST `normal` group의 tracker 2건이 stable full-window proof로 collection `pw-chrome-1786770091632-09cdcd5bf1f622bd6789`을 함께 원자 300 commit하고 과거 격리를 해제한 뒤, 6초 handoff된 다음 `normal` group도 collection `pw-chrome-1786770144536-d136456beb36186815a2`로 원자 300 commit했습니다. cycle #10 누적 25 distinct group/37 tracker·commit 35·fail 2, 중복 0입니다. v1.1.8 이후 failure 0·snapshot 30건/20 collection 원자 위반 0·stable proof 9건이며, terminal processing/lane/run/lease null입니다. stale24 11·stale48 9로 감소했고 pending 35건 중 eligible ahead 34·의도적 partial 격리 behind 1입니다.
- 14:10 KST 다음 `normal` group도 collection `pw-chrome-1786770651385-b1aba05e97f51cec8b1f`으로 원자 300 commit하고 과거 격리를 해제했습니다. cycle #10 누적 26 distinct group/38 tracker·commit 36·fail 2, 중복 0이고 cursor sort 800으로 전진했습니다. v1.1.8 이후 21 group/31 tracker는 failure 0·전부 commit했으며 snapshot 31건/21 collection의 원자 계약 위반은 0입니다. pending 34건 중 eligible ahead 33·의도적 partial 격리 behind 1, circuit closed·processing/lane/run/lease null입니다.
- 14:20 KST 다음 `normal` group은 stable full-window proof로 collection `pw-chrome-1786771295620-94ae1fe3e4cbc41eda76`을 원자 300 commit하고 과거 격리를 해제했습니다. cycle #10 누적 27 distinct group/39 tracker·commit 37·fail 2, 중복 0이고 cursor sort 900으로 전진했습니다. v1.1.8 이후 22 group/32 tracker는 failure 0·전부 commit했고 stable proof는 10건입니다. pending 33건 중 eligible ahead 32·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 14:30 KST 다음 `normal` group도 stable full-window proof로 collection `pw-chrome-1786771895357-8bc42a99e9228a94d128`을 원자 300 commit했습니다. cycle #10 누적 28 distinct group/40 tracker·commit 38·fail 2, 중복 0이고 cursor sort 1000으로 전진했습니다. v1.1.8 이후 23 group/33 tracker는 failure 0·전부 commit했고 stable proof는 11건입니다. pending 32건 중 eligible ahead 31·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 14:40 KST 다음 `normal` group도 stable full-window proof로 collection `pw-chrome-1786772493859-1f064889f300d319038f`을 원자 300 commit했습니다. cycle #10 누적 29 distinct group/41 tracker·commit 39·fail 2, 중복 0이고 cursor sort 1100으로 전진했습니다. v1.1.8 이후 24 group/34 tracker는 failure 0·전부 commit했고 stable proof는 12건입니다. pending 31건 중 eligible ahead 30·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 14:50 KST 다음 `normal` group도 stable full-window proof로 collection `pw-chrome-1786773096179-9cd9ba66aaaf7ee3cf2a`을 원자 300 commit하고 과거 격리를 해제했습니다. cycle #10 누적 30 distinct group/42 tracker·commit 40·fail 2, 중복 0이고 cursor sort 1200으로 전진했습니다. v1.1.8 이후 25 group/35 tracker는 failure 0·전부 commit했고 stable proof는 13건입니다. stale24는 10건으로 감소했으며 pending 30건 중 eligible ahead 29·partial 격리 behind 1, circuit closed·processing/lane/run/lease null입니다.
- 14:57~15:02 KST `normal` 3개 group이 연속 완료됐습니다. 첫 group의 tracker 2건은 stable proof collection `pw-chrome-1786773506738-693d8be7eefda73bfb46`, 두 번째는 stable proof `pw-chrome-1786773694165-5e42157db6339d76fb51`, handoff된 세 번째는 일반 collection `pw-chrome-1786773746144-f51acaa322dff498d84e`로 모두 원자 300 commit했습니다. cycle #10 누적 33 distinct group/46 tracker·commit 44·fail 2, 중복 0입니다. v1.1.8 이후 28 group/39 tracker failure 0·snapshot 39건/28 collection 원자 위반 0·stable proof 16건이며, stale24·stale48 각 9건, pending eligible ahead 25·partial 격리 behind 1, terminal lane/run/lease null입니다.
- 15:10 KST 다음 `normal` group은 일반 collection `pw-chrome-1786774251791-13e60ebe40d19694c5a9`으로 원자 300 commit했습니다. cycle #10 누적 34 distinct group/47 tracker·commit 45·fail 2, 중복 0이고 cursor sort 1500으로 전진했습니다. v1.1.8 이후 29 group/40 tracker는 failure 0·전부 commit했고 snapshot 40건/29 collection의 원자 계약 위반은 0입니다. pending 25건 중 eligible ahead 24·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 15:20 KST 다음 `normal` group도 일반 collection `pw-chrome-1786774851817-6b0ae6da270ee17ecbba`로 원자 300 commit했습니다. cycle #10 누적 35 distinct group/48 tracker·commit 46·fail 2, 중복 0입니다. v1.1.8 이후 30 group/41 tracker는 failure 0·전부 commit했고 snapshot 41건/30 collection의 원자 계약 위반은 0입니다. pending 24건 중 eligible ahead 23·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 15:30 KST 다음 `normal` group도 일반 collection `pw-chrome-1786775453043-e4384a21c56d08c1d4fe`로 원자 300 commit했습니다. cycle #10 누적 36 distinct group/49 tracker·commit 47·fail 2, 중복 0이고 cursor sort 1600으로 전진했습니다. v1.1.8 이후 31 group/42 tracker는 failure 0·전부 commit했고 snapshot 42건/31 collection의 원자 계약 위반은 0입니다. pending 23건 중 eligible ahead 22·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 15:40 KST 다음 `normal` group은 stable full-window proof로 collection `pw-chrome-1786776094067-10d21d25c0454f0d6cc6`을 원자 300 commit했습니다. cycle #10 누적 37 distinct group/50 tracker·commit 48·fail 2, 중복 0이고 cursor sort 1700으로 전진했습니다. v1.1.8 이후 32 group/43 tracker는 failure 0·전부 commit했고 stable proof는 17건입니다. pending 22건 중 eligible ahead 21·partial 격리 behind 1이며, circuit closed·processing/lane/run/lease null입니다.
- 15:50 KST 다음 `normal` group은 collection `pw-chrome-1786776652706-535a984d595ca113f3f5`으로 원자 300 commit했습니다. cycle #10 누적 38 distinct group/51 tracker·commit 49·fail 2, 중복 0입니다. v1.1.8 이후 snapshot 44건/33 collection의 원자 계약 위반·민감 proof 저장·failure는 모두 0이고 success streak 34입니다. pending 21건 중 eligible 20·partial 격리 1이며, circuit closed·processing/lane/run/lease null입니다.
- 15:54·16:00 KST `normal` group 2건이 각각 collection `pw-chrome-1786776926673-b13f80e6214bd7e2ef59`(stable proof)·`pw-chrome-1786777253128-0d106413bcbb9d9b5c49`으로 원자 300 commit했고 과거 격리도 정상 해제됐습니다. cycle #10 누적 40 distinct group/53 tracker·commit 51·fail 2·중복 0, cursor sort 1800입니다. v1.1.8 이후 snapshot 46건/35 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 19건 중 eligible 18·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 16:10~16:12 KST `normal` group 2건이 collection `pw-chrome-1786777889788-d719d4e02098384ee428`(stable proof)·`pw-chrome-1786777941855-e390b61be3f8b0ad649a`으로 원자 300 commit했고 과거 격리 1건도 해제됐습니다. cycle #10 누적 42 distinct group/55 tracker·commit 53·fail 2·중복 0, cursor sort 1900입니다. v1.1.8 이후 snapshot 48건/37 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 17건 중 eligible 16·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
...
- 16:56·17:00 KST `normal` group 2건이 collection `pw-chrome-1786780648643-c7bf2f714efe9de4162f`(stable proof)·`pw-chrome-1786780852044-d54c93cd9ca8e16ce82a`으로 원자 300 commit했고 과거 격리도 해제됐습니다. cycle #10 누적 48 distinct group/61 tracker·commit 59·fail 2·중복 0, cursor sort 2400입니다. v1.1.8 이후 snapshot 54건/43 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 11건 중 eligible 10·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:10 KST 다음 `normal` group은 stable proof collection `pw-chrome-1786781496380-f3d7ad9a529899eb72f0`으로 원자 300 commit했습니다. cycle #10 누적 49 distinct group/62 tracker·commit 60·fail 2·중복 0, cursor sort 2600입니다. v1.1.8 이후 snapshot 55건/44 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 10건 중 eligible 9·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:20 KST 다음 `normal` group은 collection `pw-chrome-1786782051055-c62e6458df42eb8b37bd`으로 원자 300 commit했고 과거 격리도 해제됐습니다. cycle #10 누적 50 distinct group/63 tracker·commit 61·fail 2·중복 0, cursor sort 2700입니다. v1.1.8 이후 snapshot 56건/45 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 9건 중 eligible 8·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:30 KST 다음 `normal` group은 tracker 3건을 collection `pw-chrome-1786782652392-e1fcbd219e84859d6b67` 한 번으로 각각 원자 300 commit해 브라우저 중복 수집이 없었습니다. cycle #10 누적 51 distinct group/66 tracker·commit 64·fail 2·중복 0, cursor sort 2800입니다. v1.1.8 이후 snapshot 59건/46 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 6건 중 eligible 5·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:40 KST 다음 `normal` group은 collection `pw-chrome-1786783252770-4d2b11c7b52361ccece5`으로 원자 300 commit했습니다. cycle #10 누적 52 distinct group/67 tracker·commit 65·fail 2·중복 0, cursor sort 3200입니다. v1.1.8 이후 snapshot 60건/47 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 5건 중 eligible 4·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:50 KST 다음 `normal` group은 collection `pw-chrome-1786783851708-ffd804f4643b88e08c9a`으로 원자 300 commit했습니다. cycle #10 누적 53 distinct group/68 tracker·commit 66·fail 2·중복 0, cursor sort 3300입니다. v1.1.8 이후 snapshot 61건/48 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 4건 중 eligible 3·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 17:59·18:00 KST `normal` group 2건이 collections `pw-chrome-1786784388968-c5d056c1608873d156e5`·`pw-chrome-1786784450244-6f22e02bb1a786426ca3`으로 각각 원자 300 commit했습니다. cycle #10 누적 55 distinct group/70 tracker·commit 68·fail 2·중복 0, cursor sort 3600입니다. v1.1.8 이후 snapshot 63건/50 collection의 원자 계약 위반·민감 proof 저장·failure는 0이며 pending 2건 중 eligible 1·partial 격리 1, circuit closed·processing/lane/run/lease null입니다.
- 18:10 KST cycle #10의 마지막 eligible `normal` group도 collection `pw-chrome-1786785050384-fc0a26bbf7edade775e1`으로 원자 300 commit했습니다. 누적 56 distinct group/71 tracker·commit 69·fail 2·중복 0, cursor sort 3700이며 미claim은 시작 시 partial 격리 1건뿐입니다. v1.1.8 이후 snapshot 64건/51 collection의 원자 계약 위반·민감 proof 저장·failure는 0이고 circuit closed·processing/lane/run/lease null입니다. 24시간 기준 18:17:51 KST 이후 cycle terminal/다음 cycle 시작을 별도 확인합니다.
- 24시간 기준을 지난 18:20:08 KST cycle #10이 `completed`로 전이됐습니다. roster 57 group/72 tracker/9 agency 중 시작 격리 1 tracker는 claim 0, eligible 56 group/71 tracker는 전부 정확히 1회 claim됐고 9/9 agency가 full coverage입니다. priority는 new 1→resume 1→normal 54, group/tracker 중복 0, terminal은 commit 69+fail 2=71로 open/multiple terminal 0입니다. commit snapshot 누락·checkedCount/source/evidence/policy/adExcluded 위반은 모두 0이며 cycle 완료 전 #11 시작도 0입니다. #11은 완료 뒤 18:30:07 KST에 시작됐습니다.
...
- 20:00 KST `normal` group 1건은 `provider_duplicate_identity:8:3:page_overlap:7`로 fail-closed·30분 격리됐습니다. 새 snapshot은 저장하지 않았고 lane·lease를 해제해 다음 순서를 막지 않았습니다. cycle #8 누적 28 group/39 tracker/9 agency, 같은-cycle group 중복 0, 복구 기준 atomic 위반 0입니다.
- 20:10 KST `normal` group 1건은 실제 오가닉 결과가 37개뿐인 `provider_partial_window:37_300`으로 fail-closed됐습니다. 원자 300을 만들 수 없어 snapshot을 저장하지 않고 해당 tracker만 24시간 격리했으며 global circuit·lane은 열지 않았습니다. cycle #8 누적 29 group/40 tracker/9 agency, atomic 위반 0입니다.
- 20:20 KST 다음 `normal` group 1건은 collection `pw-chrome-1786706452021-8acced7cdd80938c4b99`로 광고 30개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 30 group/41 tracker/9 agency, 복구 이후 18 snapshot/11 collection이며 같은-cycle group 중복과 atomic 위반은 0입니다.
- 20:24~20:25 KST 다음 `normal` group의 tracker 2건은 동일한 `provider_duplicate_identity:8:11:page_overlap:5`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 31 group/43 tracker/9 agency로 전진했습니다. 남은 26 group/29 tracker는 모두 현재 claim 가능하며 같은-cycle group 중복과 복구 기준 atomic 위반은 0입니다.
- 20:30 KST 다음 `normal` group은 collection `pw-chrome-1786707053432-590e86740dae1418fb35`로 광고 45개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 32 group/44 tracker/9 agency, 남은 25 group/28 tracker는 모두 claim 가능하며 복구 이후 19 snapshot/12 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 20:34·20:40 KST `normal` group 2건이 각각 collection `pw-chrome-1786707287004-95991c7e1070c3386a5f`, `pw-chrome-1786707651150-deaccaabf30aeb84478a`로 광고 제외 오가닉 300개를 완료했습니다. cycle #8 누적 34 group/46 tracker/9 agency, 남은 23 group/26 tracker는 모두 claim 가능하고 stale24는 24→23으로 감소했습니다. 복구 이후 21 snapshot/14 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 20:50 KST 다음 `normal` group이 collection `pw-chrome-1786708252428-38ed0c1d7caa30d32f7d`로 광고 45개 제외·오가닉 300개를 완료하고 과거 격리도 해제했습니다. cycle #8 누적 35 group/47 tracker/9 agency, 남은 22 group/25 tracker는 모두 claim 가능하고 stale24는 22로 감소했습니다. 복구 이후 22 snapshot/15 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 21:00 KST 다음 `normal` group이 collection `pw-chrome-1786708851890-591f33f253fc7c120777`로 광고 30개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 36 group/48 tracker/9 agency, 남은 21 group/24 tracker는 모두 claim 가능하며 복구 이후 23 snapshot/16 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 21:10~21:11 KST 다음 `normal` group은 `provider_duplicate_identity:5:2:page_overlap:4`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 37 group/49 tracker/9 agency로 전진했습니다. 남은 20 group/23 tracker는 모두 현재 claim 가능하며 같은-cycle group 중복·atomic 위반은 계속 0입니다.
- 21:20 KST 다음 `normal` group의 tracker 2건은 collection `pw-chrome-1786710053743-2fcfd19ee724be222a62` 하나로 광고 30개 제외·오가닉 300개를 함께 완료하고 과거 격리도 해제했습니다. cycle #8 누적 38 group/51 tracker/9 agency, 남은 19 group/21 tracker는 모두 claim 가능하며 stale24는 20·stale48은 19로 감소했습니다. 복구 이후 25 snapshot/17 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 21:27 KST `normal` group 1건은 `provider_duplicate_identity:7:2:page_overlap:6`로 fail-closed·30분 격리됐고, 이어 21:30 KST 다음 `normal` group은 collection `pw-chrome-1786710653940-f8f1eb59a3be72619b04`로 광고 30개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 40 group/53 tracker/9 agency, 남은 17 group/19 tracker는 모두 claim 가능하며 복구 이후 26 snapshot/18 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 21:40 KST 다음 `normal` group은 collection `pw-chrome-1786711250309-f2e58bc8be440b01e285`로 광고 45개 제외·오가닉 300개를 완료하고 과거 격리도 해제했습니다. cycle #8 누적 41 group/54 tracker/9 agency, 남은 16 group/18 tracker는 모두 claim 가능하며 stale24는 19·stale48은 18로 감소했습니다. 복구 이후 27 snapshot/19 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 21:50~21:51 KST 다음 `normal` group은 `provider_duplicate_identity:4:22:page_overlap:2`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 42 group/55 tracker/9 agency로 전진했습니다. 남은 15 group/17 tracker는 모두 현재 claim 가능하며 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:00~22:01 KST 다음 `normal` group은 `provider_duplicate_identity:3:7:page_overlap:2`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 43 group/56 tracker/9 agency로 전진했습니다. 남은 14 group/16 tracker는 모두 현재 claim 가능하며 global circuit은 closed, 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:03 KST `normal` group 1건은 `provider_duplicate_identity:6:21:page_overlap:3`으로 fail-closed·30분 격리됐고, 22:10 KST 다음 `normal` group은 collection `pw-chrome-1786713052105-b1cc9e41860f28fbfb43`로 광고 30개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 45 group/58 tracker/9 agency, 남은 12 group/14 tracker는 모두 claim 가능하며 복구 이후 28 snapshot/20 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:20~22:21 KST 다음 `normal` group은 `provider_duplicate_identity:4:28:page_overlap:3`으로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 46 group/59 tracker/9 agency로 전진했습니다. 남은 11 group/13 tracker는 모두 현재 claim 가능하며 global circuit closed, 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:30~22:31 KST 다음 `normal` group은 `provider_duplicate_identity:8:3:page_overlap:7`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 47 group/60 tracker/9 agency로 전진했습니다. 남은 10 group/12 tracker는 모두 현재 claim 가능하며 global circuit closed, 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:40~22:41 KST 다음 `normal` group은 `provider_duplicate_identity:6:13:page_overlap:5`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 48 group/61 tracker/9 agency로 전진했습니다. 남은 9 group/11 tracker는 모두 현재 claim 가능하며 global circuit closed, 같은-cycle group 중복·atomic 위반은 0입니다.
- 22:50~22:51 KST 다음 `normal` group은 `provider_duplicate_identity:5:3:page_overlap:4`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 lane·lease를 해제했고 cycle #8은 49 group/62 tracker/9 agency로 전진했습니다. 남은 8 group/10 tracker는 모두 현재 claim 가능하며 global circuit closed, 같은-cycle group 중복·atomic 위반은 0입니다.
- 23:00 KST `normal` group 1건이 collection `pw-chrome-1786716045675-67da69a7366f226495c0`로 원자 300을 완료한 뒤 6초 handoff로 다음 group도 collection `pw-chrome-1786716094992-25cd28a14a56369965b3`로 광고 30개 제외·오가닉 300개를 완료하고 과거 격리를 해제했습니다. cycle #8 누적 51 group/64 tracker/9 agency, 남은 6 group/8 tracker는 모두 claim 가능하며 복구 이후 30 snapshot/22 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 23:10 KST 다음 `normal` group의 tracker 3건은 collection `pw-chrome-1786716652291-8d2cd6b820b3f0e1545d` 하나로 광고 30개 제외·오가닉 300개를 함께 완료했습니다. cycle #8 누적 52 group/67 tracker/9 agency, 남은 5 group/5 tracker는 모두 claim 가능하며 복구 이후 33 snapshot/23 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 23:20 KST 다음 `normal` group은 collection `pw-chrome-1786717254167-2e3aaa66a34f27cf4140`로 광고 30개 제외·오가닉 300개를 완료했습니다. cycle #8 누적 53 group/68 tracker/9 agency, 남은 4 group/4 tracker는 모두 claim 가능하며 복구 이후 34 snapshot/24 collection의 같은-cycle group 중복·atomic 위반은 0입니다.
- 23:30 KST 다음 `normal` group은 collection `pw-chrome-1786717855475-2f738909ec6423342baa`로 오가닉 300개를 완료했습니다. cycle #8 누적 54 group/69 tracker/9 agency, 남은 3 group/3 tracker는 모두 claim 가능하며 복구 이후 35 snapshot/25 collection의 같은-cycle group 중복·atomic 위반은 0입니다. terminal 뒤 lane·lease는 해제됐습니다.
- 23:40 KST 다음 `normal` group은 collection `pw-chrome-1786718449093-c5960af08dffe50d869d`로 오가닉 300개를 완료했습니다. cycle #8 누적 55 group/70 tracker/9 agency, 남은 2 group/2 tracker이며 복구 이후 36 snapshot/26 collection의 같은-cycle group 중복·atomic 위반은 0입니다. terminal 뒤 lane·lease는 해제됐습니다.
- 23:50·00:00 KST 남은 `normal` group 2건이 각각 collection `pw-chrome-1786719051702-89b9ce708481e1a7f594`, `pw-chrome-1786719651602-3020ea52d493ed5c9fb7`로 원자 300을 완료했고 00:10 KST cycle #8이 전체 완료됐습니다. 복구 기준 38 snapshot/28 collection의 atomic 위반과 같은-cycle 중복은 0입니다.
- 00:20 KST cycle #9가 시작돼 신규 group을 먼저 1회 처리했으나 `provider_duplicate_identity:2:4:page_overlap:1`로 fail-closed됐습니다. 이어 기존 cursor의 `resume`으로 복귀했지만 00:29·00:30 KST 서로 다른 group에서 `naver_page_navigation_failed`가 연속 발생해 전역 circuit이 open됐습니다. snapshot 오염·lane/lease 고립은 없지만 이후 공정 순환은 정지 상태이므로 정상으로 판정하지 않습니다.
- 자동 복구 마이그레이션은 정확히 `navigating:naver_page_navigation_failed` circuit만 10분 뒤 primary worker의 다음 정상 순서 1건으로 half-open 검증합니다. `next_check_at`·격리·cycle cursor를 별도로 변경하지 않고, 성공은 원자 300 증거가 있을 때만 circuit을 닫으며 재실패는 즉시 다시 open합니다. 수동 canary의 exact-tracker 계약은 유지했습니다.
- 03:30 KST 첫 자동 half-open은 cycle #9의 다음 정상 group을 기존 순서로 claim해 page 6까지 진행했으므로 브라우저 탐색 복구는 확인됐습니다. 다만 결과 검증이 `provider_duplicate_identity:4:1:page_overlap:3`으로 tracker 단위 종료된 뒤 lane release가 이를 `probe_incomplete` 전역 circuit으로 다시 열어 cycle은 4/57 group에서 재정지했습니다. 원자 300 성공으로 확대 보고하지 않습니다.
- 두 번째 보완은 자동 navigation half-open에서 tracker 단위 오류로 terminal된 경우에만 tracker 격리·last-good을 그대로 보존하고 전역 navigation circuit만 닫습니다. 실제 navigation/system 오류는 기존대로 다시 open하며, 배포 당시 정확한 false-open 상태만 control-plane에서 1회 복구합니다. 순서·cursor·격리·wake는 변경하지 않습니다. 전용 9/9, server contract 52/52, baseline, 보호 잠금 22함수·83파일·33마이그레이션 self-test와 전체 `check:release`가 통과했지만 운영 migration·다음 순서 terminal 전에는 복구 완료로 판정하지 않습니다.
- 03:40 KST Production `e409f633bac0`과 DB migration을 반영했습니다. exact false-open만 닫힌 뒤 cycle #9의 다음 normal group이 collection `pw-chrome-1786732843086-33be93755eeb0d0b666a`로 광고 45개 제외·오가닉 300개를 완료했고, 6초 handoff된 다음 group은 실제 page overlap으로 tracker만 30분 격리됐습니다. 두 번째 오류 뒤에도 circuit closed·lane/run/lease null이므로 전역 재정지 결함은 운영에서 해소됐습니다. cycle 누적 6 group/8 tracker/7 agency, 같은-cycle group 중복 0, snapshot 1건의 atomic 위반 0이며 24시간 최종 판정은 계속 보류합니다.
- 03:50 KST 다음 자연 `normal` group은 동일 키워드 tracker 2건을 collection `pw-chrome-1786733454439-358906d98f00d5e940d7` 하나로 묶어 각각 광고 45개 제외·오가닉 300개로 완료했습니다. cycle #9 누적 7 group/10 tracker/8 agency, 같은-cycle group 중복 0, snapshot 3건/collection 2개의 atomic 위반 0이며 circuit closed·processing 0·lane/run/lease null입니다. 강제 wake·순서·격리 조작 없이 계속 관측합니다.
- 03:53 KST 활성 72 tracker/57 group/9 agency 중 8 agency가 현재 cycle에 진입했고, 아직 미진입한 `dlalsrb8421`은 후속 cursor 대상입니다. 격리 3건은 processing 0이며, 이전 cycle의 strict partial 격리 1건은 cycle #9 claim 0으로 건너뛴 상태에서도 다른 normal group들이 계속 처리됐습니다. 이는 격리가 전체 순환을 막지 않는 중간 증거이며 전체 coverage 완료 증거는 아닙니다.
- 04:00 KST 다음 자연 `normal` group은 과거 격리가 만료된 tracker를 원래 cursor 순서에서 collection `pw-chrome-1786734054279-dae4c55edd7c3ed63f9d`로 광고 45개 제외·오가닉 300개 완료하고 `quarantine_cleared`까지 기록했습니다. cycle #9 누적 8 group/11 tracker/8 agency, 같은-cycle 중복 0, snapshot 4건/collection 3개의 atomic 위반 0, 격리 3→2, terminal lane/run/lease null입니다.
- 04:03 KST 정기 SELECT-only 증거는 runtime `1.1.7`, heartbeat 26초, cycle #9 active, 72 tracker/57 group/9 agency 중 8 group/11 tracker/8 agency claim, same-cycle 중복 0입니다. stale24 21·stale48 19·never-checked 1·격리 2·processing 0, cycle snapshot 4/collection 3·atomic 위반 0, circuit closed·lane/run/lease null입니다.
- 04:10 KST 다음 자연 `normal` group은 `provider_duplicate_identity:8:3:page_overlap:7`로 fail-closed·30분 격리됐습니다. snapshot은 저장하지 않았고 circuit closed·processing 0·lane/run/lease null로 다음 순서를 막지 않습니다. cycle #9 누적 9 distinct group/12 tracker/8 agency, 중복 0, snapshot 4/collection 3·atomic 위반 0입니다.
- 04:20 KST 다음 자연 `normal` group은 collection `pw-chrome-1786735253508-646d551edb676754abd4`로 광고 60개 제외·오가닉 300개를 완료했습니다. cycle #9 누적 10 distinct group/13 tracker/8 agency, 같은-cycle 중복 0, snapshot 5/collection 4·atomic 위반 0, 격리 2·processing 0·circuit closed·lane/run/lease null입니다.
- 04:30 KST 다음 자연 `normal` group은 collection `pw-chrome-1786735854088-4378ef93bdce9cc2f8d1`로 광고 45개 제외·오가닉 300개를 완료했습니다. cycle #9 누적 11 distinct group/14 tracker/8 agency, 같은-cycle 중복 0, snapshot 6/collection 5·atomic 위반 0, 격리 2·processing 0·circuit closed·lane/run/lease null입니다.
- 04:34~04:35 KST 다음 자연 `normal` group 1건은 `provider_duplicate_identity:6:0:page_overlap:5`로 fail-closed·30분 격리됐습니다. snapshot 저장 없이 terminal됐고 cycle #9은 12 distinct group/15 tracker/8 agency, 같은-cycle 중복 0, snapshot 6/collection 5·atomic 위반 0입니다. circuit closed·processing 0·lane/run/lease null이라 전역 정지 없이 다음 순서를 기다립니다.
- 04:40 KST 다음 자연 `normal` group은 collection `pw-chrome-1786736452015-2b3072bca3bfa76a5c30`으로 광고 60개 제외·오가닉 300개를 완료했습니다. cycle #9 누적 13 distinct group/16 tracker/8 agency, 같은-cycle 중복 0, snapshot 7/collection 6·atomic 위반 0이며 terminal 뒤 circuit closed·lane/run/lease null입니다.
- 04:50 KST 다음 `normal` group은 collection `pw-chrome-1786737051499-0eee6fb4fa8be4d404f3`으로 광고 45개 제외·오가닉 300개를 완료했습니다. 05:00~05:01 KST 다음 group은 `provider_duplicate_identity:3:3:page_overlap:2`로 snapshot 없이 30분 격리됐습니다. 05:03 KST cycle #9은 15 distinct group/18 tracker/8 agency, 중복 0, snapshot 8/collection 7·atomic 위반 0이고 circuit closed·processing 0·lane/run/lease null입니다.
- 05:08·05:10·05:20 KST `normal` 3개 group이 각각 tracker 2·4·1건을 세 collection으로 광고 30·45·45개 제외, 오가닉 300개 완료했습니다. 05:30~05:31 KST 다음 group은 `provider_duplicate_identity:3:20:page_overlap:1`로 snapshot 없이 30분 격리됐습니다. 05:35 KST cycle #9은 19 distinct group/26 tracker/9 agency로 모든 광고주가 진입했고, 중복 0, snapshot 15/collection 10·atomic 위반 0, circuit closed·processing 0·lane/run/lease null입니다.
- 05:40~05:41 KST `normal` group의 tracker 2건은 동일 page-overlap으로 snapshot 없이 30분 격리됐습니다. 05:50·06:00 KST 다음 2개 group은 tracker 4·1건을 두 collection으로 각각 광고 45개 제외·오가닉 300개 완료했고, 06:00 성공 직후 6초 handoff된 다음 group은 06:02 KST page-overlap으로 snapshot 없이 격리됐습니다. 06:06 KST cycle #9은 23 distinct group/34 tracker/9 agency, 중복 0, snapshot 20/collection 12·atomic 위반 0, 격리 4·processing 0·circuit closed·lane/run/lease null입니다.
- 06:10~06:11 KST `normal` group의 tracker 2건은 동일 page-overlap으로 snapshot 없이 30분 격리됐습니다. 06:20 KST 다음 group은 collection `pw-chrome-1786742451483-b09ac4306d201161e793`으로 광고 45개 제외·오가닉 300개를 완료했고, 06:30~06:31 KST 다음 group은 page-overlap으로 snapshot 없이 격리됐습니다. 06:38 KST cycle #9은 26 distinct group/38 tracker/9 agency, 중복 0, snapshot 21/collection 13·atomic 위반 0, 격리 4·processing 0·circuit closed·lane/run/lease null입니다.
- 06:40~06:41 KST 다음 group은 page-overlap으로 snapshot 없이 격리됐습니다. 06:50·06:55 KST 두 group은 각 collection으로 광고 45·30개 제외·오가닉 300개를 완료했고, 07:00~07:01 KST 다음 group은 page-overlap으로 격리됐습니다. 07:10 KST tracker 2건의 다음 group도 collection `pw-chrome-1786745451655-0b9c3fbafb6cadb194f8` 하나로 광고 30개 제외·오가닉 300개 완료했습니다. 07:11 KST cycle #9은 31 distinct group/44 tracker/9 agency, 중복 0, snapshot 25/collection 16·atomic 위반 0, terminal 뒤 circuit closed·lane/run/lease null입니다.
- 07:20~07:21 KST 다음 group은 page-overlap으로 snapshot 없이 30분 격리됐습니다. 07:30·07:40 KST 다음 2개 group은 collection `pw-chrome-1786746653449-6f30ff6c9c50bcc5e506`, `pw-chrome-1786747251377-406fd1d424462cd6e5af`로 각각 광고 30개 제외·오가닉 300개 완료했습니다. 07:43 KST cycle #9은 34 distinct group/47 tracker/9 agency, 중복 0, snapshot 27/collection 18·atomic 위반 0, 격리 2·processing 0·circuit closed·lane/run/lease null입니다.
- 07:50·07:56·08:00·08:10 KST 다음 `normal` 4개 group이 각 collection으로 광고 30·60·45·45개 제외·오가닉 300개를 연속 완료했습니다. 08:14 KST cycle #9은 38 distinct group/51 tracker/9 agency, 중복 0, snapshot 31/collection 22·atomic 위반 0, 격리 1·processing 0·circuit closed·lane/run/lease null입니다.
- 08:20~08:41 KST 다음 `normal` 3개 group은 각각 page-overlap으로 snapshot 없이 30분 격리됐습니다. 연속 실패에도 모두 tracker 범위로 종료돼 08:46 KST cycle #9은 41 distinct group/54 tracker/9 agency로 전진했고, 중복 0, snapshot 31/collection 22·atomic 위반 0, 격리 4·processing 0·circuit closed·lane/run/lease null입니다.
