-- 2026-09-19: let a standby register a new runtime identity while the primary is down.
-- Incident (KST 09-19): a runtime release went live at 11:50 while windows-desktop-primary had
-- been down since 09-18 13:34. The runtime migration clears the coordination identity and the
-- lane claim refused every standby whose identity differed from that (now NULL) value, so
-- macbook-standby was answered `runtime_identity_invalid` until the owner filled the identity
-- by hand at 12:5x (70 minutes without collection).
-- This re-declares mi_claim_naver_shopping_worker_lane with one change: a standby is admitted
-- when the coordination identity is entirely unset and the primary is stale. The progress
-- gate (the single runtime-literal carrier) still pins the identity to the expected runtime on
-- the first report, and a set-but-different identity is still refused. No runtime literal.
begin;
