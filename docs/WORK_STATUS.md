...
- 성능 판정 SQL은 activation부터 고정 wall time의 group을 claim별 correlated subquery로 집계해 grouped tracker×commit 조인 증폭을 제거하고, 각 tracker의 terminal 정확1·atomic snapshot·cursor·cycle 중복을 별도 검증하도록 독립 실행했습니다. baseline 재현 고정시각 `15:26:23.815705Z`에서 wall4220.816초, raw group9/fully-terminal9, 동시대 remote/early 2건 제외 뒤 scheduled-compatible7(bootstrap 뒤6), atomic/failure/cursor 위반0입니다. scheduled fixed-wall은 `5.970410 group/hour`, remote 포함 raw는 `7.676241 group/hour`이며 모두 candidate 전 수치입니다. activation은 setter 직후 별도 SELECT로 캡처한 exact DB timestamp만 쓰고 계속 바뀌는 현재 coordination.updated_at은 금지하며, mutable wake의 과거 attribution을 동시대 claim ID로 보존하지 못하면 성능 판정을 fail-closed합니다.
