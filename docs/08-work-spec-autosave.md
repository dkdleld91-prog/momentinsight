...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | 메인 기능 쇼케이스 우선순위·그룹 보정 | `Product Intelligence`를 `Trust Standard` 위로 이동하고, 현재 데이터 카드와 30일 추적 카드를 두 그룹으로 분리. 네이비 패널을 저채도 잉크 네이비로 보정 | 섹션 순서·그룹별 정확한 카드 구성 기준선, 1440×900·390×844 레이아웃, 문서 가로 넘침·카드 잘림·콘솔 오류 0건, 독립 2차 검토 통과 | 로컬 완료 · push/배포 없음 |
| 완료 | 메인 홈페이지 익명 기능 쇼케이스 | 실제 광고주 상품·키워드·ID·매장 정보를 사용하지 않고 상품 오가닉 순위, 30일 상품 추적, 플레이스 추적, 키워드 시장 분석을 중립 예시 데이터 4카드로 압축. 기존 반복 섹션 2개를 1개로 교체 | 실고객 문자열·9자리 이상 ID·외부 URL·이미지 금지 기준선, `check:quality`, 1440×900·390×844 육안검수, 카드 4개·모바일 가로 탐색·텍스트 잘림·콘솔 오류·문서 가로 넘침 0건 | 로컬 완료 · push/배포 없음 |
| 완료 | 메인 홈페이지 프리미엄 보완 2차 | 문서·모바일 렌더 기반을 바로잡고, 팝업 외곽 규격·위치·노출 로직은 보존한 채 헤더·히어로·샘플 대시보드·신뢰 기준·운영 흐름·CTA·푸터의 시각 위계를 정돈. 모바일 반복 카드는 가로 탐색으로 압축 | `check:quality`, `build:vercel`, `git diff --check`, 1440×900·390×844 브라우저 육안검수, 팝업 348×489px·모바일 높이 489px, 닫기·1주일 숨김, 모바일 헤더 65px 한 줄, 가로 넘침·콘솔 오류 0건 | 로컬 완료 · push/배포 없음 |
