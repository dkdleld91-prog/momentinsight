...
| 진행 중 | 네이버 API Hub 이관·쇼핑 검색 종료 대응 | Search Trend·Shopping Insight·blog/local을 `legacy|hub|auto`로 분리하는 공통 어댑터와 공급자별 인증·경로·오류 분류를 구현. 기본값·오타는 legacy로 닫고 쇼핑 검색은 종료 전까지 legacy 전용으로 격리해 기존 운영을 임의 전환하지 않음 | 공식 공지·Hub 명세 대조, 대상 66/66·전체 API/서버 178/178·수집기 51/51·서버 계약 23/23·Production 인증 18/18·전체 `check:release` 통과, legacy `써큘레이터` 실호출 HTTP 200·경고 0건 | NCP Hub 앱·구독·실키가 없어 Hub 실호출과 shadow 비교는 미완료. 2026-07-31 종료되는 쇼핑 검색의 검증 가능한 새 상품 순위 소스를 별도 확정해야 함 |
