...
const shoppingSeamRepeatRuntime1122Migration = read("supabase/migrations/20260911003000_naver_shopping_runtime_1_1_22_seam_repeat_and_login_redirect.sql");
const shoppingRenderedPageToleranceRuntime1123Migration = read("supabase/migrations/20260911090000_naver_shopping_runtime_1_1_23_rendered_page_tolerance.sql");
...
const shoppingSamePageTwinsUnboundedRuntime1128Migration = read("supabase/migrations/20260912160000_naver_shopping_runtime_1_1_28_same_page_twins_unbounded.sql");
const shoppingRenderedCrossPageRepeatsRuntime1129Migration = read("supabase/migrations/20260913050000_naver_shopping_runtime_1_1_29_rendered_cross_page_repeats.sql");
const shoppingFiniteDriftEvidenceRuntime1130Migration = read("supabase/migrations/20260913150000_naver_shopping_runtime_1_1_30_finite_drift_evidence.sql");
const shoppingEvidenceV2ThirdPassRuntime1131Migration = read("supabase/migrations/20260918030000_naver_shopping_runtime_1_1_31_evidence_v2_third_pass.sql");
const shoppingFiniteCrossPageRuntime1132Migration = read("supabase/migrations/20260919020000_naver_shopping_runtime_1_1_32_finite_cross_page_repeats.sql");
...
const shoppingWorkerRuntime1129Fingerprint =
  "3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77";
const shoppingWorkerRuntime1130Fingerprint =
  "62e09cb15720dda97fdc652cfa9b3fba40bdbbc4c98372269867c21ee472d10c";
const shoppingWorkerRuntime1131Fingerprint =
  "099bf53085a118ee3917c5f5596e72a08efe91faa3247b9091ddb27241149116";
const shoppingWorkerRuntime1132Fingerprint = calculateN30RuntimeFingerprint({
...
const checks = {
  ...
  homeDevelopmentNoticeVisible: homeSource.includes("9월 서비스 운영 안내")
    ...
    && homeDevelopmentNoticeMarkupSource.includes("하루 2~3회 자동 갱신됩니다. 정해진 시각이 아니라 등록된 순서대로 차례로 기록됩니다.")
    ...
    && homeDevelopmentNoticeMarkupSource.includes('data-status="하루 2~3회"')
  ...
  shoppingRemoteWakeIsAtomicAndOneJobBounded: shoppingChromeManifest.version === "1.1.32"
    ...
    && shoppingLocalWorker.includes('const EXPECTED_RUNTIME_VERSION = "1.1.32";')
    && shoppingLocalWorkerHandler.includes('const EXPECTED_WORKER_RUNTIME_VERSION = "1.1.32";')
    && rankServer.includes('const SHOPPING_WORKER_EXPECTED_RUNTIME_VERSION = "1.1.32";')
  ...
  shoppingRuntime1116ExactParentProofExactIdentity:
    ...
    && shoppingFiniteGeneralRuntime1121Migration.includes("current_row.runtime_version is distinct from '1.1.20'")
    && shoppingFiniteGeneralRuntime1121Migration.includes(shoppingWorkerRuntime1120Fingerprint)
    && shoppingFiniteGeneralRuntime1121Migration.includes("set runtime_version = '1.1.21'")
    ...
    && shoppingFiniteGeneralRuntime1121Migration.includes("security invoker")
    && shoppingFiniteGeneralRuntime1121Migration.includes("set search_path = ''")
    && shoppingSeamRepeatRuntime1122Migration.includes("current_row.runtime_version is distinct from '1.1.21'")
    ...
    && shoppingSeamRepeatRuntime1122Migration.includes("naver_shopping_runtime_1_1_22_requires_completed_account_priority")
    && shoppingSeamRepeatRuntime1122Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingSeamRepeatRuntime1122Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && !shoppingSeamRepeatRuntime1122Migration.includes("create or replace function public.mi_enqueue_naver_shopping_account_priority(")
    && shoppingSeamRepeatRuntime1122Migration.includes("security invoker")
    && shoppingSeamRepeatRuntime1122Migration.includes("set search_path = ''")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("current_row.runtime_version is distinct from '1.1.22'")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes(shoppingWorkerRuntime1122Fingerprint)
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("set runtime_version = '1.1.23'")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes(shoppingWorkerRuntime1123Fingerprint)
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("expected_runtime_version constant text := '1.1.23'")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("naver_shopping_runtime_1_1_23_requires_completed_account_priority")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingRenderedPageToleranceRuntime1123Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && !shoppingRenderedPageToleranceRuntime1123Migration.includes("create or replace function public.mi_enqueue_naver_shopping_account_priority(")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("security invoker")
    && shoppingRenderedPageToleranceRuntime1123Migration.includes("set search_path = ''")
    && shoppingFailureEvidenceRuntime1124Migration.includes("current_row.runtime_version is distinct from '1.1.23'")
    && shoppingFailureEvidenceRuntime1124Migration.includes(shoppingWorkerRuntime1123Fingerprint)
    && shoppingFailureEvidenceRuntime1124Migration.includes("set runtime_version = '1.1.24'")
    && shoppingFailureEvidenceRuntime1124Migration.includes(shoppingWorkerRuntime1124Fingerprint)
    && shoppingFailureEvidenceRuntime1124Migration.includes("expected_runtime_version constant text := '1.1.24'")
    && shoppingFailureEvidenceRuntime1124Migration.includes("naver_shopping_runtime_1_1_24_requires_completed_account_priority")
    && shoppingFailureEvidenceRuntime1124Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingFailureEvidenceRuntime1124Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && !shoppingFailureEvidenceRuntime1124Migration.includes("create or replace function public.mi_enqueue_naver_shopping_account_priority(")
    && shoppingFailureEvidenceRuntime1124Migration.includes("security invoker")
    && shoppingFailureEvidenceRuntime1124Migration.includes("set search_path = ''")
    && shoppingRenderedIdentityRuntime1125Migration.includes("current_row.runtime_version is distinct from '1.1.24'")
    && shoppingRenderedIdentityRuntime1125Migration.includes(shoppingWorkerRuntime1124Fingerprint)
    && shoppingRenderedIdentityRuntime1125Migration.includes("set runtime_version = '1.1.25'")
    && shoppingRenderedIdentityRuntime1125Migration.includes(shoppingWorkerRuntime1125Fingerprint)
    && shoppingRenderedIdentityRuntime1125Migration.includes("expected_runtime_version constant text := '1.1.25'")
    && shoppingRenderedIdentityRuntime1125Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingRenderedIdentityRuntime1125Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingRenderedIdentityRuntime1125Migration.includes("security invoker")
    && shoppingRenderedIdentityFallbackRuntime1126Migration.includes("current_row.runtime_version is distinct from '1.1.25'")
    && shoppingRenderedIdentityFallbackRuntime1126Migration.includes(shoppingWorkerRuntime1125Fingerprint)
    && shoppingRenderedIdentityFallbackRuntime1126Migration.includes("set runtime_version = '1.1.26'")
    && shoppingRenderedIdentityFallbackRuntime1126Migration.includes(shoppingWorkerRuntime1126Fingerprint)
    && shoppingRenderedIdentityFallbackRuntime1126Migration.includes("expected_runtime_version constant text := '1.1.26'")
    ...
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("current_row.runtime_version is distinct from '1.1.27'")
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes(shoppingWorkerRuntime1127Fingerprint)
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("set runtime_version = '1.1.28'")
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes(shoppingWorkerRuntime1128Fingerprint)
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("expected_runtime_version constant text := '1.1.28'")
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingSamePageTwinsUnboundedRuntime1128Migration.includes("security invoker")
    && shoppingRenderedCrossPageRepeatsRuntime1129Migration.includes("current_row.runtime_version is distinct from '1.1.28'")
    ...
    && shoppingRenderedCrossPageRepeatsRuntime1129Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingRenderedCrossPageRepeatsRuntime1129Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingRenderedCrossPageRepeatsRuntime1129Migration.includes("security invoker")
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes("current_row.runtime_version is distinct from '1.1.29'")
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes(shoppingWorkerRuntime1129Fingerprint)
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes("set runtime_version = '1.1.30'")
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes(shoppingWorkerRuntime1130Fingerprint)
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes("expected_runtime_version constant text := '1.1.30'")
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingFiniteDriftEvidenceRuntime1130Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingFiniteDriftEvidenceRuntime1130Migration.includes("security invoker")
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("current_row.runtime_version is distinct from '1.1.30'")
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes(shoppingWorkerRuntime1130Fingerprint)
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("set runtime_version = '1.1.31'")
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes(shoppingWorkerRuntime1131Fingerprint)
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("expected_runtime_version constant text := '1.1.31'")
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingEvidenceV2ThirdPassRuntime1131Migration.includes("security invoker")
    && shoppingFiniteCrossPageRuntime1132Migration.includes("current_row.runtime_version is distinct from '1.1.31'")
    && shoppingFiniteCrossPageRuntime1132Migration.includes(shoppingWorkerRuntime1131Fingerprint)
    && shoppingFiniteCrossPageRuntime1132Migration.includes("set runtime_version = '1.1.32'")
    && shoppingFiniteCrossPageRuntime1132Migration.includes(shoppingWorkerRuntime1132Fingerprint)
    && shoppingFiniteCrossPageRuntime1132Migration.includes("expected_runtime_version constant text := '1.1.32'")
    && shoppingFiniteCrossPageRuntime1132Migration.includes("create or replace function public.mi_report_naver_shopping_worker_progress(")
    && !shoppingFiniteCrossPageRuntime1132Migration.includes("create or replace function public.mi_commit_naver_shopping_worker_result(")
    && shoppingFiniteCrossPageRuntime1132Migration.includes("security invoker")
    && shoppingCandidatePerformanceAudit.includes('export const N30_TARGET_RUNTIME_VERSION = "1.1.32";')
    && shoppingCandidatePerformanceAudit.includes(shoppingWorkerRuntime1132Fingerprint)
  ...
  migrationsCarryNoRuntimeLiteralsOutsideAllowlist: migrationRuntimeLiteralAudit.ok
    ...
    && migrationRuntimeLiteralAudit.carriers[0].fingerprints
      .includes(shoppingWorkerRuntime1132Fingerprint),
  ...
  shoppingManualExtensionQueuesEntireTrackerSite: shoppingChromeManifest.version === "1.1.32"
    && shoppingChromeWorker.includes('port.postMessage({ action: "run", trigger, ...runtimeIdentity })')
  ...
  shoppingCollectorProductionGateRequiresAtomic300: shoppingCollectorContract.includes("const MAX_RANK_LIMIT = 300")
    ...
    && shoppingCollectorLiveCheck.includes("window.checkedCount !== limit")
    && shoppingCollectorLiveCheck.includes("collector_window_short"),
  shoppingCollectorSamePageRankSlotsStayAtomic: shoppingCollectorProvider.includes('if (collisionKind !== "duplicate_row" && !preserveStableCrossPage && !rejectAllIdentityDuplicates)')
  ...
  placeNativeExhaustionCacheIsFailClosed: placeRankCollector.includes('collection?.stopReason === "naver_result_list_exhausted"')
    ...
    && placeRankCollector.includes('["collection_deadline_reached", "max_scrolls_reached", "max_pages_reached", "next_page_unconfirmed", "memory_guard"]')
