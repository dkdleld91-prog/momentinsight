...
7. 진행: DB anchor는 `2026-08-24T17:58:36.220664Z`로 유지됐습니다. 잔여 `12f5330a…`는 자연 순번에서 `provider_partial_window:137_300`으로 snapshot0·last-good 보존·24시간 격리됐고 cycle37은 중복0으로 완료됐습니다. 후속 cycle38도 정상 진행했고 `2026-08-25T06:01:04.111338Z` operations는 streak88·exact identity·recent atomic300·baseline10·candidate false·closed/null·processing0·완전 idle입니다. 고정 checkpoint `04:36:17.584413Z`의 baseline10 correlated fixed-wall은 bootstrap 제외 atomic catch-up58/38261.363749초=`5.457202 group/hour`이고 provenance/atomic/중복/open/overlap/grid 위반0입니다. 성공6회 조건은 충족했지만 anchor+24시간이 아직 false라 candidate_eligible=false입니다. 가장 이른 anchor gate는 `2026-08-25T17:58:36.220664Z`이나, 알려진 residual 중 가장 늦은 `12f5…` 격리 만료 `2026-08-26T02:10:35.927243Z`와 자연 terminal·post-idle을 먼저 확인해 120분 failure0 창에 예견된 실패를 넣지 않습니다. 그 뒤에도 exact identity·fresh heartbeat·recent atomic300·closed/null·processing0·완전 idle과 candidate 시도0을 다시 확인하기 전에는 setter를 호출하지 않습니다.
8. 대기: canonical candidate6·baseline10 SQL 생성기는 독립 감사에서 확인한 claim 일부집합·snapshot 중복·비배열 JSON P1을 보완해 claim/run/collection 전체 exact set으로 강화했고, 전환+성능 감사10/10·생성기 coverage100%와 Production read-only 재컴파일에서 최신 exact proof `atomicProofOk=true`를 확인했습니다. 실제 후보 실행 직전 SELECT의 run UUID·collection ID·last-success를 고정 입력하고 같은 coordination lock에서 proof가 그대로인지 재검증합니다. DB attempt ledger가 없어 cross-execution exactly-once는 SQL만으로 불가능하므로 요청 전에 외부 `attempted=true`를 영구 기록하고 transaction 안 setter1회만 실행합니다. timeout·거부·응답 유실·committed-unknown을 포함해 어떤 결과도 재호출하지 않습니다. 알려진 residual의 자연 terminal·post-idle 전에는 실행하지 않습니다.
...
10. 완료(감사 도구): 위 판정을 pinned identity·전 exact run global integrity를 포함한 파라미터형 read-only SQL 생성기와 100% coverage 테스트로 고정하고 Production baseline checkpoint에서 compile·동일 처리량을 재현했습니다. claim/commit 집합·event 순서·cycle identity·같은 cycle의 unexpected cursor NULL reset·malformed cursor·coordination updatedAt 경계·비배열 JSON도 fail-closed합니다. 현 ledger는 seed/selectionMode를 저장하지 않아 strict seed 순서의 완전 증명은 불가능하므로, candidate 측정창에서는 tracker 하나라도 cursor 이하인 claim을 세는 `cursor_nonforward_or_fallback_count=0`을 요구합니다. 이 보수 조건이 정상 grouped sibling/fallback을 거부하면 재호출하지 않고 성공0으로 판정합니다. 전체 `npm test`의 기존 개인 일정 syncToken 실패1은 별도 보호 범위라 수정하지 않았고 전체 PASS로 기록하지 않습니다.
