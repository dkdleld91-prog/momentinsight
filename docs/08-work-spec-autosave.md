...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | N 30일 순위 슬롯 `상품` 문구 정리 | 일별 PM/AM 슬롯에서 정확 상품인 경우 중복되는 `상품` 문구만 숨기고, 원부 구분과 순위 조회·저장·대표값 판정 로직은 그대로 보존 | 양 역할 런타임 출력 `PM`·`AM · 원부`, 전용 릴리즈 기준선, 전체 `check:quality`, 광고주 390px·운영팀 데스크톱 빌드 및 콘솔 0건 통과 | 로컬 커밋, 현재 배포 승인 없음 |
