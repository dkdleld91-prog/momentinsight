-- Runtime 1.1.29 (2026-09-13): cross-page repeats in the rendered-order path.
-- 1.1.28 accepted the same-page twins of 콘트로이친 (ofyou); its first run
-- (2026-09-13 13:25 KST) then failed one layer later with
-- `provider_duplicate_identity:6:7:page_overlap:4` — the same seller product is
-- also listed on pages 4 and 6. The strict path already keeps every slot of a
-- cross-page repeat under the stable full-window proof; the rendered-order
-- candidate pass and its two-capture proof now do the same (contract, provider,
-- server trusted window). The bounded page seam is still skipped. The local
-- worker also passes the finite-market arbitration reason through
-- (`provider_stable_finite_window_unproven:<reason>`).
...
-- Apply right after the 1.1.29 server release is live and the coordination row
-- is idle on 1.1.28 (the release itself stops the 1.1.28 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_29_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.28'
    ...
    raise exception 'naver_shopping_runtime_1_1_29_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.28'
       or runtime_fingerprint is distinct from
         '6bf207914784d28c8cff9ebb857614d3244684e06b4749453f3f7ee845c2fe88'
  ) then
    raise exception 'naver_shopping_runtime_1_1_29_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.29',
      runtime_fingerprint =
        '3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77'
  where runtime_version = '1.1.28'
    and runtime_fingerprint = '6bf207914784d28c8cff9ebb857614d3244684e06b4749453f3f7ee845c2fe88';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_29_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.28'
    and runtime_fingerprint = '6bf207914784d28c8cff9ebb857614d3244684e06b4749453f3f7ee845c2fe88';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_29_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.29';
