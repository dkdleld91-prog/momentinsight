...
- N 상품 30일 전체 갱신 안전 가속: `mml93-a01`의 운영 추적 25개를 직렬 처리해 약 186초가 걸리던 UI 전체 갱신을 동시 2개로 제한했다. 페이지 진입 시 숨은 `sync-due`도 50개에서 2개로 줄이고 자동·수동 중첩을 차단했다. 운영 2차 실측은 화면 25개 완료·DB 25/25 정상·오류/재시도 0건, 115초로 기존보다 약 38% 단축됐다. 성공 응답을 브라우저가 놓친 경우 서버의 최신 확인 시각으로 재대조한 뒤 진짜 미완료 항목만 1회 재시도한다.
...
- 운영 실측·정확성 보존: 플레이스 10건은 고유 키워드 3개(`구월동 맛집` 8건·`홍대 맛집` 1건·`부평 맛집` 1건) 기준 721초에 화면 10개 완료, DB 오류 0건으로 끝났다. 구월동 8건은 첫 수집 뒤 정확 ID 일치 7건이 약 41초 안에 이어졌고 최종 8/8 순위 일치, 홍대 7위 일치, 부평은 실제 77개 미발견을 부분 확인으로 유지했다. 상품의 정확 상품ID·검증 원부ID·광고 제외와 플레이스 네이티브 PC 오가닉·부분조회 진실성, DB 스키마·기존 이력은 변경하지 않았다.
