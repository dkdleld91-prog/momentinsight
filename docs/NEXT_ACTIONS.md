...
## 현재: 키워드 연령별 쇼핑 클릭 비중 복구

- 네이버 표본의 검증된 대분류 ID를 보존해 기존 5개 세로 막대그래프에 최신 완료 월 연령 비중을 표시합니다.
- 보호 잠금·전체 `check:release`·독립 리뷰는 통과했고 Production 배포도 승인됐습니다. 남은 게이트는 복구 태그·운영 release 일치와 `남자팬티` 운영 그래프 렌더링 확인입니다.
- N 30일 순위, Windows 수집기, DB, 그래프 UI 자체는 변경하지 않습니다.

## 변경 금지: 30일 추적

- 사용자의 다음 명시적 요청 전에는 N 상품·N 플레이스 30일 추적, Windows 작업기·스케줄러, 순위 DB·이력 로직을 수정·승격·정리하지 않습니다.
