...
- 트래픽 점수·검증·보호 상태: 다른 등록 항목이 모두 충족되면 광고 제외 오가닉 5위 이내 95점, 40위 이내 90점, 40위 초과 또는 상위 300개 완주 미노출은 85점이다. SEO 회귀 13/13, 전체 API·서버 212/212, 플레이스 수집기 51/51, 서버 계약 27/27, Production 인증 18/18, 역할 parity·기준선·공개 빌드/CSP·전체 `check:release`가 통과했다. 양 역할 데스크톱 빌드에서 4열 입력 폼·리뷰 직접 입력·8개 점검 기준·점수표 배치를 육안 확인했다. 키워드 조회·N 상품 순위·N 상품/플레이스 30일 추적은 기존 해시를 갱신하지 않고 잠금 self-test와 현재 잠금 13함수·21파일·11마이그레이션을 모두 통과했다. 대표님 배포 승인에 따라 커밋·Production 반영과 운영 상태 검증을 진행한다.
