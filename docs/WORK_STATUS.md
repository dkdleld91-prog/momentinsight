...
- 상품 SEO `상품 노출 구조` 참고 카드 제거 검증 완료: 운영팀·광고주 공통 엔진과 화면에서 원부형/단일형 참고 카드·요약·점수 기준 문구를 제거했다. 현재 표시 항목은 `상품명 적합도 20점·상위 상품 핵심어 10점·카테고리 적합도 15점·브랜드/제조사 10점·대표 이미지 10점·리뷰 수량 직접 입력 20점·순위/트래픽 15점`이며, 원부형·단일형 판별을 사용하는 N 상품 순위 기능은 변경하지 않았다.
- 트래픽 점수·검증·보호·배포 상태: 기존 광고 제외 오가닉 기준인 5위 이내 95점, 40위 이내 90점, 40위 초과 또는 상위 300개 완주 미노출 85점을 그대로 유지한다. SEO 회귀 13/13, 전체 API·서버 212/212, 플레이스 수집기 51/51, 서버 계약 27/27, Production 인증 18/18, 역할 parity·기준선·공개 빌드/CSP와 로컬·Vercel Production 전체 `check:release`가 통과했다. 키워드 조회·N 상품 순위·N 상품/플레이스 30일 추적은 기존 해시를 갱신하지 않고 잠금 self-test와 현재 잠금 13함수·21파일·11마이그레이션을 모두 통과했다. 코드 `3cf5422`·Production `dpl_2Ch1ztEz6tDpqiGEH1xsDaD4nLxm`를 운영 별칭에 반영했고 `/health`·`/ready`는 릴리스 `3cf5422fdddc`·서울 `icn1`·Supabase ready다. 실제 운영팀·광고주 HTML과 공통 엔진에서 제거 대상 문구가 없고 SEO v11이 반영된 것을 확인했다.
