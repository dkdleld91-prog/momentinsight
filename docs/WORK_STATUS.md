...
- N 상품 30일 순위 데이터원 종료 대응·서버 복구 경로 보완 진행 중: 2026-07-31 17시경부터 기존 네이버 쇼핑 검색 API가 `Invalid search api (존재하지 않는 검색 api 입니다.)`로 종료돼 활성 상품 추적기 71개 중 35개가 실패한 사실을 Production DB·GitHub Actions·직접 API 호출로 교차 확인했다. 새 NAVER API Hub 키는 블로그 검색에 정상 응답하지만 공개 쇼핑 상품 검색 엔드포인트는 404이며, 공식 이관 문서상 쇼핑 검색은 API Hub 이관 대상에서 제외됐다. 기존 순위와 스냅샷은 삭제되지 않았고 로그인 여부나 로컬 Mac 전원과 무관하게 Vercel 서울 함수·GitHub Actions·Vercel 백업 cron이 계속 실행되는 구조다.
- 복구 코드·검증 상태: 검증된 외부 쇼핑 결과 수집원만 허용하는 서버 어댑터, 오가닉 목록 근거·source 강제 검증, 동일 키워드 페이지 캐시, 영구 종료 오류의 과도한 재시도 방지, 마지막 정상 순위 보존, 한 배치 실패 후에도 전체 대기열을 끝까지 처리하는 cron 복구를 구현했다. 대상 35/35, 전체 API·서버 240/240, 플레이스 51/51, 서버 계약 29/29, Production 인증 18/18, 역할 5상태·양 역할 parity·보호 잠금 21함수·23파일·11마이그레이션과 전체 `check:release`가 통과했다. 승인 범위의 보호 파일 3개만 새 기준으로 재잠금했다. 다만 정확한 N쇼핑 300위 결과를 제공할 실제 서버 수집원 자격정보가 아직 없어 라이브 신규 순위 갱신 증명·배포는 진행하지 않는다. 통합검색 순위를 N쇼핑 순위로 대체하거나 임의 값을 기록하지 않는다.
