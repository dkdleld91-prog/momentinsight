...
- 트래픽 점수·검증·보호·배포 상태: 기존 광고 제외 오가닉 기준인 5위 이내 95점, 40위 이내 90점, 40위 초과 또는 상위 300개 완주 미노출 85점을 그대로 유지한다. SEO 회귀 13/13, 전체 API·서버 212/212, 플레이스 수집기 51/51, 서버 계약 27/27, Production 인증 18/18, 역할 parity·기준선·공개 빌드/CSP·전체 `check:release`가 통과했다. 키워드 조회·N 상품 순위·N 상품/플레이스 30일 추적은 기존 해시를 갱신하지 않고 잠금 self-test와 현재 잠금 13함수·21파일·11마이그레이션을 모두 통과했다. 현재 요청에는 배포 승인이 없어 로컬 커밋 후 배포 대기 상태로 보관한다.
