...
- 상태: 원부 제목에 검색 키워드가 없더라도 정확 상품과 모델번호·브랜드·상위 카테고리가 일치하면 관련 원부로 연결하도록 보강. `써큘레이터`는 원부 8위·정확 상품 59위 중 원부를 대표 순위로 선택하며 운영팀·광고주 화면과 전체 품질검사 2회를 통과
...
- 사용자 배포 승인: 양 역할 실조회·링크·반응형·콘솔, 회귀 검사, 실행환경·Supabase 검증이 모두 통과해 Production 배포와 운영 재검증 단계
