...
test("1.1.26 is the newest runtime migration and the live fingerprint matches its pin", () => {
...
test("live surfaces are 1.1.26 while the archived 1.1.25 evidence keeps its historical identity", () => {
  assert.match(read("tools/naver-shopping-chrome-extension/manifest.json"), /"version": "1\.1\.26"/u);
  for (const relativePath of [
    ...
    assert.match(read(relativePath), /"1\.1\.26"/u, relativePath);
    assert.doesNotMatch(read(relativePath), /"1\.1\.25"/u, relativePath);
  }
  for (const relativePath of [
