...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 배포 대기 | 보안 전용 릴리즈·Owner 부가세 도구 격리 | Production `b37ca5f` 기준으로 세션·CSRF·속도 제한·권한 범위·업로드·SSRF·오류·CSP·공급망·크론·DB 권한만 보강. 이전 이메일/매직링크와 월간/PPT/보고서 목업은 제외. 부가세 계산기는 공개 HTML에 넣지 않고 정확한 `mml93-a01` Owner 세션에만 서버 전달·서버 계산 | `check:release`: 서버 계약 22/22, API·서버 122/122, 플레이스 26/26, Production 인증 14/14, 공개 빌드·공급망·`git diff --check` 통과. Supabase 3개 마이그레이션 적용, Vercel Production 민감 환경변수 4개 등록. 운영 점검에서 발견한 중첩 API 404와 다중 쿠키 로그아웃 누락을 전용·공통 어댑터 및 자동 검사로 보정 | 대표님 보안 전용 배포 승인, Production 재반영·실화면 재검증 진행 |
