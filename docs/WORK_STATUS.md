...
- 대상 42/42, API·서버 389/389, 플레이스 51/51, 쇼핑 51/51, 서버 계약 37/37, Production 인증 18/18, 역할 5상태, 보호 잠금 22함수·58파일·14마이그레이션과 전체 `npm run check:release`를 통과했다. 로컬 완료 상태이며 Production과 중앙 Mac 설치본은 현재 배포 승인 전까지 변경하지 않는다.
