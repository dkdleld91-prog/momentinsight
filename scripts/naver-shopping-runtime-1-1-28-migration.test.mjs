...
// Archived 2026-09-13 (superseded by runtime 1.1.29).
test("keeps the archived runtime 1.1.28 migration pinned to its historical fingerprint", () => {
  ...
  assert.equal(NEW_RUNTIME.fingerprint, "6bf207914784d28c8cff9ebb857614d3244684e06b4749453f3f7ee845c2fe88");
...
test("the archived 1.1.27 evidence keeps its historical identity", () => {
  assert.match(priorMigration, new RegExp(OLD_RUNTIME.fingerprint, "u"));
  assert.doesNotMatch(priorMigration, /1\.1\.28/u);
});

test("1.1.28 unbounded same-page twin allowance is still present in the fingerprinted runtime files (cross-page repeats added in 1.1.29)", () => {
