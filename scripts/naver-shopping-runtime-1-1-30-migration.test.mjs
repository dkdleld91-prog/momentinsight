...
// Runtime 1.1.30 (2026-09-12): rendered-order proof identity falls back to the
...
const migrationName = "20260913150000_naver_shopping_runtime_1_1_30_finite_drift_evidence.sql";
const priorMigrationName = "20260913050000_naver_shopping_runtime_1_1_29_rendered_cross_page_repeats.sql";
...
test("migration moves only the runtime identity pins from 1.1.29 to 1.1.30", () => {
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_30_requires_completed_account_priority'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_30_requires_idle_control_plane'/u);
  ...
  assert.match(migration, new RegExp(`set runtime_version = '1\\.1\\.30',\\s+runtime_fingerprint =\\s+'${NEW_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, new RegExp(`where runtime_version = '1\\.1\\.29'\\s+and runtime_fingerprint = '${OLD_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, /check \(runtime_version = '1\.1\.30'\)/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_30_finite_target_identity_mismatch'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_30_target_mismatch'/u);
  assert.match(migration, /runtime_version = null,\s+runtime_fingerprint = null/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_30_coordination_mismatch'/u);
  ...
  assert.match(progress, /expected_runtime_version constant text := '1\.1\.30';/u);
  ...
  assert.doesNotMatch(progress, /1\.1\.29|3e7a09a0/u);
...
test("the runtime literal audit still passes with the 1.1.30 progress gate as the only carrier", () => {
...
test("1.1.30 finite-drift arbitration and failure evidence are present in the fingerprinted runtime files", () => {
  ...
  assert.match(provider, /if \(renderedOrderCandidate && organicCount > 0\) \{/u);
  const worker = read("scripts/naver-shopping-local-worker.mjs");
  assert.match(worker, /function boundedFailureEvidence\(value\)/u);
});
