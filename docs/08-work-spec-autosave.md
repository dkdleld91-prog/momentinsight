...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 배포 대기 | 보안 세션 후 30일 순위 목록 복구 | DB의 상품 88개·플레이스 13개와 이력은 보존된 상태에서, 총관리자 기본 조회 코드가 `session`으로 남아 두 목록 API가 403을 반환하고 UI가 빈 목록처럼 표시한 회귀를 수정. 총관리자 대상 코드는 서버가 확인한 실제 활성 코드로 기본화하고 API 오류 시 기존 카드를 지우지 않음 | 원본·스냅샷 보존 확인, 대소문자 불일치 0건, 역할별 조회 계약, 서버 계약 22/22, API·서버 122/122, 플레이스 26/26, 전체 `check:quality`, 공개 빌드 CSP 무결성 통과. Production 실화면 재검증 예정 | 복구 배포 대기 |
