...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | 키워드 경쟁강도 절대 상품 포화도 보정 | 기존 검색광고 경쟁도와 `상품수÷검색량` 비율만으로는 `써큘레이터`처럼 검색량과 절대 상품수가 모두 큰 대형 포화 시장이 낮게 평가되는 결함을 수정. 키워드명 예외 없이 절대 쇼핑 상품수 로그 점수와 수요 대비 상품 밀도 중 더 강한 공급 신호를 65%, 검색광고 경쟁도를 35% 반영하고 판매 기회율도 보정된 경쟁점수로 재계산 | 신규 포화도 회귀 3개 포함 API·서버 168/168, 플레이스 수집기 51/51, 서버 계약 23/23, Production 인증 18/18, 운영팀·광고주 parity, 기준선, 순위 기능 잠금 4함수·20파일·11마이그레이션, 공개 빌드·CSP, 전체 `check:release`, `git diff --check` 통과. 표본 `71,400회·상품 약 34만·광고경쟁 보통`은 경쟁 `85·매우 높음`, 같은 수요·상품 2만 표본은 `46·보통` | 로컬 커밋 후 배포 승인 대기. 현재 요청에는 별도 배포 지시 없음 |
