...
// Runtime 1.1.29 (2026-09-12): rendered-order proof identity falls back to the
...
const migrationName = "20260913050000_naver_shopping_runtime_1_1_29_rendered_cross_page_repeats.sql";
const priorMigrationName = "20260912160000_naver_shopping_runtime_1_1_28_same_page_twins_unbounded.sql";
...
const NEW_RUNTIME = Object.freeze({
  version: "1.1.29",
  fingerprint: "3e7a09a0045518c507c3104b5f056a44c722114c85d134039c7bae8c9b9a0b77",
});
...
test("1.1.29 is the newest runtime migration and the live fingerprint matches its pin", () => {
...
test("migration moves only the runtime identity pins from 1.1.28 to 1.1.29", () => {
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_29_requires_completed_account_priority'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_29_requires_idle_control_plane'/u);
  ...
  assert.match(migration, new RegExp(`set runtime_version = '1\\.1\\.29',\\s+runtime_fingerprint =\\s+'${NEW_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, new RegExp(`where runtime_version = '1\\.1\\.28'\\s+and runtime_fingerprint = '${OLD_RUNTIME.fingerprint}'`, "u"));
  assert.match(migration, /check \(runtime_version = '1\.1\.29'\)/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_29_finite_target_identity_mismatch'/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_29_target_mismatch'/u);
  assert.match(migration, /runtime_version = null,\s+runtime_fingerprint = null/u);
  assert.match(migration, /raise exception 'naver_shopping_runtime_1_1_29_coordination_mismatch'/u);
  ...
  assert.match(progress, /expected_runtime_version constant text := '1\.1\.29';/u);
  ...
  assert.doesNotMatch(progress, /1\.1\.28|6bf20791/u);
...
test("the runtime literal audit still passes with the 1.1.29 progress gate as the only carrier", () => {
  const result = auditMigrationRuntimeLiterals({ migrationDirectory });
  assert.deepEqual(result.violations, []);
});

test("live surfaces are 1.1.29 while the archived 1.1.28 evidence keeps its historical identity", () => {
  assert.match(read("tools/naver-shopping-chrome-extension/manifest.json"), /"version": "1\.1\.29"/u);
  for (const relativePath of [
    ...
    assert.match(read(relativePath), /"1\.1\.29"/u, relativePath);
    assert.doesNotMatch(read(relativePath), /"1\.1\.28"/u, relativePath);
  }
  for (const relativePath of [
  ...
  assert.doesNotMatch(priorMigration, /1\.1\.29/u);
});

test("1.1.29 cross-page repeat acceptance is present in the fingerprinted runtime files", () => {
  const contract = read("tools/naver-shopping-rank-collector/src/contract.mjs");
  assert.match(contract, /for \(const item of normalizedItems\) renderedOrderIdentitySignal\(item\);/u);
  assert.match(contract, /if \(value\.renderedOrderProof === undefined\) \{\n      crossPageProof = validateStableFullWindowProof/u);
  assert.doesNotMatch(contract, /\|\| crossPageDuplicate\n      \|\| crossPageProof/u);
  const provider = read("tools/naver-shopping-rank-collector/src/provider.mjs");
  assert.match(provider, /if \(collisionKind !== "duplicate_row" && !preserveStableCrossPage && !rejectAllIdentityDuplicates\) \{/u);
  const handler = read("src/server/handlers/naver-shopping-rank.mjs");
  assert.match(handler, /const stableCrossPageProof = crossPageDuplicate\n    && payload\?\.renderedOrderProof === undefined/u);
  const worker = read("scripts/naver-shopping-local-worker.mjs");
  assert.match(worker, /if \(baseCode === "provider_stable_finite_window_unproven"\) \{/u);
});
