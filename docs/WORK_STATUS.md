...
- 상태: 원부 제목에 검색 키워드가 없더라도 정확 상품과 모델번호·브랜드·상위 카테고리가 일치하면 관련 원부로 연결하도록 보강. `써큘레이터`는 원부 8위·정확 상품 59위 중 원부를 대표 순위로 선택하며 Production 배포와 운영팀·광고주 재검증 완료
- 차단 사항: 네이버 쇼핑 자동 화면 접근은 CAPTCHA·HTTP 405/418 제한이 있어 우회하지 않는다. 현재 결과는 공식 검색 API와 로그인 아이템스카우트의 원부 9위·정확 상품 60위를 교차검증했으며, 기존 쇼핑 검색 API는 2026-07-31 종료 예정
- 사용자 배포 승인: 양 역할 실조회·링크·반응형·콘솔, 회귀 검사, 실행환경·Supabase 검증을 모두 통과해 Production 배포와 운영 재검증 완료
