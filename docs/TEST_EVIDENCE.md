...
- 자동 검증: API·서버 153/153, 플레이스 tracker 계약 42/42, 수집기 40/40, 서버 계약 22/22, Production 인증 18/18, 역할 parity, 순위 매칭, 릴리즈 기준선, CSP 공개 빌드, `git diff --check` 통과. 미리보기 오인, ID 없는 실제 행의 순위 압축, body 메뉴·중첩 프로모션의 순위 팽창, 가상 목록 끝 점프, drain-mode degraded summary 전달을 각각 회귀 테스트로 고정했다.
