...
- SEO 진단 v2 트래픽·구매·리뷰 신호 강화 완료·운영 반영: 리뷰 수가 많을수록 리뷰 신뢰 점수와 총점이 상승하도록 독립 신호를 유지하고, 최근 30일 유입수·구매수로 트래픽과 전환율을 별도 평가한다. 상품명·카테고리·시장 수요·리뷰를 근거별로 표시하며 API 결과 순번은 점수에 포함하지 않는 참고값이다. 리뷰만 입력해도 점수는 상승하지만 데이터 신뢰도는 15%로 유지되어 `데이터 추가 필요` 상태를 벗어나지 않는다.
- 역할·안전 검증과 배포: 운영팀·광고주가 같은 `seo-evaluation.js` 평가 엔진을 사용하며 데스크톱과 광고주 390px UI를 2차 확인했다. 리뷰·트래픽·전환·미입력 신뢰도 회귀 7/7, API·서버 186/186, 플레이스 수집기 51/51, 서버 계약 23/23, Production 인증 18/18, 4대 기능 잠금 13함수·21파일·11마이그레이션, 역할 parity·공개 빌드·CSP·전체 `check:release`를 통과했다. 기능 코드 `109191d`를 Production `momentinsight-eseg8dnw8-momentlabs.vercel.app`과 운영 별칭에 반영했고 `/health` live·`/ready` Supabase ready, 양 역할 HTML과 공통 엔진의 배포 산출물 일치를 확인했다.
