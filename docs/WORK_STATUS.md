...
- NAVER API Hub 이관 어댑터 준비 완료·운영 전환 보류: Search Trend·Shopping Insight·blog/local 요청을 `legacy|hub|auto` 모드와 공급자별 인증 헤더·기본 주소·경로로 분리했다. 기본값과 잘못된 모드는 `legacy`로 닫아 Hub 환경변수 추가만으로 운영 트래픽이 바뀌지 않으며, Hub 인증 실패·종료·호출 한도·일시 장애를 401/403·404/410·429·5xx로 구분한다. 통합 상태와 런타임 환경 검사도 두 공급자를 별도로 표시한다.
...
- 검증 상태: 새 어댑터·핸들러·상태 API 대상 회귀 66/66, 전체 API·서버 178/178, 플레이스 수집기 51/51, 서버 계약 23/23, Production 인증 18/18, 보호 잠금·공개 빌드·전체 `check:release`가 통과했다. 로컬 legacy `써큘레이터?profile=trend` 실호출은 HTTP 200·provider `legacy`·Search Trend/Shopping Insight 정상·경고 0건이다. NCP Hub 실키가 없어 새 인증 실호출은 아직 완료하지 않았고, Production 배포·모드 전환도 보류한다.
