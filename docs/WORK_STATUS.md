...
- 검증 상태: 1280×720 브라우저에서 총관리자 생성 카드 2개가 각각 923×115px, 입력 5개와 CTA 2개가 모두 40px 높이, CTA 폭 116px·오른쪽 기준선 동일, 가로 넘침 0으로 확인됐다. 총관리자 전용 요소는 Owner 10개 노출·운영팀 0개 노출이며 운영팀 화면 제목·연결 입력 숨김도 기존 기준과 일치한다. 로그인 없는 격리 화면 캡처 2개를 별도 제공한다. 역할 5상태, 운영팀·광고주 parity, 보호 잠금 21함수·23파일·11마이그레이션, API·서버 226/226, 플레이스 수집기 51/51, 서버 계약 28/28, Production 인증 18/18, 공개 빌드/CSP와 전체 `check:release`가 통과했다. 배포 승인은 없으므로 Production은 변경하지 않는다.
