...
- 전체 `check:release` 통과 뒤 exact owner canary만 Production에 배포합니다. 총관리자 실세션에서 오늘 브리핑, 날짜 있는 일정 초안, 날짜 없는 확인 필요, 명시적 내부 일정 1건 등록, 새로고침 후 동일 일정 확인, 로그아웃 제거와 모바일 가로 넘침을 확인합니다.
