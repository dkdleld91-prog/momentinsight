...
<!-- autosave:start -->
- 마지막 자동 저장: 2026. 06. 25. 17:35:25
- 기준 커밋: d2fee24
- 작업트리: M 02_아임웹_적용코드/복붙용_광고주형_CODE.txt /  M 02_아임웹_적용코드/아임웹_원샷코드_대시보드형_모먼트인사이트.html /  M docs/08-work-spec-autosave.md /  M src/server/handlers/agency-code-api.mjs /  M src/server/handlers/client-api.mjs /  M src/server/handlers/naver-rank-trackers.mjs
<!-- autosave:end -->
...
| 상태 | 작업 | 핵심 내용 | 검증 | 배포 |
| --- | --- | --- | --- | --- |
| 완료 | 광고주 코드 비노출 및 순위추적 권한 보강 | 광고주 대행사 연결 화면과 공개 검증 API에서 운영팀/대행사 코드 노출을 제거하고, 표시용 대행사명과 연동 상태만 노출하도록 정리. 순위추적은 active 광고주와 동일 접근 코드 조합만 허용 | `build:vercel`, `check:env`, 정적 산출물 코드 노출 검색, 로컬 API validate/401/200 테스트, 브라우저 로그인/비노출/새로고침 잠금 확인 | 배포 대기 |
