-- Runtime 1.1.31 (2026-09-18): failure evidence v2, third stable-window capture, finite counter jitter.
-- Four days of 1.1.30 (2026-09-14 14:19 ~ 09-18 10:02 KST: 681 commits, 20 failures, 2.9%)
-- showed three things. (1) The evidence trim kept the paid rows at the top of every page
-- and dropped the organic rows where two captures differed (all six
-- `provider_stable_window_unproven:digest_mismatch` rows were cut to 14 rows a page); the
-- trim now drops paid/helper rows first, the bound is 24000 chars, and every failure also
-- carries the collector's branch trace and the slot-level diff of the last digest
-- comparison (collection-evidence-v2). (2) The finite arbitration reason `three_passes`
-- was missing from the worker's vocabulary (2026-09-14 19:07, 09-16 14:30 탄소매트 were
-- recorded without a reason). (3) A live market of two million results moves between two
-- captures (러그·칼슘쌀·콘트로이친·키크는): the stable full-window proof now gets the same
-- bounded third capture the finite and rendered-order proofs have, and a finite market
-- whose live counter jitters within the shared 1% tolerance (탄소매트 223/222/223 over
-- 222 rendered rows) is provable.
...
-- Apply right after the 1.1.31 server release is live and the coordination row
-- is idle on 1.1.30 (the release itself stops the 1.1.30 worker at the HTTP gate).
begin;
...
begin
  ...
  if active_request_count <> 0 or unfinished_member_count <> 0 then
    raise exception 'naver_shopping_runtime_1_1_31_requires_completed_account_priority';
  end if;

  if coordination_found is not true
    or current_row.runtime_version is distinct from '1.1.30'
    ...
    raise exception 'naver_shopping_runtime_1_1_31_requires_idle_control_plane';
  end if;
end
$migration_guard$;
...
begin
  ...
  if exists (
    select 1
    from public.naver_shopping_finite_window_targets
    where runtime_version is distinct from '1.1.30'
       or runtime_fingerprint is distinct from
         '62e09cb15720dda97fdc652cfa9b3fba40bdbbc4c98372269867c21ee472d10c'
  ) then
    raise exception 'naver_shopping_runtime_1_1_31_finite_target_identity_mismatch';
  end if;

  update public.naver_shopping_finite_window_targets
  set runtime_version = '1.1.31',
      runtime_fingerprint =
        '099bf53085a118ee3917c5f5596e72a08efe91faa3247b9091ddb27241149116'
  where runtime_version = '1.1.30'
    and runtime_fingerprint = '62e09cb15720dda97fdc652cfa9b3fba40bdbbc4c98372269867c21ee472d10c';
  get diagnostics target_updated_count = row_count;

  if target_updated_count <> prior_target_count then
    raise exception 'naver_shopping_runtime_1_1_31_target_mismatch';
  end if;
end
$target_transition$;
...
begin
  ...
  where lane_key = 'global'
    ...
    and runtime_version = '1.1.30'
    and runtime_fingerprint = '62e09cb15720dda97fdc652cfa9b3fba40bdbbc4c98372269867c21ee472d10c';
  get diagnostics coordination_updated_count = row_count;

  if coordination_updated_count <> 1 then
    raise exception 'naver_shopping_runtime_1_1_31_coordination_mismatch';
  end if;
end
...
as $$
declare
  expected_runtime_version constant text := '1.1.31';
