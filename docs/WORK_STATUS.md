...
- 상태: 안전 복구 구현·집중 회귀 통과 / 승인된 Production 배포 진행 중.
...
- 네이티브 호스트·로컬 워커 집중 회귀 32/32, API·서버 397/397, 플레이스 51/51, 쇼핑 51/51, 서버 계약 38/38, Production 인증 18/18과 전체 `npm run check:release`를 통과했다. 운영 배포 증거는 아래 배포 단계에서 확정한다.
